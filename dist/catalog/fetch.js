...
function git(args, cwd) {
    const proc = spawnSync('git', args, { cwd, encoding: 'utf-8', timeout: 300_000 });
    if (proc.error) {
        const code = proc.error.code;
        if (code === 'ENOENT') {
            throw new FetchError(`git is required to install catalog workflows but was not found on PATH.`);
        }
        throw new FetchError(`git ${args[0]} failed: ${proc.error.message}`);
    }
    return { status: proc.status ?? -1, stderr: (proc.stderr ?? '').trim() };
}
export function fetchSourceTree(source, scratchParent) {
    const cloneDir = mkdtempSync(join(scratchParent, 'riglane-fetch-'));
    let r = git(['init', '--quiet'], cloneDir);
    if (r.status !== 0)
        throw new FetchError(`git init failed: ${r.stderr}`);
    r = git(['remote', 'add', 'origin', source.repo], cloneDir);
    if (r.status !== 0)
        throw new FetchError(`git remote add failed: ${r.stderr}`);
    r = git(['fetch', '--quiet', '--depth', '1', 'origin', source.sha], cloneDir);
    if (r.status !== 0) {
        r = git(['fetch', '--quiet', 'origin'], cloneDir);
        if (r.status !== 0) {
            throw new FetchError(`cannot fetch ${source.repo}: ${r.stderr}\n` +
    ...
    if (r.status !== 0) {
        throw new FetchError(`commit ${source.sha} does not exist in ${source.repo} (or is not reachable): ${r.stderr}\n` +
    ...
    if (!existsSync(join(workflowDir, 'workflow.yaml'))) {
        throw new FetchError(`no workflow.yaml at '${source.path || '.'}' inside ${source.repo} @ ${source.sha.slice(0, 12)}… — ` +
