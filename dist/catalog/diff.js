const deciderKey = (d) => `${d.field} @ ${d.at}`;
const flagKey = (f) => `${f.flag}${f.where}${f.match}`;
const isExecutable = (b) => b.role === 'script' || b.role === 'mcp-server';
export function diffInventories(oldInv, newInv) {
    ...
    const shellSurfaceChanged = tools.added.length + tools.removed.length + tools.changed.length > 0 ||
