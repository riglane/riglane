...
function hasGlobChar(path) {
    return path.includes('*') || path.includes('?') || path.includes('[');
}
export function normalizePerIterationOutputs(workflow) {
    for (const step of collectAllSteps({ steps: workflow.steps })) {
        const outputs = step.outputs;
        if (!Array.isArray(outputs))
            continue;
        for (const out of outputs) {
            if (out === null || typeof out !== 'object' || out.per_iteration !== true)
                continue;
            const p = out.path;
            if (typeof p !== 'string' || p.length === 0 || p.includes('{iteration}'))
                continue;
...
export function injectSpecAuthoringOutputs(workflow) {
    {
        for (const step of collectAllSteps({ steps: workflow.steps })) {
            if (step.spec_authoring !== 'persist')
                continue;
            const outputs = Array.isArray(step.outputs)
                ? step.outputs
                : [];
            const manual = outputs.filter((o) => typeof o.path === 'string' && o.path.includes('.riglane/specs/'));
            if (manual.length > 0) {
                const injected = new Set([SPEC_OUTPUT_SCOPED, SPEC_OUTPUT_GENERIC]);
                const foreign = manual.filter((o) => !injected.has(o.path));
                if (foreign.length > 0 && manual.length === foreign.length) {
                    // eslint-disable-next-line no-console
                    console.error(`[workflow-engine] Warning: step '${String(step.name)}' declares manual ` +
                ...
                if (foreign.length === 0 && manual.length === 2)
                    continue;
            }
...
export function resolveOutputPath(pathTemplate, runtimeDir) {
...
export function isTouched(current, baseline, stepStartedAt) {
    if (!current.exists)
        return false;
...
function nowIsoLocalWithOffset(d) {
...
export function saveSnapshot(runtimeDir, stepName, snapshot) {
    const path = snapshotPath(runtimeDir, stepName);
...
export function deleteSnapshot(runtimeDir, stepName) {
    try {
...
export function createSnapshot(outputs, runtimeDir, stepName, branches) {
    ...
    const snapshotOutputList = (outs, baseDir) => {
        const result = {};
        for (const out of outs) {
            ...
            for (const mp of matched)
                baseline[mp] = computeFileFingerprint(mp);
            result[opath] = {
                write_proof: wp,
                baseline,
            };
...
export class MutableValidationResult {
    ...
    extend(checks, failures, details) {
        this.checks += checks;
...
export function narrowOutputsForBranch(outputs, branchIndex) {
    const narrowed = [];
...
export function branchOutputsFromResolved(declaredOutputs, resolvedOutputs) {
    const byDeclared = new Map();
    for (const raw of declaredOutputs) {
        if (typeof raw === 'object' && raw !== null)
            byDeclared.set(raw.path, { ...raw });
        else
            byDeclared.set(String(raw), { path: String(raw) });
    }
    return resolvedOutputs.map((ro) => {
        const base = byDeclared.get(ro.declared);
        const merged = base ? { ...base, path: ro.working } : { path: ro.working };
        return typeof ro.struct === 'string' && ro.struct.length > 0
            ? { ...merged, struct: ro.struct }
            : merged;
    });
}
export function validateOutputs(outputs, definitionDir, runtimeDir, options) {
    ...
    for (const output of outputs) {
        ...
        if (files.length === 0 && !isOptional && waitForFiles) {
            let maxWaitMs;
            try {
                const { gateFileWaitMs } = require('../config/config.js');
            ...
            if (Number.isFinite(maxWaitMs) && maxWaitMs > 0) {
                ...
                if (files.length > 0) {
                    result.addDetail(`[late write] output '${resolvedPath}' appeared after ${elapsed}ms — ` +
                        `waited for the subagent's write to flush.`);
                }
            }
        }
        if (isOptional && files.length === 0) {
            result.addDetail(`optional output '${resolvedPath}' absent at gate time — checks skipped ` +
...
function checkWriteProof(outputPath, wpMode, currentFiles, baselineEntry, stepStartedAt, result) {
    const baseline = baselineEntry?.baseline ?? {};
    if (wpMode === 'required') {
        if (currentFiles.length === 0) {
            result.addCheck(false, `write_proof: required output '${outputPath}' not found on disk. Fix: subagent must produce this file during the step.`);
            return;
        }
        for (const fp of currentFiles) {
            ...
            if (touched) {
                ...
                result.addCheck(false, `write_proof failure: '${fp}' was not modified during step execution (hash unchanged, mtime not advanced past step start). Fix: subagent must write this file — claiming success without writing is not acceptable.`);
        ...
        if (currentFiles.length === 0) {
            result.addCheck(false, `write_proof: no files matching '${outputPath}' after step. Fix: subagent must produce at least one matching file.`);
            return;
        }
        for (const fp of currentFiles) {
            const cur = computeFileFingerprint(fp);
            ...
            if (touched) {
                ...
                result.addCheck(false, `write_proof (all_members_fresh): '${fp}' matches glob '${outputPath}' but was not modified in this step (stale from previous run). Fix: either rewrite this file, or ensure workflow_init clears stale outputs before the step.`);
                result.proofResults.push({ path: fp, mode: wpMode, status: 'stale' });
            }
        }
    }
        ...
        if (anyTouched) {
            ...
            result.addCheck(false, `write_proof (any_member): no files matching '${outputPath}' were modified in this step. Fix: subagent must write at least one matching file.`);
        }
    }
}
