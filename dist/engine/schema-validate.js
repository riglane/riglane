...
import { extractSectionContent, parseMarkdownFrontmatter, parseMarkdownSections, } from './markdown.js';
...
function pythonReMatch(pattern, value) {
...
export function detectFormat(schema) {
    if (schema.format)
...
export function validateStandardJsonSchema(data, schema, filename) {
    let checks = 0;
    let failures = 0;
    const details = [];
        ...
        for (const [fieldName, fieldSchema] of Object.entries(properties)) {
            ...
            if (expectedType !== undefined) {
                ...
                if (!checkType(value, expectedType)) {
                    failures += 1;
                    details.push(`${filename}: '${fieldName}' type mismatch ` +
            ...
            if (fieldSchema.enum !== undefined) {
                ...
                if (!fieldSchema.enum.includes(value)) {
                    failures += 1;
                    details.push(`${filename}: '${fieldName}' value '${value}' not in ${JSON.stringify(fieldSchema.enum)}`);
                }
            }
            if (fieldSchema.pattern !== undefined && typeof value === 'string') {
                ...
                if (!pythonReMatch(fieldSchema.pattern, value)) {
                    failures += 1;
                    details.push(`${filename}: '${fieldName}' doesn't match pattern '${fieldSchema.pattern}'`);
                }
            }
            if (fieldSchema.min_items !== undefined && Array.isArray(value)) {
                ...
                if (value.length < fieldSchema.min_items) {
                    failures += 1;
                    details.push(`${filename}: '${fieldName}' has ${value.length} items, ` +
            ...
            if (fieldSchema.minimum !== undefined && typeof value === 'number') {
                ...
                if (value < fieldSchema.minimum) {
                    failures += 1;
                    details.push(`${filename}: '${fieldName}' value ${value} is below minimum ${fieldSchema.minimum}`);
                }
            }
            if (fieldSchema.maximum !== undefined && typeof value === 'number') {
                ...
                if (value > fieldSchema.maximum) {
                    failures += 1;
                    details.push(`${filename}: '${fieldName}' value ${value} exceeds maximum ${fieldSchema.maximum}`);
                }
            }
            if (fieldSchema.minLength !== undefined && typeof value === 'string') {
                ...
                if (value.length < fieldSchema.minLength) {
                    failures += 1;
                    details.push(`${filename}: '${fieldName}' length ${value.length} is below minLength ${fieldSchema.minLength}`);
                }
            }
            if (fieldSchema.maxLength !== undefined && typeof value === 'string') {
                ...
                if (value.length > fieldSchema.maxLength) {
                    failures += 1;
                    details.push(`${filename}: '${fieldName}' length ${value.length} exceeds maxLength ${fieldSchema.maxLength}`);
                }
            }
            if (expectedType === 'object' && isPlainObject(value)) {
                if (fieldSchema.required !== undefined || fieldSchema.properties !== undefined) {
                    const nested = validateStandardJsonSchema(value, fieldSchema, `${filename}.${fieldName}`);
            ...
            if (expectedType === 'array' && Array.isArray(value)) {
                ...
                if (itemsSchema !== undefined &&
                    ...
                    for (let i = 0; i < value.length; i += 1) {
                        const nested = validateStandardJsonSchema(value[i], itemsSchema, `${filename}.${fieldName}[${i}]`);
                        checks += nested.checks;
                        failures += nested.failures;
                        details.push(...nested.details);
                    }
...
function validateArrayField(parentData, fullPath, fieldDef, filename) {
    let checks = 0;
    let failures = 0;
    const details = [];
    ...
    if (!arrLookup.found || !Array.isArray(arrLookup.value)) {
        failures += 1;
        details.push(`${filename}: required array '${arrPath}' not found (or value is not an array). Fix: add '${arrPath}' as an array at the appropriate path.`);
    ...
    if (fieldDef.min_items !== undefined && arrValue.length < fieldDef.min_items) {
        checks += 1;
        failures += 1;
        details.push(`${filename}: array '${arrPath}' has ${arrValue.length} items ` +
    ...
    for (let i = 0; i < arrValue.length; i += 1) {
        ...
        if (!itemLookup.found) {
            checks += 1;
            failures += 1;
            details.push(`${filename}: array item '${arrPath}[${i}]' is missing required field '${itemField}'. ` +
                `Fix: add '${itemField}: <value>' to item at index ${i}.`);
        }
        else if (fieldDef.type !== undefined && !checkType(itemLookup.value, fieldDef.type)) {
            checks += 1;
            failures += 1;
            details.push(`${filename}: array item '${arrPath}[${i}].${itemField}' has wrong type (expected '${fieldDef.type}', got '${jsonTypeName(itemLookup.value)}'). Fix: change the value to match the expected type.`);
        }
        else if (fieldDef.enum !== undefined && !fieldDef.enum.includes(itemLookup.value)) {
            checks += 1;
            failures += 1;
            details.push(`${filename}: array item '${arrPath}[${i}].${itemField}' = ` +
...
export function validateFile(filePath, schema) {
    let checks = 0;
    let failures = 0;
    const details = [];
    ...
    const getSize = () => {
        if (cachedSize === null)
            cachedSize = statSync(filePath).size;
        return cachedSize;
    };
    if (fileChecks.min_size !== undefined) {
        ...
        if (size < fileChecks.min_size) {
            failures += 1;
            details.push(`${filename}: file is too small (${size} bytes, minimum ${fileChecks.min_size}). Likely cause: required content is missing or sections are empty. Fix: ensure all required sections have substantive content.`);
        }
    }
    if (fileChecks.max_size !== undefined) {
        ...
        if (size > fileChecks.max_size) {
            failures += 1;
            details.push(`${filename}: file is too large (${size} bytes, maximum ${fileChecks.max_size}). Fix: split the content into multiple files or remove redundant sections.`);
        }
    }
    if (fileChecks.name_pattern !== undefined) {
        ...
        if (!pythonReMatch(fileChecks.name_pattern, filename)) {
            failures += 1;
            details.push(`${filename}: filename does not match required pattern '${fileChecks.name_pattern}'. Fix: rename the file so its basename matches this regex.`);
        }
    }
    ...
    if (fmt === 'json') {
        ...
        try {
            ...
            details.push(`${filename}: invalid JSON — ${msg}. Fix: check the file for JSON syntax errors (missing commas, unmatched braces/brackets, trailing commas, unquoted keys).`);
            return { passed: false, checks, failures, details };
        }
        ...
        for (const fieldDef of jsonSchema.required_fields ?? []) {
            ...
            if (fieldName.includes('[].')) {
                ...
                if (!lookup.found) {
                    failures += 1;
                    details.push(`${filename}: required field '${fieldName}' is missing. ` +
                ...
                if (fieldDef.type !== undefined && !checkType(value, fieldDef.type)) {
                    checks += 1;
                    failures += 1;
                    details.push(`${filename}: field '${fieldName}' has wrong type (expected '${fieldDef.type}', got '${jsonTypeName(value)}'). Fix: change the value to match the expected type.`);
                }
                if (fieldDef.pattern !== undefined && typeof value === 'string') {
                    checks += 1;
                    if (!pythonReMatch(fieldDef.pattern, value)) {
                        failures += 1;
                        details.push(`${filename}: field '${fieldName}' = '${value}' does not match ` +
                ...
                if (fieldDef.enum !== undefined && !fieldDef.enum.includes(value)) {
                    checks += 1;
                    failures += 1;
                    details.push(`${filename}: field '${fieldName}' = '${String(value)}' is not allowed. ` +
                        `Fix: choose one of ${JSON.stringify(fieldDef.enum)}.`);
                }
                if (fieldDef.min_items !== undefined && Array.isArray(value)) {
                    ...
                    if (value.length < fieldDef.min_items) {
                        failures += 1;
                        details.push(`${filename}: array '${fieldName}' has ${value.length} items ` +
                            `(minimum required: ${fieldDef.min_items}). ` +
                            `Fix: add more items until the array has at least ${fieldDef.min_items} entries.`);
                    }
                }
        ...
        try {
            ...
            failures += 1;
            const msg = e instanceof Error ? e.message : String(e);
            details.push(`${filename}: failed to parse markdown file — ${msg}. Fix: ensure the file has valid YAML frontmatter between '---' markers at the top, followed by markdown body content.`);
            return { passed: false, checks, failures, details };
        }
        ...
        if (frontmatterSchema.required !== undefined && frontmatterSchema.required.length > 0) {
            if (frontmatter === null) {
                checks += 1;
                failures += 1;
                details.push(`${filename}: YAML frontmatter block is missing. Fix: add a frontmatter block at the very top of the file, delimited by '---' on its own line before and after the YAML.`);
            }
            else {
                for (const fieldDef of frontmatterSchema.required) {
                    const fieldName = fieldDef.field;
                    checks += 1;
                    if (!(fieldName in frontmatter)) {
                        const allowed = fieldDef.values;
                        const hint = allowed !== undefined ? ` Allowed values: ${JSON.stringify(allowed)}.` : '';
                        failures += 1;
                        details.push(`${filename}: required frontmatter field '${fieldName}' is missing. ` +
                    ...
                    if (fieldDef.type !== undefined && !checkType(value, fieldDef.type)) {
                        checks += 1;
                        failures += 1;
                        details.push(`${filename}: frontmatter '${fieldName}' has wrong type (expected '${fieldDef.type}', got '${jsonTypeName(value)}'). Fix: change the value format to match the expected type.`);
                    }
                    if (fieldDef.pattern !== undefined && typeof value === 'string') {
                        checks += 1;
                        if (!pythonReMatch(fieldDef.pattern, value)) {
                            failures += 1;
                            details.push(`${filename}: frontmatter '${fieldName}' = '${value}' does not ` +
                    ...
                    if (fieldDef.values !== undefined && !fieldDef.values.includes(value)) {
                        checks += 1;
                        failures += 1;
                        details.push(`${filename}: frontmatter '${fieldName}' = '${String(value)}' is not allowed. ` +
                    ...
                    if (fieldDef.body_match_section !== undefined && typeof value === 'string') {
                        ...
                        if (sectionText !== null && !sectionText.toLowerCase().includes(value.toLowerCase())) {
                            failures += 1;
                            details.push(`${filename}: frontmatter declares '${fieldName}: ${value}' but the word '${value}' does not appear literally in the body of '## ${targetSection}'. Fix: rewrite the '## ${targetSection}' body to use '${value}' directly (e.g., 'The system ${value} do X').`);
                        }
                    }
                }
            }
        ...
        if (requiredSections.length > 0) {
            ...
            for (const section of requiredSections) {
                ...
                if (!sections.includes(section)) {
                    failures += 1;
                    details.push(`${filename}: required markdown section '## ${section}' is missing. Fix: add '## ${section}' as a heading at the top level (##, not ### or deeper), followed by body content for that section.`);
                }
            }
        }
    }
        ...
        try {
            ...
            failures += 1;
            const msg = e instanceof Error ? e.message : String(e);
            details.push(`${filename}: invalid YAML — ${msg}. Fix: check the file for YAML syntax errors (indentation, unmatched quotes, tabs mixed with spaces).`);
            return { passed: false, checks, failures, details };
        }
        ...
        for (const fieldDef of yamlSchema.required_fields ?? []) {
            ...
            if (!lookup.found) {
                failures += 1;
                details.push(`${filename}: required YAML field '${fieldName}' is missing. ` +
