...
function collectStepsWithCtx(workflow) {
    const out = [];
    const visit = (steps, seq) => {
...
function knownParams(workflow) {
    const names = new Set(['run_id', 'iteration']);
    for (const p of (workflow.params ?? [])) {
        if (typeof p.name === 'string')
            names.add(p.name);
    }
    for (const step of collectAllSteps({ steps: workflow.steps })) {
        for (const key of Object.keys((step.param_bindings ?? {}))) {
            names.add(key);
        }
    }
    return names;
...
function ackSet(container) {
    const raw = container?.acknowledge_warnings;
    return new Set(Array.isArray(raw) ? raw.filter((x) => typeof x === 'string') : []);
}
function countDeclaredOutputs(workflow) {
    let count = 0;
...
export const STRUCT_FIELD_CONTAINERS = [
    'properties',
    'json_schema',
    'yaml_schema',
    'frontmatter',
...
export function structHasFieldContainer(parsed) {
    if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed))
        return false;
    const obj = parsed;
    return STRUCT_FIELD_CONTAINERS.some((k) => k in obj);
}
export function structUnknownTopLevelKeys(parsed) {
    const outcome = tryValidateStruct(parsed);
    if (outcome.ok)
        return [];
    return outcome.errors
        .filter((err) => err.keyword === 'additionalProperties')
        .map((err) => String(err.params.additionalProperty ?? ''))
        .filter((k) => k.length > 0);
}
export function isLocalWebhookUrl(url) {
    return /^https?:\/\/(127\.0\.0\.1|localhost|\[::1\])([:/]|$)/i.test(url);
}
export const LINT_WARNING_IDS = [
    'inline-agent-gate',
    'input-token',
    'goal-token',
    'delegate-target',
...
export function lintWorkflow(workflow, opts = {}) {
    ...
    };
    {
        ...
        if (typeof hook === 'string' && hook && !isLocalWebhookUrl(hook)) {
            push({
                id: 'webhook-url',
                message: `workflow inbox_webhook points at a non-localhost URL ('${hook}') inside a ` +
    ...
    for (const { step, name } of steps) {
        ...
        if (opts.definitionDir !== undefined) {
            ...
            for (const entry of [...inputs, ...outputs]) {
                const structName = typeof entry.struct === 'string' ? entry.struct : '';
                if (structName === '' || warnedStructs.has(structName))
                    continue;
                ...
                if (!structHasFieldContainer(parsed))
                    continue;
                ...
                push({
                    ...
                    step: name,
                    message: `Struct '${structName}' (structs/${structName}.schema.yaml) carries top-level ` +
        ...
        for (const blockKey of ['loop_back', 'routes']) {
            const block = step[blockKey];
            const when = (block?.when ?? undefined);
            ...
            if (firstTok && /\.(py|js|mjs|cjs|sh|ps1|rb|pl)$/i.test(firstTok)) {
                push({
                    ...
                    step: name,
                    message: `Step '${name}' ${blockKey}.when.script starts with the script file itself ` +
        ...
        if (inline && (gate.semantic === true || gate.human === true)) {
            const wfGateLint = (workflow.gate ?? {});
        ...
        if (inline) {
            ...
            if (typeof step.model === 'string')
                noop.push('model');
            if ('max_gate_retries' in gate)
                noop.push('gate.max_gate_retries');
            if ('max_step_retries' in gate)
                noop.push('gate.max_step_retries');
            if ('allow_partial_step_complete' in gate)
                noop.push('gate.allow_partial_step_complete');
            if (Array.isArray(step.deny) && step.deny.length > 0)
                noop.push('deny');
            if (noop.length > 0) {
                push({
                    id: 'inline-noop-fields',
                    step: name,
                    message: `Step '${name}' is inline (subagent: false) but declares ${noop.join(', ')} — ` +
        ...
        if (!isPlanning) {
            const pf = ['max_substeps', 'max_plan_attempts', 'allow_parallel', 'allow_delegation'].filter((f) => f in step);
            if (pf.length > 0) {
                push({
                    id: 'planning-fields',
                    step: name,
                    message: `Step '${name}' declares ${pf.join(', ')} but is not a planning step — these ` +
        ...
        if ((isPlanning || isDelegation) && Array.isArray(step.tools)) {
            push({
                id: 'tools-on-nonexec',
                step: name,
                message: `Step '${name}' declares a tools: filter but is a ${isPlanning ? 'planning' : 'delegation'} ` +
        ...
        if (!isPlanning && !isDelegation && step.spec_authoring !== 'persist' && outputs.length === 0) {
            push({
                id: 'no-outputs',
                step: name,
                message: `Step '${name}' declares no outputs — the structural gate has nothing to verify, ` +
        ...
        if (outputs.length > 0 && outputs.every((o) => o.optional === true)) {
            push({
                id: 'all-optional',
                step: name,
                message: `Every output of step '${name}' is optional: true — the gate cannot fail on ` +
        ...
        for (const o of outputs) {
            ...
            if (p.endsWith('.json') && !('struct' in o)) {
                push({
                    id: 'json-no-struct',
                    step: name,
                    message: `Output '${p}' of step '${name}' is JSON but declares no struct: — the gate ` +
        ...
        if (goal.includes('{{')) {
            push({
                id: 'goal-token',
                step: name,
                message: `Goal of step '${name}' contains '{{ ... }}' — Riglane placeholders are SINGLE-brace ` +
        ...
        for (const inp of (step.inputs ?? [])) {
            ...
            if (p.includes('{{') || badTokens.length > 0 || hasParallelNs) {
                ...
                push({
                    id: 'input-token',
                    step: name,
                    message: `Input path '${p}' of step '${name}' contains ${what} — the token stays ` +
        ...
        for (const o of outputs) {
            if (typeof o.from_delegated === 'string' && typeof o.write_proof === 'string') {
                push({
                    ...
                    step: name,
                    message: `Step '${name}' declares write_proof on a from_delegated output ('${String(o.path)}') — ` +
        ...
        if (isDelegation && opts.definitionDir !== undefined) {
            ...
            const candidates = ['my_workflows', 'predefined', 'examples'].map((fam) => join(templatesDir, fam, targetName, 'workflow.yaml'));
            const found = candidates.find((c) => existsSync(c));
            if (found === undefined) {
                push({
                    id: 'delegate-target',
                    step: name,
                    message: `Step '${name}' delegates to '${targetName}', which was not found under the ` +
                ...
                try {
                    const target = loadYaml(found);
                    const targetParams = new Set((target.params ?? [])
                        .map((p) => (typeof p.name === 'string' ? p.name : ''))
                        .filter((n) => n !== ''));
                    const passed = Object.keys((step.params ?? {}));
                    const unknown = passed.filter((k) => !targetParams.has(k));
                    if (unknown.length > 0) {
                        push({
                            id: 'delegate-param',
                            step: name,
                            message: `Step '${name}' passes param '${unknown[0]}' to '${targetName}', but the ` +
                    ...
                    try {
                        ...
                        if (childOutputCount === 0) {
                            push({
                                id: 'handoff-source',
                                step: name,
                                message: `Step '${name}' declares from_delegated, but the delegated workflow ` +
    ...
    for (const p of (workflow.params ?? [])) {
        if (p.default === '') {
            push({
                id: 'empty-default',
                message: `Param '${String(p.name)}' has default: "" — an empty string is a REAL value at ` +
    ...
    }
    {
        ...
        const wfNorm = norm(workflow.name ?? '');
        for (const t of (workflow.tools ?? [])) {
            ...
            if (surfaced.length > 64) {
                push({
                    id: 'tool-name-length',
                    message: `Script tool '${tn}' surfaces to the model as '${surfaced}' ` +
    ...
    {
        const loopSeqs = new Set(steps.filter((s) => s.step.loop_back !== undefined).map((s) => s.sequence));
        const bySeq = new Map();
        for (const { step, name, sequence } of steps) {
            if (loopSeqs.has(sequence))
                continue;
            for (const o of (step.outputs ?? [])) {
                const p = typeof o.path === 'string' ? o.path : '';
                if (p === '' || /[*?[]/.test(p))
                    continue;
                const seqMap = bySeq.get(sequence) ?? new Map();
                ...
                if (first !== undefined && first !== name) {
                    push({
                        id: 'dup-output',
                        step: name,
                        message: `Steps '${first}' and '${name}' both declare output '${p}' in the same ` +
    ...
    }
    {
        ...
        for (const [, arr] of seqSteps) {
            ...
            for (let i = 0; i < arr.length; i += 1) {
                ...
                if (entry === undefined)
                    continue;
                const outs = (entry.step.outputs ?? []);
                const hasPerIter = outs.some((o) => o.per_iteration === true ||
                    (typeof o.path === 'string' && o.path.includes('{iteration}')));
                if (!hasPerIter)
                    continue;
                ...
                if (!inRange) {
                    push({
                        id: 'periter-outside',
                        step: entry.name,
                        message: `Step '${entry.name}' declares a per-iteration output but sits OUTSIDE the ` +
    ...
    for (const t of (workflow.tools ?? [])) {
        ...
        if (t.type === 'script' && 'expected_tools' in t) {
            push({
                id: 'tool-cross-type',
                message: `Script tool '${tname}' declares expected_tools — that field is consumed only for ` +
        ...
        if (t.type === 'mcp' && 'input_schema' in t) {
            push({
                id: 'tool-cross-type',
                message: `MCP tool '${tname}' declares input_schema — that field is consumed only for ` +
    ...
    {
        const lanesOwners = collectAllSteps({ steps: workflow.steps }).filter((s) => s.lanes?.define !== undefined);
        for (const owner of lanesOwners) {
            const define = (owner.lanes.define ?? []);
            ...
            for (const lane of define) {
                ...
                for (const s of subtree) {
                    for (const key of Object.keys((s.param_bindings ?? {}))) {
                        if (!boundBy.has(key))
                            boundBy.set(key, laneId);
                    }
                }
            }
            if (boundBy.size === 0)
                ...
                for (const s of subtree) {
                    ...
                    for (const text of texts) {
                        for (const m of text.matchAll(TOKEN_RE)) {
                            ...
                            if (binderLane === undefined || binderLane === laneId || seen.has(token))
                                continue;
                            seen.add(token);
                            push({
                                id: 'lane-cross-param',
                                step: String(s.name ?? '<unnamed>'),
                                message: `Step '${String(s.name)}' (lane '${laneId}' of fork step ` +
    ...
    {
        const allAcks = new Set([...wfAck]);
        for (const { step } of steps)
            for (const a of ackSet(step))
                allAcks.add(a);
        for (const a of allAcks) {
            if (!LINT_WARNING_IDS.includes(a)) {
                warnings.push({
                    id: 'unknown-ack',
                    message: `acknowledge_warnings contains '${a}', which is not a known warning id — ` +
