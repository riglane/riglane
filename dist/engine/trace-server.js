...
export function _setLocalServerStartForTests(fn) {
    startOverride = fn;
}
const MIME = {
...
function serveFile(rootDir, pathname, res, method = 'GET') {
...
function projectEntries(currentRoot) {
    let entries = [];
    try {
...
function scopeEntries(root) {
    ...
    if (!out.some((s) => s.id === 'generic'))
        out.unshift({ id: 'generic', hasIndex: false });
    return out;
}
const adhocMounts = new Map();
function mountRoot(slug) {
...
function handleOpenProject(root, req, res, url) {
    ...
    const chunks = [];
    req.on('data', (c) => { if (chunks.length < 64)
        chunks.push(c); });
    req.on('end', () => {
        try {
            ...
            if (!slug) {
                ...
                slug = stem;
                let n = 2;
                while (adhocMounts.has(slug) && adhocMounts.get(slug) !== mounted)
                    slug = `${stem}-${n++}`;
...
function handleWrite(root, req, res, url) {
    if (!requireServeToken(req, url, res))
        return;
    const reject = (code, msg) => { res.writeHead(code, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ ok: false, error: msg })); };
    const chunks = [];
    let size = 0;
    let aborted = false;
    req.on('data', (c) => {
    ...
    req.on('end', () => {
        ...
        if (!relPath || content === null)
            return reject(400, 'Body must be { path, content }');
        const ext = extname(relPath).toLowerCase();
        if (ext !== '.yaml' && ext !== '.yml')
            return reject(403, 'Only .yaml/.yml files may be written');
        ...
        if (target !== templatesRootDir && !target.startsWith(templatesRootDir + sep))
            return reject(403, 'Path escapes workflows/templates/');
        try {
    ...
    req.on('error', () => { if (!aborted)
        reject(400, 'Request error'); });
}
function handleValidate(root, req, res, url) {
    if (!requireServeToken(req, url, res))
        return;
    const reject = (code, msg) => { res.writeHead(code, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ ok: false, error: msg })); };
    const chunks = [];
    ...
    req.on('end', () => {
        ...
        const content = typeof body.content === 'string' ? body.content : null;
        if (content === null)
            return reject(400, 'Body must be { content, path? }');
        let workflow;
        try {
            workflow = parseYamlText(content);
        }
        catch (e) {
            res.writeHead(200, { 'Content-Type': 'application/json' });
            ...
            if (target === templatesRootDir || target.startsWith(templatesRootDir + sep)) {
                try {
                    if (statSync(target).isDirectory())
                        definitionDir = target;
                }
                catch { }
            }
        }
        try {
            const r = fullValidateWorkflow(workflow, definitionDir !== undefined ? { definitionDir } : {});
            res.writeHead(200, { 'Content-Type': 'application/json' });
...
function handleRequest(root, req, res) {
    try {
        ...
        if (pathname === '/')
            pathname = '/tools/trace-viewer.html';
        if (handleLocalApi(root, req, res, pathname, url))
            return;
        {
            const amount = /^\/p\/([^/]+)(\/api\/.+)$/.exec(pathname);
            if (amount) {
                const target = mountRoot(amount[1] ?? '');
                if (target && handleLocalApi(target, req, res, amount[2] ?? '', url))
                    return;
            }
        }
        if (req.method === 'POST') {
            if (pathname === '/api/write') {
                handleWrite(root, req, res, url);
                return;
            }
            if (pathname === '/api/validate') {
                handleValidate(root, req, res, url);
                return;
            }
            if (pathname === '/api/open-project') {
                handleOpenProject(root, req, res, url);
                return;
            }
            ...
            if (wmount) {
                ...
                if (wmount[2] === 'validate')
                    handleValidate(target, req, res, url);
                else
                    handleWrite(target, req, res, url);
                return;
            }
            res.writeHead(405);
...
export function ensureLocalServer(root) {
    if (startOverride)
        return startOverride(resolve(root));
    const absRoot = resolve(root);
    if (server && serverRoot === absRoot && serverBase) {
        return Promise.resolve(serverBase);
    }
    if (starting && starting.root === absRoot)
...
function serveDiscoveryPath(root) {
...
function writeServeDiscovery(root, base) {
    try {
        const prior = JSON.parse(readFileSync(serveDiscoveryPath(root), 'utf-8'));
        if (typeof prior.pid === 'number' && prior.pid !== process.pid && pidAlive(prior.pid))
            return;
    }
    catch {
    }
    try {
        ...
        writeFileSync(serveDiscoveryPath(root), `${JSON.stringify({
            base,
...
function removeServeDiscovery(root) {
    if (!root)
        return;
    try {
...
function traceOpenLog(agentDir, msg) {
    try {
...
function openBrowser(url, agentDir) {
    const candidates = browserCommands(url);
    const tryNext = (i) => {
        if (i >= candidates.length) {
            if (agentDir)
                traceOpenLog(agentDir, `openBrowser: all openers failed — url=${url}`);
        ...
        try {
            ...
            child.on('error', () => {
                if (agentDir)
                    traceOpenLog(agentDir, `openBrowser: '${cmd}' errored — trying next`);
                tryNext(i + 1);
            });
            child.on('spawn', () => {
                if (agentDir)
                    traceOpenLog(agentDir, `openBrowser: '${cmd}' spawned ok`);
            });
            child.unref();
        }
        catch {
...
export async function openToolViewer(toolsParent, relPath) {
    try {
...
export function openTraceViewer(agentDir, traceServerPath) {
    try {
        const can = canOpenBrowser();
        traceOpenLog(agentDir, `openTraceViewer: canOpenBrowser=${can} platform=${process.platform} isWSL=${isWSL()} ` +
...
export function currentLocalServerBase() {
    return serverBase;
}
export function refTraceServer() {
    try {
        server?.ref();
    }
    catch {
...
export function stopTraceServer() {
    if (server) {
        try {
            server.close();
        }
