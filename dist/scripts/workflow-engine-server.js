...
import { runWithCallContext, setHostBridge, } from '../engine/host-bridge.js';
import { recordEngineClient } from '../engine/host-context.js';
import { toIsoLocal } from '../engine/iso-time.js';
import { toolSpecCreateBatch, toolSpecMove, toolSpecLink, toolSpecSearch, toolSpecWrite, } from '../engine/spec-tools.js';
import { appendMcpCall, toolAgentNotesSearch, toolInbox, toolAgentNotesWrite, toolListAgentFiles, toolStepBegin, toolStepBeginDynamic, toolStepCollectResult, toolStepCollectResultDynamic, toolStepComplete, toolStepCompleteDynamic, toolWorkflowFinalize, toolWorkflowFinalizeDynamic, toolWorkflowInit, toolWorkflowInvokeDynamic, toolWorkflowReplanDynamic, toolWorkflowResolve, toolWorkflowResume, toolWorkflowValidate, toolWorkflowValidateDynamic, } from '../engine/workflow-engine.js';
...
export const TOOLS = [
    {
        ...
        description: 'Find a workflow definition and return its metadata: ' +
    ...
    {
        name: 'workflow_init',
        description: 'Create a new workflow run: generate manifest.json, trace file, ' +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                model_override: {
                    ...
                    description: 'Run-level model MODE override for every subagent step (the --model flag).',
                },
                inbox_webhook: {
                    type: 'string',
                    description: "Run-level inbox webhook override — the http(s) URL this run's question " +
                ...
                trace_viewer: {
                    type: 'string',
                    enum: ['off'],
                    description: "Run-level trace-viewer suppression (the launcher's --no-trace-viewer flag). " +
    ...
    },
    {
        ...
        description: 'Resume an existing workflow run. Returns current state, ' +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                run_id: {
                    type: 'string',
                    description: 'WHICH run to resume. Omit and the latest resumable run of this workflow is ' +
    ...
    {
        name: 'step_begin',
        description: 'Prepare a step for execution. Returns pre-composed text blocks ' +
    ...
    },
    {
        ...
        description: 'After a subagent completes, read gate-result.json and determine ' +
    ...
    {
        name: 'step_complete',
        description: 'Mark a step as completed. Writes summary to context/, applies ' +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                loop_decision: {
                    type: 'string',
                    enum: ['loop', 'proceed'],
                    description: 'Answer to a prior AWAITING_LOOP_DECISION response. Only ' +
                ...
                route_decision: {
                    type: 'string',
                    description: 'Answer to a prior AWAITING_ROUTE_DECISION response — a route id or ' +
                ...
                delegated_run_id: {
                    type: 'string',
                    description: "Delegation steps only: the child run's run_id (as returned by the " +
    ...
    },
    {
        ...
        description: 'Finalize a completed workflow. Sets manifest status, finalizes ' +
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                pattern: {
                    type: 'string',
                    description: "Filename pattern filter (e.g. '*.md', '*.json'). " +
    ...
    },
    {
        ...
        description: 'Validate a workflow YAML draft (raw text). Runs the full validation ' +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                workflow_name: {
                    type: 'string',
                    description: 'Optional. Workflow name — used to locate the on-disk directory for ' +
    ...
    },
    {
        ...
        description: 'Validate an orchestrator-drafted child workflow against the parent ' +
    ...
    },
    {
        ...
        description: 'Finalize a planning-step child workflow run. Bridges child terminal ' +
    ...
    },
    {
        ...
        description: 'Prepare a child substep for execution. Thin wrapper around step_begin ' +
    ...
    },
    {
        ...
        description: 'After a child substep subagent completes, read the gate-result.json ' +
    ...
    },
    {
        ...
        description: "Mark a child substep as completed. Writes summary to the child's " +
    ...
    },
    {
        ...
        description: 'Record a reflection note after a planning-step child workflow ' +
    ...
    },
    {
        ...
        description: 'Search reflection notes for a planning step template. Returns ' +
    ...
    {
        name: 'workflow_learn',
        description: 'Learn about workflow system capabilities. Returns knowledge content ' +
    ...
    },
    {
        ...
        description: 'Reset a planning step for iterative improvement. After a child workflow ' +
    ...
    {
        name: 'inbox',
        description: 'The run inbox — human gate / loop / route questions delivered OUTSIDE the ' +
        ...
        inputSchema: {
            ...
            properties: {
                op: {
                    ...
                    description: "The operation: rules (fetch composition rules; unlocks posting this pass) | " +
                ...
                message: {
                    type: 'object',
                    description: "ask + post. Message content: { title (short), body? (markdown), kind? " +
                ...
                message_id: {
                    type: 'string',
                    description: "The id op:'ask'/op:'post' returned (check + respond; on ask it RESUMES the " +
                ...
                type: {
                    ...
                    description: 'respond only. The response kind the user chose: "choice" picks one offered ' +
                        'entry (text = the entry), "respond" is free text, "items" answers a grouped ' +
                        'message.',
                },
                text: {
                    type: 'string',
                    description: "respond only. The user's words (the picked entry for choice; required for " +
                        'respond/edit unless args given).',
                },
                args: { type: 'object', description: 'respond only. Edited request args (edit responses).' },
                items: {
                    type: 'object',
                    description: 'respond only. Grouped answers keyed by item id: { <id>: { type, text?, args? }, ' +
    ...
    {
        name: 'spec_write',
        description: 'Author/commit/delete a behavioral spec. The engine owns identity (mints ' +
        ...
        inputSchema: {
            ...
            properties: {
                op: {
                    ...
                    description: 'create (new spec) · create_batch (bulk create N mutually-ref specs — see drafts) · ' +
                ...
                scope: {
                    type: 'string',
                    description: 'Target scope (the spec lives here). Agent DECIDES it here (generic vs ' +
                ...
                acknowledge_distinct: {
                    type: 'boolean',
                    description: 'Override a near-certain dedup HOLD: set true to create anyway when your spec is ' +
                ...
                acknowledge_single_rule: {
                    type: 'boolean',
                    description: 'Override a multi_rule HOLD (Rule Statement structurally flagged as an enumerated ' +
                ...
                set_domain_description: {
                    type: 'boolean',
                    description: "op:update only — overwrite the target domain's description in _index with the " +
                ...
                drafts: {
                    type: 'array',
                    description: 'For op:create_batch ONLY — bulk-author N mutually-referencing specs in one call. Each ' +
                ...
                acknowledge_distinct_temp_keys: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'For op:create_batch — temp_keys you assert are distinct despite a near-certain dedup ' +
                        'match (per-draft create-with-ack). Include them in the re-submitted batch payload.',
                },
                acknowledge_single_rule_temp_keys: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'For op:create_batch — temp_keys you assert are genuinely ONE rule despite the ' +
                        'multi_rule structural flag (per-draft override, recorded for audit). Include them ' +
                        'in the re-submitted batch payload.',
                },
                to_domain: {
                    type: 'string',
                    description: 'For op:move — destination domain the spec(s) move into. If new in the destination ' +
                ...
                spec_ids: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'For op:move — batch relocate several specs together (all-or-nothing). Use spec_id for a ' +
                        'single spec.',
                },
                domain: {
                    type: 'string',
                ...
                new_domain: {
                    type: 'string',
                    description: 'For op:rename_domain — the new name (must NOT already exist — that would be a merge).',
                },
            },
        },
    },
    {
        name: 'spec_search',
        description: 'Deterministic query over the spec _index (NOT embeddings — you are the ' +
    ...
    {
        name: 'spec_link',
        description: 'Maintain _registry.json (spec → implementing files). Call at IMPLEMENTATION ' +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                role: {
                    ...
                    description: 'Required for add. implements=produces behavior (verify ALL) · configures=sets ' +
...
function specWriteDispatch(args) {
    if (args.op === 'create_batch')
        return toolSpecCreateBatch(args);
    if (args.op === 'move' || args.op === 'rename_domain')
        return toolSpecMove(args);
    return toolSpecWrite(args);
}
export const TOOL_HANDLERS = new Map([
    ['workflow_resolve', toolWorkflowResolve],
    ['workflow_init', toolWorkflowInit],
    ['workflow_resume', toolWorkflowResume],
    ['step_begin', toolStepBegin],
    ['step_collect_result', toolStepCollectResult],
    ['step_complete', toolStepComplete],
    ['workflow_finalize', toolWorkflowFinalize],
    ['list_agent_files', toolListAgentFiles],
    ['workflow_validate', toolWorkflowValidate],
    ['workflow_validate_dynamic', toolWorkflowValidateDynamic],
    ['workflow_invoke_dynamic', toolWorkflowInvokeDynamic],
    ['workflow_finalize_dynamic', toolWorkflowFinalizeDynamic],
    ['step_begin_dynamic', toolStepBeginDynamic],
    ['step_collect_result_dynamic', toolStepCollectResultDynamic],
    ['step_complete_dynamic', toolStepCompleteDynamic],
    ['agent_notes_write', toolAgentNotesWrite],
    ['agent_notes_search', toolAgentNotesSearch],
    ['inbox', toolInbox],
    ['workflow_replan_dynamic', toolWorkflowReplanDynamic],
    ['workflow_learn', toolWorkflowLearn],
    ['spec_write', specWriteDispatch],
    ['spec_search', toolSpecSearch],
    ['spec_link', toolSpecLink],
]);
export function summarizeCall(toolName, args, result) {
    ...
    try {
        ...
        if (toolName === 'step_begin') {
            const stype = r.type ?? 'regular';
            const step = r.step_name ?? args.step ?? '?';
            if (stype === 'delegation') {
        ...
        if (toolName === 'step_collect_result') {
            ...
            const step = r.step ?? args.step ?? '?';
            const action = r.action ?? '?';
            if (passed === true)
                return `Gate passed for ${step} → ${action}`;
            if (passed === false)
                return `Gate failed for ${step} → ${action}`;
        ...
        if (toolName === 'step_complete') {
            ...
            if (r.workflow_done)
                return `Completed ${step} (workflow done)`;
        ...
        if (toolName === 'workflow_validate_dynamic') {
            ...
            const firstErr = typeof errs[0] === 'string' ? errs[0].slice(0, 120) : '';
            return firstErr
        ...
        if (toolName === 'workflow_finalize_dynamic') {
            if ('error' in r)
                return `Finalize-dynamic failed: ${r.error}`;
        ...
        if (toolName === 'step_begin_dynamic') {
            if ('error' in r)
                return `Begin-dynamic failed: ${r.error}`;
        ...
        if (toolName === 'workflow_replan_dynamic') {
            if ('error' in r)
                return `Replan failed: ${r.error}`;
            return `Reset planning phase for ${args.parent_step ?? '?'}, attempt ${r.attempt ?? '?'}, ${(r.previous_runs ?? []).length} prior run(s)`;
        }
        if (toolName === 'workflow_learn') {
            if ('error' in r)
                return `Learn failed: ${r.error}`;
            return `Topic: ${args.topic ?? 'overview'}`;
        }
        if (toolName === 'workflow_validate') {
            if ('error' in r)
                return `Validate failed: ${r.error}`;
            const errs = Array.isArray(r.errors) ? r.errors : [];
            const wfName = args.workflow_name ?? '(unnamed draft)';
            if (r.ok)
                return `Validated ${wfName} — ok`;
...
export function makeResponse(msgId, result) {
    return { jsonrpc: '2.0', id: msgId, result };
}
export function makeError(msgId, code, message) {
...
export function getNegotiatedProtocolVersion() {
...
export function makeStdioBridge(write) {
    return {
        sendRequest(method, params) {
            ...
            pendingOutgoing.set(id, settle);
            write(JSON.stringify({ jsonrpc: '2.0', id, method, params }));
            return {
                result,
                cancel() {
                    if (!pendingOutgoing.has(id))
                        return;
...
export async function handleMessage(msg, options = {}) {
    ...
    if (method === 'initialize') {
        ...
        const negotiated = requested !== null && SUPPORTED_PROTOCOL_VERSIONS.includes(requested)
            ? requested
            : LATEST_PROTOCOL_VERSION;
        negotiatedProtocolVersion = negotiated;
        const caps = typeof params.capabilities === 'object' &&
            params.capabilities !== null &&
            !Array.isArray(params.capabilities)
            ? params.capabilities
            : {};
        clientCapabilities = caps;
        recordEngineClient(params.clientInfo, process.cwd(), log, {
    ...
    if (method === 'tools/call') {
        ...
        try {
            ...
            const entry = {
                timestamp: toIsoLocal(callStart),
                tool: toolName,
...
function buildContextFields(args) {
    const out = {};
    const wfName = (typeof args.name === 'string' && args.name) ||
...
function maybeSummary(toolName, args, result) {
    if (typeof result !== 'object' || result === null)
        return {};
    const summary = summarizeCall(toolName, args, result);
    return summary ? { summary } : {};
}
export async function serverLoop(options = {}) {
    const write = (line) => {
...
export async function runWorkflowEngineCli(options = {}) {
