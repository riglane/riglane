...
function ensureHomeDirMigrated(newDir) {
    if (homeMigrationDone)
        return;
...
export function gateFileWaitMs() {
    return resolve(SETTING_DEFS.find((s) => s.key === 'engine.gate_file_wait_ms'));
}
export function inboxWebhookUrl() {
    return resolve(SETTING_DEFS.find((s) => s.key === 'engine.inbox_webhook_url'));
}
export function inboxAskMaxHoldMs() {
    return resolve(SETTING_DEFS.find((s) => s.key === 'engine.inbox_ask_max_hold_ms'));
}
export function autoOpenTraceViewer() {
    return resolve(SETTING_DEFS.find((s) => s.key === 'engine.auto_open_trace_viewer'));
}
export function skipApprovalsDefault() {
    return resolve(SETTING_DEFS.find((s) => s.key === 'run.skip_approvals'));
}
...
export function catalogBaseUrl() {
    const raw = resolve(SETTING_DEFS.find((s) => s.key === 'catalog.base_url'));
    return raw.replace(/\/+$/, '');
}
