...
import { ACTIVE_SCOPE_FILE, GENERIC_SCOPE, InvalidScopeIdError, SCOPE_CONFIG_PATH, ScopeError, clearUserActiveScope, ensureScopeDir, formatScopeTable, getAvailableScopes, loadScopeConfig, resolveActiveScope, saveScopeConfig, scopeDir, scopeExists, validateScopeId, writeUserActiveScope, } from './scope-context.js';
...
export function cmdSet(opts, args) {
    const r = resolveOpts(opts);
    ...
    if (!scopeExists(scopeId, r.root)) {
        const available = getAvailableScopes(r.root)
            .map((s) => s.id)
            .join(', ');
        r.stderr(`Error: scope '${scopeId}' is not declared. ` +
            `Available: ${available}.\n` +
...
export function cmdList(opts, args) {
    const r = resolveOpts(opts);
    const scopes = getAvailableScopes(r.root);
    if (args.counts) {
        for (const entry of scopes) {
            ...
            entry.label = `${entry.label}  (${count} specs)`;
        }
    }
    r.stdout('Available scopes:\n');
    r.stdout(`${formatScopeTable(scopes)}\n`);
...
export function cmdAdd(opts, args) {
    const r = resolveOpts(opts);
    const scopeId = args.scopeId ?? '';
    ...
    if (hint) {
        ...
        r.stdout('\n');
        r.stdout(`  TIP: record a coverage hint so agents know what '${scopeId}' governs\n` +
...
export function cmdHint(opts, args) {
    const r = resolveOpts(opts);
    const scopeId = args.scopeId ?? '';
    ...
    if (scopeId === GENERIC_SCOPE) {
        r.stderr(`Error: '${GENERIC_SCOPE}' is the implicit cross-integration scope — it has no ` +
            `config entry to carry a hint.\n`);
        return 1;
    }
    const config = loadScopeConfig(r.root);
    const entry = config.scopes.find((s) => s.id === scopeId);
    if (!entry) {
        r.stderr(`Error: scope '${scopeId}' is not declared in _scope-config.json. ` +
    ...
    saveScopeConfig(config, r.root);
    r.stdout(text ? `Coverage hint set for scope '${scopeId}'.\n` : `Coverage hint cleared for scope '${scopeId}'.\n`);
    return 0;
}
export function parseScopeCliArgs(argv) {
    const sub = argv[0];
    if (!sub)
        return { error: 'usage: scope_cli {show,set,unset,list,add,hint} ...' };
    ...
    if (sub === 'set') {
        ...
        if (!scopeId)
            return { error: 'scope_cli set: scope_id positional argument is required' };
    ...
    if (sub === 'hint') {
        ...
        const scopeId = parsed.positionals[0];
        if (!scopeId)
            return { error: 'scope_cli hint: scope_id positional argument is required' };
    ...
    if (!scopeId)
        return { error: 'scope_cli add: scope_id positional argument is required' };
    if (parsed.positionals.length > 2) {
        return {
...
function isScopeAction(s) {
    return (s === 'show' || s === 'set' || s === 'unset' || s === 'list' || s === 'add' || s === 'hint');
}
export async function runScopeCli(argv = process.argv.slice(2), opts = {}) {
    const r = resolveOpts(opts);
