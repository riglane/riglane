import { CC_RULE_FRONTMATTER, COPILOT_RULE_FRONTMATTER, CURSOR_RULE_FRONTMATTER, OPENCODE_RULE_FRONTMATTER, } from './spec-guidance.js';
...
export const SELECTABLE_ADAPTERS = [
    'claude',
    'cursor',
    'codex',
    'opencode',
...
export const DEFAULT_ADAPTERS = ['cursor', 'claude'];
export function adaptersToInstallOptions(adapters) {
    return { adapters };
}
export function mcpConfigProbe(d) {
    if (d.mcp.kind === 'codex-toml')
        return { kind: 'toml', path: `${d.projectDir}/config.toml` };
    if (d.mcp.kind === 'opencode-json')
        return { kind: 'opencode-json', path: d.mcp.dst };
