...
export function gitFacts(dir) {
    ...
    if (!top.ok)
        return { isRepo: false, headSha: '', clean: false, remoteUrl: '', headPushed: 'unknown' };
    ...
    if (remoteUrl !== '' && headSha !== '') {
        ...
        if (ls.ok) {
            ...
            headPushed = remoteShas.some((s) => s === headSha || git(dir, 'merge-base', '--is-ancestor', headSha, s).ok)
                ? 'yes'
                : 'no';
        }
    }
...
export function detectLicense(repoRoot) {
    for (const name of ['LICENSE', 'LICENSE.md', 'LICENSE.txt']) {
        const p = join(repoRoot, name);
        if (!existsSync(p))
            continue;
        const first = (readFileSync(p, 'utf-8').split('\n')[0] ?? '').trim().toLowerCase();
        if (first.includes('mit'))
            return 'MIT';
        if (first.includes('apache'))
            return 'Apache-2.0';
        if (first.includes('bsd'))
            return 'BSD';
        return 'see LICENSE';
    }
    return '';
}
export function runPreflight(workflowDir) {
    const rows = [];
    const dir = resolve(workflowDir);
    const id = basename(dir);
    const push = (label, status, detail, blocking = status === 'fail') => {
    ...
    if (workflow !== null) {
        ...
        if (name === id)
            push('one identity (name = directory)', 'ok', id);
        else
            push('one identity (name = directory)', 'fail', `workflow is named '${String(name)}', directory is '${id}'`);
        const v = fullValidateWorkflow(workflow, { definitionDir: dir });
        if (v.ok)
            push('engine validator', 'ok', `${v.warnings.length} advisory warning(s)`);
        else
            push('engine validator', 'fail', v.errors.slice(0, 3).join(' · '));
        const raw = readFileSync(yamlPath, 'utf-8');
        if (raw.includes('templates/my_workflows/')) {
            push('script paths are install-form', 'fail', "commands point at the authoring location (templates/my_workflows/) — publish the promoted copy, whose paths say templates/community/");
        }
        else {
    ...
    if (!g.isRepo)
        push('inside a git repository', 'fail', 'the pin (a commit SHA) must come from a git checkout');
    else {
        push('inside a git repository', 'ok', g.headSha.slice(0, 12));
        if (g.clean)
            push('workflow tree clean', 'ok', 'the tree you pack is the tree you pin');
        else
            push('workflow tree clean', 'fail', 'uncommitted changes inside the workflow directory — the lock would describe a tree no commit holds');
        if (g.remoteUrl === '')
            push('public remote (origin)', 'fail', 'no origin remote — the catalog fetches from a public URL');
        else if (/^https:\/\//.test(g.remoteUrl))
            push('public remote (origin)', 'ok', g.remoteUrl);
        else
            push('public remote (origin)', 'warn', `${g.remoteUrl} — the entry needs an https URL a stranger can fetch`, false);
        if (g.headPushed === 'yes')
            push('HEAD pushed to the remote', 'ok', 'the pinned SHA exists in the public history');
        else if (g.headPushed === 'no')
            push('HEAD pushed to the remote', 'fail', 'push first — a SHA the remote does not hold fails every fetch');
        else
            push('HEAD pushed to the remote', 'warn', 'could not verify (ls-remote failed)', false);
    }
    const repoRoot = g.isRepo ? git(dir, 'rev-parse', '--show-toplevel').out : dir;
    const licenseName = detectLicense(repoRoot);
    if (licenseName === '')
        push('LICENSE at the repo root', 'fail', 'the entry requires a license field, and the code must carry one');
    else
        push('LICENSE at the repo root', 'ok', licenseName);
    return { rows, blocked: rows.some((r) => r.status === 'fail' && r.blocking), id, git: g, licenseName };
}
function packTree(workflowDir) {
    const workflow = loadYaml(join(workflowDir, 'workflow.yaml'));
    const inv = buildWorkflowInventory(resolve(workflowDir), workflow);
    const lockText = composeLockDocument(inv);
    const shell = inv.script_tools.length + inv.deciders.length;
    return {
        lockText,
        summary: `${inv.steps.count} step(s), ${inv.script_tools.length} script tool(s), ` +
            `${inv.deciders.length} decider script(s), ${inv.bundled_files.length} bundled file(s)`,
        level: shell === 0 ? 'verified' : 'community',
    };
}
export function packLock(workflowDir) {
    const dir = resolve(workflowDir);
    const id = basename(dir);
    ...
    try {
        ...
        if (ar.status === 0 && ar.stdout !== null) {
            ...
            if (untar.status === 0)
                return { ...packTree(tree), fromCommit: true };
        }
...
export function repinEntryText(raw, pin) {
    ...
    if (!repinned)
        throw new Error('entry.yaml has no source: block to re-pin');
    return out.join('\n');
}
export function scaffoldEntryText(f) {
    return [
        `id: ${f.id}`,
        '',
        `summary: >-`,
...
export function writeEntryFiles(entryDir, entryText, lockText) {
...
export function validateCatalogCheckout(catalogRoot) {
    const script = join(catalogRoot, 'scripts', 'validate-entries.mjs');
    if (!existsSync(script))
        return { ran: false, ok: true, output: 'no scripts/validate-entries.mjs in the checkout' };
    const env = { ...process.env };
    if (env.RIGLANE_CLI === undefined) {
        const selfCli = resolve(dirname(fileURLToPath(import.meta.url)), '..', 'index.js');
        if (existsSync(selfCli))
            env.RIGLANE_CLI = selfCli;
    }
...
export function pathInsideRepo(workflowDir) {
