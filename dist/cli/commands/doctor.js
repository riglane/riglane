...
export async function runDoctor(target, opts = {}) {
    ...
    if (isFile(versionMarker)) {
        ...
        if (parsed && typeof parsed.templateHash === 'string') {
            if (parsed.templateHash === installedHash) {
                check('.riglane/.riglane-version templates match installed', true, `installedBy=${parsed.installedBy ?? '?'} hash=${parsed.templateHash.slice(0, 12)}`);
            }
            else {
                allOk =
                    check('.riglane/.riglane-version drift detected', false, `project hash=${parsed.templateHash.slice(0, 12)}, installed hash=${installedHash.slice(0, 12)} — run \`riglane update\``) && allOk;
            }
        }
        else {
            allOk =
                check('.riglane/.riglane-version legacy format (plain-string marker)', false, `marker=${raw.slice(0, 32) || '?'} — run \`riglane update\` to migrate to hash-based format`) && allOk;
        }
    }
    else {
        process.stdout.write('  [INFO] .riglane/.riglane-version absent (legacy or pre-Phase-55 install — run `riglane update` to add)\n');
    }
    const workflowsDir = join(absTarget, PRODUCT_DIR, 'workflows');
    allOk =
        check('.riglane/workflows/ present', isDir(workflowsDir), isDir(workflowsDir) ? '' : 'run `riglane init` to bootstrap') && allOk;
    const legacyOrphans = findLegacyRuntimeOrphans(workflowsDir);
    if (legacyOrphans.length > 0) {
        process.stdout.write(`  [INFO] ${legacyOrphans.length} legacy runtime dir(s) under .riglane/workflows/ ` +
    ...
    check('.riglane/specs/generic/ present', isDir(specsGeneric), isDir(specsGeneric) ? '' : 'run `riglane init` to bootstrap');
    ...
    if (isFile(codexProjectConfig)) {
        ...
        try {
            ...
            if (checks === null) {
                allOk =
                    check('global codex config.toml readable', false, `${join(codexHomeDir(), 'config.toml')} is malformed — hooks stay untrusted until fixed`) && allOk;
            }
                ...
                for (const { entry, status } of checks) {
                    ...
                    allOk =
                        check(`hook trusted: ${entry.command}`, ok, ok
                            ? ''
                            : status === 'untrusted'
                                ? 'no trust entry — Codex silently skips this hook; run --fix or `riglane update`'
                                : 'trust hash mismatch (command changed or formula drift) — run --fix or `riglane update`') && allOk;
                    if (!ok)
                        codexTrustFixNeeded = true;
                }
            }
        }
        catch (e) {
    ...
    if (isFile(ocConfigPath)) {
        ...
        if (!isFile(installedPluginPath)) {
            ...
            allOk =
                check('.opencode/plugins/riglane-hooks.ts present', false, 'missing — gate/ledger/file-guard hooks will NOT fire; run --fix or `riglane update`, then restart OpenCode') && allOk;
        }
            ...
            if (!same)
                ocPluginFixNeeded = true;
            allOk =
                check('.opencode/plugins/riglane-hooks.ts up to date', same, same ? '' : 'differs from the packaged shim — run --fix or `riglane update`, then restart OpenCode') && allOk;
        }
        process.stdout.write('\n');
    }
    ...
    const copilotInstalled = isFile(copilotHooksPath) || isFile(join(absTarget, '.github', 'mcp.json'));
    if (isFile(copilotHooksPath)) {
        ...
        if (isFile(copilotPackagedHooksPath)) {
            ...
            allOk =
                check('.github/hooks/riglane.json up to date', same, same
                    ? ''
                    : 'differs from the packaged hooks config — run --fix or `riglane update`, then restart the Copilot session') && allOk;
        }
        process.stdout.write('\n');
    }
        ...
        copilotHooksFixNeeded = true;
        allOk =
            check('.github/hooks/riglane.json present', false, 'missing — gate/ledger/file-guard hooks will NOT fire; run --fix or `riglane update`, then restart the Copilot session') && allOk;
        process.stdout.write('\n');
    }
    ...
    if (geminiInstalled) {
        ...
        allOk =
            check('.gemini/settings.json carries the riglane hooks', geminiHooksPresent, geminiHooksPresent
                ? ''
                : 'gate/veto/ledger hook commands missing from the merged settings — run `riglane update --gemini`, then restart the Gemini session') && allOk;
        const driftedAgents = checkGeminiAgentAcks(absTarget);
        if (driftedAgents.length > 0)
            geminiAgentAckFixNeeded = true;
        allOk =
            check('gemini agents acknowledged (headless consent)', driftedAgents.length === 0, driftedAgents.length === 0
                ? ''
                : `${driftedAgents.length} un-acknowledged/stale agent(s): ${driftedAgents.join(', ')} — ` +
                    `these are SILENTLY absent in headless runs; run --fix or \`riglane init-workflow <workflow>\``) && allOk;
        process.stdout.write('  [INFO] If Gemini silently ignores project hooks/MCP/commands, check folder trust ' +
    ...
    }
    {
        ...
        if (rec === null) {
            process.stdout.write('  [INFO] no handshake recorded yet — open this project in your harness once ' +
                '(any engine call records it), then re-run doctor\n');
        }
        else {
            const who = `${rec.host ?? rec.name}${rec.version ? ` ${rec.version}` : ''}`;
            if (elicit === true) {
                process.stdout.write(`  [OK  ] ${who} — native question dialog: yes (declares MCP elicitation)\n`);
            }
            else if (elicit === false) {
                process.stdout.write(`  [INFO] ${who} — native question dialog: no; a human-gate question is relayed by the agent in its reply instead\n`);
            }
            else {
                process.stdout.write(`  [INFO] ${who} — native question dialog: unknown (this record predates capability capture; reconnect the harness to refresh it)\n`);
            }
        }
        process.stdout.write('\n');
    }
    ...
    if (isDir(specsRoot)) {
        ...
        for (const sc of getAvailableScopes(absTarget).map((s) => s.id)) {
            try {
                ...
                allOk =
                    check(`scope '${sc}' _index.json matches .md files`, n === 0, n === 0
                        ? ''
                        : `${n} drift(s): ${changes.added.length} missing, ${changes.removed.length} stale, ${changes.modified.length} altered — run --fix`) && allOk;
                if (n > 0)
                    driftedScopes.push(sc);
            }
            catch (e) {
                allOk =
    ...
    if (isDir(specsRoot)) {
        ...
        for (const sc of getAvailableScopes(absTarget).map((s) => s.id)) {
            try {
                ...
                allOk =
                    check(`scope '${sc}' _registry.json mappings all point to live specs`, orphaned === 0, orphaned === 0
                        ? ''
                        : `${orphaned} orphaned mapping(s) (spec deleted): ${changes.removedMappings.join(', ')} — run --fix`) && allOk;
                if (orphaned > 0)
                    registryDriftedScopes.push(sc);
                if (changes.danglingFiles.length > 0) {
                    process.stdout.write(`  INFO  scope '${sc}': ${changes.danglingFiles.length} registry file ref(s) point to missing files (possible rename) — review via registry-sync; not auto-fixed:\n` +
    ...
    if (isDir(specsRoot)) {
        ...
        for (const sc of getAvailableScopes(absTarget).map((s) => s.id)) {
            try {
                ...
                allOk =
                    check(`scope '${sc}' related_specs all point to existing specs`, n === 0, n === 0
                        ? audited > 0
    ...
    if (opts.fix) {
        ...
        if (!isFile(versionMarker)) {
            ...
            if (markerHash !== installedHash) {
                writeRiglaneVersion(absTarget, false);
                process.stdout.write(`  FIXED: updated ${versionMarker} (hash → ${installedHash.slice(0, 12)})\n`);
                fixCount += 1;
            }
        }
        ...
        for (const sc of driftedScopes) {
            try {
                ...
                const n = changes.added.length + changes.removed.length + changes.modified.length;
                if (n > 0) {
                    process.stdout.write(`  FIXED: reconciled scope '${sc}' _index.json (${changes.added.length} added, ${changes.removed.length} removed, ${changes.modified.length} repaired)\n`);
                    fixCount += 1;
                }
            }
            catch (e) {
        ...
        for (const sc of registryDriftedScopes) {
            try {
                ...
                if (changes.removedMappings.length > 0) {
                    process.stdout.write(`  FIXED: pruned ${changes.removedMappings.length} orphaned mapping(s) from scope '${sc}' _registry.json (${changes.removedMappings.join(', ')})\n`);
                    fixCount += 1;
                }
            }
            catch (e) {
        ...
        if (codexTrustFixNeeded) {
            try {
                ...
                if (n > 0) {
                    process.stdout.write(`  FIXED: persisted codex hook trust (${report.added.length} added, ${report.updated.length} re-trusted)\n`);
                    fixCount += 1;
                }
            }
            catch (e) {
        ...
        if (ocPluginFixNeeded) {
            try {
                ...
                if (isFile(srcPlugin)) {
                    ...
                    process.stdout.write('  FIXED: refreshed .opencode/plugins/riglane-hooks.ts (restart OpenCode to load it)\n');
                    fixCount += 1;
                }
            }
            catch (e) {
        ...
        if (copilotHooksFixNeeded) {
            try {
                ...
                if (isFile(srcHooks)) {
                    ...
                    process.stdout.write('  FIXED: refreshed .github/hooks/riglane.json (restart the Copilot session to load it)\n');
                    fixCount += 1;
                }
            }
            catch (e) {
        ...
        if (geminiAgentAckFixNeeded) {
            try {
                ...
                if (acked.length > 0) {
                    process.stdout.write(`  FIXED: acknowledged ${acked.length} gemini agent(s) (headless consent)\n`);
                    fixCount += 1;
                }
            }
            catch (e) {
        ...
        if (fixCount === 0) {
            process.stdout.write('  No auto-repairable items found. Run `riglane update` for full refresh.\n');
        }
        else {
            process.stdout.write(`  ${fixCount} item(s) repaired. For more (skill files, hooks, MCP config), run \`riglane update\`.\n`);
        }
    }
    if (allOk) {
    ...
    if (!opts.fix)
        process.stderr.write('Run `riglane doctor --fix` to auto-repair fixable issues (spec-index, version marker, scope dirs, orphans).\n');
    return 1;
}
function isDir(p) {
    try {
...
function findLegacyRuntimeOrphans(workflowsDir) {
    let entries;
    try {
    ...
    for (const e of entries) {
        if (!e.isDirectory())
            continue;
        if (e.name === 'templates' || e.name === 'dynamic')
            continue;
