...
import { ADAPTERS, SELECTABLE_ADAPTERS, DEFAULT_ADAPTERS, } from '../../adapters/index.js';
...
import { findByPath as findRegistryEntry, isTemporaryLocation, load as loadRegistry, register as registerInRegistry, } from '../../registry/registry.js';
...
import { checkDependencies, ensureCursorIgnore, injectManagedBlock, mergeClaudeSettings, mergeGeminiSettings, } from '../_merge.js';
import { ackGeminiAgents } from '../gemini-agent-ack.js';
import { mergeCodexConfig, mergeOpencodeConfig, mergeCursorFileGuardHook, mergeCursorLoggerHooks, mergeCursorSpawnThrottleHook, mergeHooks, normalizeCursorHookShell, mergeMcpConfig, removeManagedBlock, } from '../_merge.js';
...
export function printHeader(target, adapters, mode, dryRun) {
...
export function removeLegacySkills(target, adapterDir, dryRun) {
    ...
    if (found.length === 0)
        return;
    if (dryRun) {
        process.stdout.write(`[cleanup] would remove ${found.length} legacy skill(s) from ${adapterDir}/skills/: ${found.join(', ')}\n\n`);
        return;
    }
    for (const legacy of found) {
        rmSync(join(skillsDir, legacy), { recursive: true, force: true });
    }
    process.stdout.write(`[cleanup] removed ${found.length} legacy skill(s) from ${adapterDir}/skills/ (Phase 6 /riglane- rename): ${found.join(', ')}\n\n`);
}
...
export function removeLegacyWorkflows(target, dryRun) {
    ...
    if (existsSync(predefinedDir)) {
        for (const legacy of LEGACY_WORKFLOW_NAMES) {
            ...
            if (existsSync(dir))
                found.push({ dir, legacy, family: 'predefined' });
        }
    }
    if (existsSync(examplesDir)) {
        for (const legacy of LEGACY_EXAMPLE_NAMES) {
            ...
            if (existsSync(dir))
                found.push({ dir, legacy, family: 'examples' });
        }
    }
    if (found.length === 0)
        return;
    ...
    if (dryRun) {
        process.stdout.write(`[cleanup] would remove ${found.length} legacy workflow(s) from .riglane/workflows/templates/: ${names}\n\n`);
        return;
    }
    for (const f of found) {
        rmSync(f.dir, { recursive: true, force: true });
    }
    process.stdout.write(`[cleanup] removed ${found.length} stale workflow(s) from .riglane/workflows/templates/ (renamed or relocated): ${names}\n\n`);
}
function removeLegacyAgentDocs(target, opts) {
...
const LEGACY_BRAND_DIRS = [
    '.opencode/acp',
    '.gemini/commands/acp',
];
const LEGACY_BRAND_AGENT_DIRS = [
    { dir: '.opencode/agents', ext: '.md' },
    { dir: '.github/agents', ext: '.agent.md' },
    { dir: '.gemini/agents', ext: '.md' },
];
export function removeLegacyBrandArtifacts(target, dryRun) {
    const removed = [];
    const drop = (abs, rel, recursive) => {
        if (!existsSync(abs))
            return;
        if (!dryRun)
    ...
    for (const rel of LEGACY_BRAND_FILES)
        drop(join(target, rel), rel, false);
    for (const rel of LEGACY_BRAND_DIRS)
        drop(join(target, rel), `${rel}/`, true);
    for (const { dir, ext } of LEGACY_BRAND_AGENT_DIRS) {
        const abs = join(target, dir);
        if (!existsSync(abs))
            continue;
        for (const name of readdirSync(abs)) {
            if (!name.startsWith('acp-') || !name.endsWith(ext))
                continue;
            const p = join(abs, name);
            try {
                if (!statSync(p).isFile())
                    continue;
                if (!readFileSync(p, 'utf-8').includes(GENERATED_MARKER))
                    continue;
            }
            catch {
                continue;
    ...
    process.stdout.write(`[cleanup] ${verb} ${removed.length} acp-era artifact(s):\n` +
        removed.map((r) => `          ${r}\n`).join(''));
    process.stdout.write('\n');
}
function removeLegacyTools(target, opts) {
    ...
    process.stdout.write(`[cleanup] removed legacy ${dstRel} (superseded by projects-spec-audit.html — Projects Spec & Audit)\n\n`);
}
export function installUniversal(target, srcRoot, opts) {
...
export function installSpecs(target, srcRoot, opts) {
...
export function ensureWorkflowsDir(target) {
...
function writeSpecGuidanceRuleFile(absPath, content, dryRun) {
...
function removeSpecGuidanceForAdapter(target, sg, dryRun) {
...
export async function installAdapter(adapter, target, srcRoot, opts) {
    ...
    if (adapter === 'claude' || adapter === 'cursor') {
        removeLegacySkills(target, adapterDir, opts.dryRun);
    }
    if (d.skills) {
    ...
    switch (d.hooks.kind) {
        ...
        case 'gemini-settings-json': {
            process.stdout.write(`[2/5] Settings (${d.projectDir}/settings.json — hooks + MCP servers + context)\n`);
            ...
            if (isFile(srcGemini)) {
                ...
                process.stdout.write(`  WARN  Source ${d.srcDir}/settings.json.template not found — skipping\n`);
            }
            break;
        }
    }
    ...
    switch (d.mcp.kind) {
        ...
        case 'codex-toml':
            process.stdout.write(`[3/5] MCP server config (${d.projectDir}/config.toml — merged with hooks in step 2)\n`);
            break;
        case 'gemini-settings-json':
            process.stdout.write(`[3/5] MCP server config (${d.projectDir}/settings.json — merged with hooks in step 2)\n`);
            break;
        case 'opencode-json': {
...
export async function defaultRunUpdateWorkflows(target, dryRun) {
    if (dryRun)
        return;
...
export function defaultBootstrapScopeGeneric(target, dryRun) {
    if (dryRun)
        return;
...
export function printSummary(opts) {
    ...
    if (opts.update || opts.dryRun) {
        ...
        if (counts.new)
            process.stdout.write(`  New files:     ${counts.new}\n`);
        if (counts.updated)
            process.stdout.write(`  Updated:       ${counts.updated}\n`);
        if (counts.pruned)
            process.stdout.write(`  Pruned:        ${counts.pruned}\n`);
        if (counts.unchanged)
            process.stdout.write(`  Unchanged:     ${counts.unchanged}\n`);
...
function lookupRegistryAdapters(absTarget) {
    try {
...
function lookupRegistrySpecGuidance(absTarget) {
    try {
...
export async function runInit(target, opts = {}) {
    ...
    const explicit = opts.adapters && opts.adapters.length > 0
    ...
    const singleFlagUnion = selectedAdapter
        ? SELECTABLE_ADAPTERS.filter((a) => a === selectedAdapter || (recordedAdapters ?? []).includes(a))
        : null;
    const adapters = explicit ?? singleFlagUnion ?? recordedAdapters ?? DEFAULT_ADAPTERS;
    const specGuidanceEnabled = opts.specGuidance ?? (lookupRegistrySpecGuidance(absTarget) ?? true);
    checkDependencies();
    resetCounts();
    ...
    if (!dryRun)
        writePredefinedHashes(absTarget);
...
export function writeRiglaneVersion(target, dryRun) {
    if (dryRun)
        return;
...
function walkRelativeSkipSymlinkDirs(root) {
    const out = [];
    function recurse(currentDir, prefix) {
        let entries;
        try {
