...
import { findByPath as findRegistryByPath, load as loadRegistry, register as registerInRegistry, } from '../../registry/registry.js';
import { ADAPTERS, SELECTABLE_ADAPTERS, mcpConfigProbe } from '../../adapters/index.js';
import { readOrCreateProjectId } from '../projectId.js';
const ENGINE_SCRIPT_FILES = new Set([
    'workflow-engine.py',
    'output_validator.py',
    'gate-check.py',
    'schema-validate.py',
...
export function isCopyBased(target) {
    const dirs = [...LEGACY_DIRS, ACP_DIR];
    return dirs.some((d) => isFile(join(target, d, 'mcp', 'workflow-engine.py')) ||
        isFile(join(target, d, 'scripts', 'gate-check.py')));
}
export function firstLegacyDir(target) {
    for (const d of LEGACY_DIRS) {
        if (isDir(join(target, d)))
            return d;
    }
    return null;
}
export function preflight(target, opts) {
    if (!isDir(target)) {
    ...
    if (!hasLegacy && !hasNew) {
        process.stderr.write(`NOTICE: ${target} has none of ${[...LEGACY_DIRS, ACP_DIR].map((d) => `${d}/`).join(', ')} — Riglane is not installed here.\n`);
    ...
    if (isDir(join(target, '.git'))) {
        try {
            ...
            if (result.status === 0 && stdout.trim()) {
                process.stdout.write('  WARN  git working tree in target has uncommitted changes — ' +
...
export function makeBackup(target, opts) {
    ...
    if (result.error || (result.status !== null && result.status !== 0)) {
        ...
        throw new Error(`ERROR: tar invocation failed (${errMsg}). Aborting to avoid data loss without backup. Install \`tar\` (apt/brew/chocolatey) or perform manual backup before re-running migrate.`);
    }
    let sizeMb;
    try {
        ...
        if (e instanceof Error && 'code' in e && e.code === 'ENOENT') {
            throw new Error(`ERROR: backup tar reported success but ${backupPath} does not exist. Aborting migrate to avoid data loss without verifiable backup.`);
        }
        throw e;
    }
...
export function deleteEngineFiles(target, opts) {
    const removed = [];
...
export function refreshPredefinedWorkflows(target, opts) {
    ...
    if (!opts.dryRun)
        writePredefinedHashes(target);
}
export function rewriteMcpJson(path, opts) {
    if (!isFile(path))
        return;
    let data;
    try {
...
export function rewriteClaudeSettings(target, opts) {
    ...
    for (const group of groups) {
        ...
        const hooks = groupObj.hooks ?? [];
        for (const hook of hooks) {
            const hookObj = hook;
            const cmd = hookObj.command ?? '';
            if (cmd.includes('scripts/gate-check.py') ||
                cmd.includes('.agent\\scripts\\gate-check.py')) {
                if (cmd !== 'riglane gate-check') {
                    if (opts.dryRun) {
                        process.stdout.write('  WOULD REWRITE  .claude/settings.json SubagentStop -> riglane gate-check\n');
                        return;
                    }
...
export function rewriteCursorHooks(target, opts) {
    ...
    for (const hook of hooks) {
        ...
        if (cmd.includes('scripts/gate-check.py') ||
            ...
            if (cmd !== 'riglane gate-check') {
                if (opts.dryRun) {
                    process.stdout.write('  WOULD REWRITE  .cursor/hooks.json subagentStop -> riglane gate-check\n');
                    return;
                }
                hookObj.command = 'riglane gate-check';
                changed = true;
...
export function appendGitignoreBackupPattern(target, opts) {
...
function rewriteAgentRefs(target, acpDir, opts) {
    if (opts.dryRun)
        return;
    const fromTok = `${LEGACY_ACP_DIR}/`;
    const toTok = `${ACP_DIR}/`;
    const tokenPairs = LEGACY_DIRS.flatMap((legacy) => [
        [`${legacy}/`, `${ACP_DIR}/`],
        [`${legacy}\\`, `${ACP_DIR}\\`],
    ]);
    const hasAnyToken = (text) => tokenPairs.some(([from]) => text.includes(from));
    const rewriteTokens = (text) => tokenPairs.reduce((acc, [from, to]) => acc.split(from).join(to), text);
...
export function renameRiglaneDir(target, opts) {
    const legacyName = firstLegacyDir(target);
    const next = join(target, ACP_DIR);
    if (legacyName === null || isDir(next))
        return false;
...
function renameMarkerFiles(acpDir) {
    const pairs = [
    ...
    for (const [oldName, newName] of pairs) {
        ...
        if (!isFile(oldPath) || existsSync(newPath))
            continue;
        try {
...
export async function runMigrate(target, opts = {}) {
    ...
    if (!needsRename && !copyBased) {
        process.stdout.write(`Nothing to migrate: ${ACP_DIR}/ is already in place and no copy-based engine files were found.\n`);
        process.stdout.write('(Use `riglane update` to refresh templates/skills and `riglane doctor` to verify.)\n');
        return 0;
    }
    ...
    if (dryRun) {
        ...
        try {
            ...
            if (initRc !== 0) {
                process.stdout.write('  WARN  the update pass returned non-zero — run `riglane update` manually and check output.\n');
            }
        }
        catch (e) {
            const msg = e instanceof Error ? e.message : String(e);
    ...
    if (!dryRun) {
        ...
        try {
            ...
            if (entry && entry.adapters.length > 0)
    ...
    if (!dryRun) {
        ...
        process.stdout.write(`           ${ACP_DIR}/workflows/<id>/ runtime data, my_workflows/, ${ACP_DIR}/local/\n`);
        process.stdout.write('\n');
