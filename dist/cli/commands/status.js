...
export function computeProjectStatus(target) {
    const abs = resolve(target);
    const installed = isDir(join(abs, PRODUCT_DIR, 'workflows'));
    const adapters = Object.entries(ADAPTERS).map(([id, d]) => {
...
export async function runStatus(args) {
    ...
    if (json) {
        ...
        process.stdout.write(`installed=${st.installed} mcp=${st.mcp_configured} templates=${st.templates} ` +
