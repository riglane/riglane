...
import { defaultPaths, toolWorkflowFinalize } from '../../engine/workflow-engine.js';
...
export function clearWorkflowRun(name, target, options = {}) {
    ...
    if (options.runId !== undefined && options.runId !== '') {
        ...
        if (!inProgress.includes(options.runId)) {
            let status = '';
            try {
                const m = JSON.parse(readFileSync(runManifestPath(paths.agentDir, options.runId), 'utf-8'));
        ...
        if (inProgress.length === 0) {
            let status = '';
            try {
                const m = JSON.parse(readFileSync(runManifestPath(paths.agentDir, allRuns[allRuns.length - 1]), 'utf-8'));
                status = typeof m.status === 'string' ? m.status : '';
            }
            catch {
                status = '';
        ...
        runId = inProgress[inProgress.length - 1];
    }
    ...
    const finalize = options.finalize ??
        ((n, p) => {
            const r = toolWorkflowFinalize({ name: n }, p);
...
export async function runWorkflowClear(args, options = {}) {
    ...
    if (!name) {
        process.stderr.write('Usage: riglane workflow-clear <workflow-name> [target] [--run <run-id>] [--json]\n');
        return 2;
    }
    ...
    if (result.reason === 'no-run') {
        ...
        process.stderr.write(`Refused: ${result.detail}. Nothing was cleared — a run id that does not resolve is ` +
        ...
        process.stderr.write(`Refused: run ${result.run_id} of '${name}' is owned by a LIVE orchestrator session ` +
        ...
        process.stdout.write(`Cleared '${name}': finalized run ${result.run_id} as ${result.status} (lock released).\n`);
    }
    return 0;
}
