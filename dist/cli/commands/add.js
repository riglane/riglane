...
export async function runAddCli(argv, deps = {}) {
    ...
    if (existsSync(installDir)) {
        process.stderr.write(`add: '${entry.id}' is already installed at ${installDir}.\n` +
    ...
    try {
        ...
        if (wfName !== entry.id) {
            process.stderr.write(`add: entry id '${entry.id}' but the workflow is named '${String(wfName)}' — these must be ` +
        ...
        const confirmed = await confirmByTypingId(entry.id, confirmationPrompt(entry.id, 'install it'), deps.prompt);
        if (!confirmed.ok) {
        ...
        process.stdout.write(`Installed '${entry.id}' to ${installDir} — SWITCHED OFF.\n` +
            `Enable it deliberately: riglane trust ${entry.id}\n`);
        if (inv.script_tools.length > 0) {
            process.stdout.write(`Note: it declares script tools — after trusting, run 'riglane init-workflow ${entry.id}' ` +
...
function readLocalEntry(dir) {
    const entry = readEntryFile(join(dir, 'entry.yaml'));
    const lockPath = join(dir, 'entry.lock.yaml');
    if (!existsSync(lockPath)) {
        throw new EntryError(`${dir} has entry.yaml but no entry.lock.yaml — generate it with 'riglane catalog pack' first; ` +
...
async function fetchCatalogEntry(id, deps) {
    ...
    if (revokedRaw !== null) {
        ...
        if (verdict.revoked) {
            process.stderr.write(`add: '${id}' has been REVOKED by the catalog${verdict.reason ? ` — ${verdict.reason}` : ''}.\n` +
                `Nothing was installed.\n`);
            return null;
        }
    }
    ...
    if (raw === null) {
        process.stderr.write(`add: no catalog entry '${id}' at ${base} — check the id (riglane search), ` +
...
export function printInspectScreen(entry, inv, workflow, installDir) {
    ...
    if (inv.script_tools.length === 0 && inv.deciders.length === 0) {
        ...
        for (const t of inv.script_tools)
            w(`    [tools.${t.name}]  ${t.command}`);
        for (const d of inv.deciders)
            w(`    [${d.field} @ ${d.at}]  ${d.command}`);
    }
...
function printStepGoals(steps, w, indent) {
    for (const s of steps) {
        ...
        if (routes?.define) {
            for (const r of routes.define)
                printStepGoals(r.steps ?? [], w, `${indent}  `);
        }
        const lanes = s.lanes;
        if (lanes?.define) {
            for (const l of lanes.define)
                printStepGoals(l.steps ?? [], w, `${indent}  `);
        }
    }
}
function serializeEntryProvenance(entry) {
    const lines = [
