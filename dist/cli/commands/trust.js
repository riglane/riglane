...
export async function runTrustCli(argv, deps = {}) {
    ...
    if (!existsSync(yamlPath)) {
        process.stderr.write(`trust: no community workflow '${id}' — expected ${yamlPath}\n` +
    ...
    const confirmed = await confirmByTypingId(id, confirmationPrompt(id, 'trust it'), deps.prompt);
    if (!confirmed.ok) {
    ...
    process.stdout.write(`Trusted '${id}' at ${entry.trusted_at} (content digest ${digest.slice(0, 12)}…).\n` +
        `Any change to the installed files switches it off again until re-trusted.\n`);
    return 0;
}
function printSurface(id, inv) {
    ...
    if (inv.capabilities.flags.length > 0) {
        ...
        for (const f of inv.capabilities.flags)
