...
export function reportFor(entry) {
    const p = probe(entry);
    let runs = [];
    if (p.pathExists) {
        try {
...
export function selectForForget(reports, opts) {
    return reports.filter((r) => (opts.temp && r.temporary) || (opts.gone && !r.path_exists));
}
export function forgetProjects(selected, opts) {
    const out = [];
...
function printTable(reports) {
    ...
    for (const r of reports) {
        process.stdout.write(`${w(driftLabel(r.drift), 14)}${w(r.temporary ? 'temp' : '', 6)}${w(r.in_progress_runs.length ? String(r.in_progress_runs.length) : '', 5)}${r.path}\n`);
    }
    const temp = reports.filter((r) => r.temporary).length;
    const gone = reports.filter((r) => !r.path_exists).length;
    const notes = [];
    if (temp)
        notes.push(`${temp} temporary (under the OS temp directory)`);
    if (gone)
        notes.push(`${gone} gone (directory no longer exists)`);
    if (notes.length) {
...
export async function runProjects(args) {
    ...
    if (sub === 'forget') {
        ...
        const n = (a) => outcomes.filter((o) => o.action === a).length;
        const parts = [];
        if (n('forgotten'))
            parts.push(`${n('forgotten')} ${prefix}forgotten (files left intact)`);
        if (n('deleted'))
            parts.push(`${n('deleted')} ${prefix}deleted`);
        if (n('kept'))
            parts.push(`${n('kept')} kept`);
