...
description: "Run a declarative multi-phase workflow (doc-spec-extraction, spec-add, spec-audit, spec-do, implement, or custom from my_workflows/). Orchestrates subagents via MCP workflow engine with structural gates, trace, and parallel execution."
---
...
**Step 0 — Find your MCP wrapper FIRST. Do this before reading anything else in this section.**

Search your direct tools list for these EXACT strings (do not paraphrase, do not generalise):

- `mcp__<server>__*` — Claude Code (double underscore, e.g. `mcp__workflow_engine__workflow_resolve`)
- `mcp_<server>_*` — Cursor direct tools (single underscore, e.g. `mcp_workflow_engine_workflow_resolve`)
- `CallMcpTool` — Cursor wrapper

Outcome:

- Found `mcp__*` entry → MCP via Claude Code direct tools. Proceed to Step 1.
- Found `mcp_<server>_*` entry → MCP via Cursor direct tools. Proceed to Step 1.
- Found `CallMcpTool` → MCP via Cursor wrapper. Proceed to Step 1.
- Found none of the three → MCP servers may still be starting (they load asynchronously when the IDE opens). **Wait ~20 seconds, then re-check your tools list.** If still not found, apply the **Fallback** below (it recognizes any naming form by core tool name) BEFORE concluding MCP is unavailable. Only if the Fallback also finds nothing → **STOP and tell the user** the MCP servers are configured but not exposed to this session — they should reconnect MCP (Cursor Settings → MCP, or Reload Window), wait ~20s, then re-run. **When MCP is genuinely absent, do NOT flail:** NEVER self-call `riglane run-workflow`, `riglane mcp-server`, `riglane mcp-tools`, or `riglane doctor`, do NOT grep the Riglane source repo, do NOT create your own MCP bridge or run scripts via shell — none of these expose MCP tools in THIS session (`riglane run-workflow` is an EXTERNAL launcher that spawns a NEW agent; you are already inside a launched one). Your only correct action is to STOP and report.

**Do NOT search your tools list for the bare unprefixed name `workflow_resolve`, `step_begin`, etc.** — bare engine names are NEVER standalone tools; they always appear with a host prefix (`mcp__<server>__<tool>` on Claude Code, `mcp_<server>_<tool>` on Cursor). Not finding the bare name is the **expected** state, NOT evidence that MCP is unavailable.

**Step 1 — Engine tools (reached THROUGH the wrapper).**

The engine provides 6 tools: `workflow_resolve`, `workflow_init`, `step_begin`, `step_collect_result`, `step_complete`, `workflow_finalize`. They MUST drive the entire run. Call them via the wrapper you found in Step 0:

| Environment | Wrapper (in your tools list) | How to call engine tools |
|---|---|---|
| Claude Code | `mcp__<server>__<tool>` family | Call directly, e.g. `mcp__workflow_engine__workflow_resolve({...})` |
| Cursor (direct) | `mcp_<server>_<tool>` | Call directly, e.g. `mcp_workflow_engine_workflow_resolve({...})` |
| Cursor (wrapper) | `CallMcpTool` | `CallMcpTool(server="<server-name>", toolName="workflow_resolve", arguments={...})` |

**Claude Code concrete example:**
```
mcp__workflow_engine__workflow_resolve({"name": "<workflow-name>"})
```
The server name `workflow_engine` is configured in `.mcp.json` / `.claude/settings.json`. List your available tools — entries beginning with `mcp__workflow_engine__` are the engine tools you can call directly.

**Step 1a (Claude Code) — load deferred schemas FIRST, before calling.** In tool-heavy sessions Claude Code lists many tools by NAME ONLY (deferred — the parameter schema is not loaded). A deferred tool is *found* but not yet *callable*: invoking it directly fails with `InputValidationError` because its schema is absent. So the moment you've confirmed `mcp__workflow_engine__*` exist, **proactively load their schemas in one shot** before your first call:

```
ToolSearch("select:mcp__workflow_engine__workflow_resolve,mcp__workflow_engine__workflow_init,mcp__workflow_engine__step_begin,mcp__workflow_engine__step_collect_result,mcp__workflow_engine__step_complete,mcp__workflow_engine__workflow_finalize")
```

(If a tool is already fully loaded, selecting it is a harmless no-op.) Loading a schema by its **full prefixed name** is NOT the forbidden bare-name scan — it is how a found-but-deferred tool becomes callable. An `InputValidationError` / "schema not loaded" is a **deferred-tool signal, NOT an engine failure**: load the schema and proceed — do **NOT** STOP for it. STOP is reserved for the engine errors in Step 2. (This applies to the dynamic-planning / agent_notes engine tools too — load each on demand the same way.)

**Script tools are deferred the same way.** When a `step_begin` `tool_docs` block names a script tool by its full `mcp__workflow_tools__<wf>__<tool>` form and YOU are the one who will call it (an **inline** step — a per-step subagent already has it whitelisted), `ToolSearch("select:mcp__workflow_tools__<wf>__<tool>")` to load its schema before calling, and treat a not-found / `InputValidationError` as the **same deferred signal** — re-select and call it. Do **NOT** conclude the script tool is missing, and do **NOT** fall back to running the script via Bash, just because it was not in your list on first look. Always `select` by the EXACT full name `tool_docs` gives you — never a loose keyword search (a loose query returns 0 even when the tool is registered).

**Step 2 — Verify via call response, not tool-list scan.**

Decide based on the **wrapper's response**, NOT on whether the unprefixed name `workflow_resolve` appears as a direct tool name:
...
**Anti-pattern #1 (the #1 way agents fail this protocol):** scanning your direct tools list for the unprefixed name `workflow_resolve`, not finding it, and concluding MCP is unavailable. The prefixed form `mcp__workflow_engine__workflow_resolve` IS in your tools list — re-do Step 0.

**Anti-pattern #2:** reading "MCP tools are not in your direct tools list" and paraphrasing it as "MCP is unavailable to me". The statement is true for unprefixed engine names only; the prefixed WRAPPER form (which IS in your list) is what makes them callable.
...
**Only if `workflow_resolve` actually returns an error from the wrapper** → **STOP and surface the error to the user verbatim.** The MCP engine is the only supported way to run a workflow.
...
- **NEVER bypass `step_begin` by inventing or guessing per-step subagent types.** The engine returns `subagent_type` in step_begin output — pass it through verbatim, do not substitute.
...
`.riglane/scripts/`, `.riglane/docs/`, `.riglane/tools/`, `.riglane/workflows/templates/predefined/`, `.cursor/skills/`, `.cursor/rules/`, `.cursor/hooks.json`
