...
# /riglane-create-workflow — the workflow-design assistant
...
`workflow_learn` / `workflow_validate` are MCP tools on the `workflow_engine` server — in Claude Code they appear in your tools list as `mcp__workflow_engine__workflow_learn` / `mcp__workflow_engine__workflow_validate` (find them via ToolSearch if deferred). They are NOT shell commands and NOT skills: never call them via Bash/shell, and never build JSON-RPC bridges to the server.
...
- **Follow-ups the user must run** after the workflow is written:
  1. `riglane init-workflow <workflow>` — regenerates the per-step subagent files with the new tool whitelisted, on every agent-file host installed in the project (Cursor uses a global inventory and Codex a generic agent — neither has per-step files to regenerate)
