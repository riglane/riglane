...
export function bumpCount(key: keyof Counts, n = 1): void {
  _counts[key] += n;
}
...
export function copyTree(src: string, dst: string, opts: InstallOptions = {}): string[] {
  ...
  let entries: string[];
  try {
    entries = readdirSync(src, { recursive: true }) as unknown as string[];
  } catch {
    entries = [];
  }
  ...
  for (const entry of entries) {
    ...
    if (segments.includes('__pycache__')) continue;
    ...
    if (isFile) fileEntries.push(entry);
  }
...
export function pruneTree(src: string, dst: string, opts: PruneOptions = {}): string[] {
  const dryRun = Boolean(opts.dryRun);
  ...
  let entries: string[];
  try {
    entries = readdirSync(dst, { recursive: true }) as unknown as string[];
  } catch {
    return pruned;
  }
  ...
  for (const entry of sorted) {
    ...
    if (basename(entry) === '__pycache__') continue;
