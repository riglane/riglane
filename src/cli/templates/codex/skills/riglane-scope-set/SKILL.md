...
# $riglane-scope-set (Codex)

Change the active scope used by spec-touching workflows and skills.

> **Codex invocation:** explicitly with `$riglane-scope-set <scope-id> [--project]`, or implicitly when the user's request matches. (Codex skills use `$name`, not the `/name` form used by other hosts.)
