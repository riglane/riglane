...
# $riglane-create-workflow — the workflow-design assistant (Codex)

> **Codex invocation:** explicitly with `$riglane-create-workflow`, or implicitly when the user signals interest in authoring a workflow. (Codex skills use `$name`, not the `/name` form used by other hosts.)
...
`workflow_learn` / `workflow_validate` are MCP tools on the `workflow_engine` server — in Codex they are exposed through the workflow_engine MCP server in your tool inventory. They are NOT shell commands and NOT skills: never call them via Bash/shell, and never build JSON-RPC bridges to the server.
...
- **Follow-ups the user must run** after the workflow is written:
  1. `riglane init-workflow <workflow>` — regenerates the per-step subagent files with the new tool whitelisted, on every agent-file host installed in the project (Codex itself spawns a generic agent — no per-step files of its own — but other hosts sharing the project have them)
  2. **Restart the host** — every host runs the `workflow_tools` MCP server, and it scans the yamls only at startup, so the new tool stays invisible until then
...
- **Create:** `.riglane/workflows/templates/my_workflows/<name>/workflow.yaml` (+ `structs/`, `scripts/`). The tree starts with `.riglane/` — NOT `.claude/`, `.cursor/` or `.codex/`; a workflow written elsewhere validates fine but the engine NEVER finds it.
