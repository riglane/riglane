...
|----------|----------------------|-------------|
| 1 | engine tools by tool name (`workflow_resolve`, `workflow_init`, …), grouped under the `workflow_engine` server | Call by the name your tools list shows for that server |
| 2 | `CallMcpTool` | `CallMcpTool(server="workflow_engine", toolName="workflow_resolve", arguments={...})` |
| 3 | `mcp__workflow_engine__*` (Claude Code, double underscore — if a CC-style host reads this file) | Call directly by full tool name |

Outcome:

- Found the engine tools in your list (by tool name, under the `workflow_engine` server) → Proceed to Step 1A.
- Found `CallMcpTool` → you have MCP via wrapper. Proceed to Step 1B.
- Found `mcp__workflow_engine__*` → you have MCP via direct tools (Claude Code form). Proceed to Step 1A.
- Found none → the MCP servers may be connected while their tools are simply **not injected into THIS session yet** (Cursor injects them asynchronously — on the FIRST message of a session they are often missing). **Wait ~20 seconds, then re-check your tools list.** If still not found, apply the **Fallback** below (it recognizes any naming form by core tool name), and then **Step 0.5** — the mandatory probe is required before ANY stop. Only after the probe has failed twice → **STOP and tell the user** the servers are configured and connected, but their tools have not been injected into this session — a **known Cursor injection issue, NOT a config problem** (do not send them chasing config or `riglane doctor`). The remedy that actually works: **send a follow-up message** (e.g. "continue") — Cursor frequently injects the tools on the NEXT turn and the probe then succeeds. Reconnecting MCP (Cursor Settings → MCP) or Reload Window is only a LAST resort if the tools stay absent across several turns. **Whatever you do, do NOT flail:** NEVER self-call `riglane run-workflow`, `riglane mcp-server`, `riglane mcp-tools`, or `riglane doctor`, do NOT grep the Riglane source repo, do NOT create your own MCP bridge or run scripts via shell — none of these expose MCP tools in THIS session (`riglane run-workflow` is an EXTERNAL launcher that spawns a NEW agent; you are already inside a launched one). Your only correct action is to STOP and report.
...
- 🛑 Second failure → STOP. Surface the exact error verbatim, then tell the user to **send a follow-up message** — Cursor often injects the tools on the NEXT turn, so a fresh turn is the reliable first fix. Suggest reconnect MCP / Reload Window only as a LAST resort after the tools stay absent across several turns — the servers are connected; the gap is injection into this session, not config. Do NOT ask the user to act BEFORE the probe has failed twice with a concrete error.
...
The engine tools live under the `workflow_engine` server. Cursor groups tools by server and shows them by tool name — call each by the name your tools list shows (do NOT prepend a prefix you don't see):
...
Script tools live under the `workflow_tools` server, named `<workflow>__<tool>`. Call each by the name your tools list shows for that server (Cursor groups by server — they appear by tool name); pass the arguments its schema defines:
```
<workflow>__<tool>({...})
```

> **⚠️ If a script tool isn't in your tool list yet, treat it as "not surfaced yet", NOT absent.** Cursor surfaces MCP tools as their server connects and has no preload/`select` mechanism like Claude Code's `ToolSearch`. For a tool the engine reported *available* (in `workflow_init`'s `tools.available`, or named in a `step_begin` `tool_docs` block) that you cannot see: (1) **wait briefly and re-check** your tool list; (2) try calling it **by the name `tool_docs` gives you** (the `<wf>__<tool>` tool under the `workflow_tools` server, as your tools list shows it) or via `CallMcpTool(server="workflow_tools", toolName="<wf>__<tool>", …)` — never a loose keyword search (it returns 0 even when the tool is registered). Only as a genuine **LAST resort** run the underlying script via the shell — invoke it with the interpreter its `command` declares — `node x.js`, `bash x.sh`, or `python x.py` (for Python: `python3` on Linux/macOS, `python` on Windows; if one reports "command not found", try the other) — and pass each argument as `--key=value` (not `--key value`). Do **NOT** fall back to Bash just because the tool was missing on first look.

**Step 1B — CallMcpTool wrapper (when you only have a `CallMcpTool` wrapper, not direct tools).**

Call engine tools through the wrapper (arguments per each tool's schema):

```
CallMcpTool(server="workflow_engine", toolName="workflow_resolve", arguments={"name": "<workflow>"})
CallMcpTool(server="workflow_engine", toolName="workflow_init", arguments={"name": "<workflow>", "params": {...}})
CallMcpTool(server="workflow_engine", toolName="step_begin", arguments={"name": "<step>"})
```

Script tools use the `workflow_tools` server:
```
CallMcpTool(server="workflow_tools", toolName="<workflow>__<tool>", arguments={...})
```
...
**Anti-pattern #2:** Finding the engine tools directly in your list but ignoring them because the skill mentions `CallMcpTool`. Both are valid MCP paths.
...
- **NEVER invent or guess subagent types.** step_begin returns `subagent_type`; on Cursor the Task tool has NO custom types — spawn your generic agent type and deliver the composed task prompt VERBATIM.
