...
# riglane-scope-add (OpenCode)

Declare a new scope. Self-contained: updates config and creates the directory with empty `_index.json` and `_registry.json`.

> **OpenCode invocation:** loaded on demand via the `skill` tool when the user's request matches — ask conversationally (e.g. "add a payments scope"). OpenCode has no slash form for skills.
