...
export function ensureCursorIgnore(target: string, opts: { dryRun?: boolean } = {}): void {
    ...
    if (normalized.includes(staleNegation) && !normalized.includes(currentNegation)) {
      if (dryRun) {
        process.stdout.write(
          `  WOULD UPDATE  .cursorignore (${staleNegation} -> ${currentNegation})\n`,
        );
        return;
      }
...
function blockMarkers(sentinelId: string): { begin: string; end: string } {
  return { begin: `<!-- ${sentinelId}:BEGIN -->`, end: `<!-- ${sentinelId}:END -->` };
}

const LEGACY_SENTINEL_IDS: Readonly<Record<string, readonly string[]>> = {
  'RIGLANE:SPEC-GUIDANCE': ['ACP:SPEC-GUIDANCE'],
};

function normalizeLegacySentinels(text: string, sentinelId: string): string {
  let out = text;
  for (const legacy of LEGACY_SENTINEL_IDS[sentinelId] ?? []) {
    for (const suffix of [':BEGIN', ':END'] as const) {
...
function blockRegex(sentinelId: string): RegExp {
...
function splitBom(s: string): { bom: string; rest: string } {
  return s.charCodeAt(0) === 0xfeff ? { bom: '﻿', rest: s.slice(1) } : { bom: '', rest: s };
}

function atomicWriteText(absFile: string, content: string): void {
  const tmp = `${absFile}.tmp`;
  writeFileSync(tmp, content, 'utf-8');
  renameSync(tmp, absFile);
}

export function hasManagedBlock(absFile: string, sentinelId: string): boolean {
  if (!isFile(absFile)) return false;
...
export function injectManagedBlock(
  ...
  sentinelId: string,
  body: string,
  opts: { dryRun?: boolean; createIfAbsent?: boolean } = {},
): ManagedBlockResult {
  const dryRun = Boolean(opts.dryRun);
  ...
  if (!exists && !createIfAbsent) return { action: 'absent', file: absFile };
  ...
  let next: string;
  let action: ManagedBlockResult['action'];

  if (matchCount > 0) {
    if (matchCount > 1) {
      process.stdout.write(
        `  WARN  ${sentinelId}: ${matchCount} blocks in ${absFile} — collapsing to one\n`,
      );
    }
    let replaced = false;
...
export function removeManagedBlock(
  ...
  const dryRun = Boolean(opts.dryRun);
  if (!isFile(absFile)) return { action: 'absent', file: absFile };
...
export function isGateCheckCommand(cmd: unknown): boolean {
  return typeof cmd === 'string' && cmd.includes('gate-check');
}

export function isToolCallLoggerCommand(cmd: unknown): boolean {
  return typeof cmd === 'string' && cmd.includes('tool-call-logger');
}

export function isFileGuardCommand(cmd: unknown): boolean {
  return typeof cmd === 'string' && cmd.includes('file-guard');
}

export function isSpawnThrottleCommand(cmd: unknown): boolean {
  return typeof cmd === 'string' && cmd.includes('spawn-throttle');
}

const SHELL_WRAP_PREFIX = 'cmd /c ';

function stripShellWrap(cmd: unknown): unknown {
  return typeof cmd === 'string' && cmd.startsWith(SHELL_WRAP_PREFIX)
    ? cmd.slice(SHELL_WRAP_PREFIX.length)
    : cmd;
}

function hookEqualModuloShellWrap(a: unknown, b: unknown): boolean {
  if (typeof a !== 'object' || a === null || typeof b !== 'object' || b === null) {
    return deepEqual(a, b);
  }
  const na = { ...(a as Record<string, unknown>), command: stripShellWrap((a as Record<string, unknown>).command) };
  const nb = { ...(b as Record<string, unknown>), command: stripShellWrap((b as Record<string, unknown>).command) };
  return deepEqual(na, nb);
}

function mergeHookGroupByFamily(
  settings: Record<string, unknown>,
  event: string,
  srcGroup: Record<string, unknown>,
  isFamily: (cmd: unknown) => boolean,
  force: boolean,
  update: boolean,
): boolean {
  const srcHooksList = (srcGroup.hooks as unknown[] | undefined) ?? [];
  if (srcHooksList.length === 0) return false;
  const ourHook = srcHooksList[0] as Record<string, unknown>;
  ...
  let foundHookIdx: number | null = null;
  for (let gi = 0; gi < groups.length; gi += 1) {
    const groupHooks = ((groups[gi] as Record<string, unknown>).hooks as unknown[] | undefined) ?? [];
    for (let hi = 0; hi < groupHooks.length; hi += 1) {
      if (isFamily((groupHooks[hi] as Record<string, unknown>).command)) {
        foundGroupIdx = gi;
        foundHookIdx = hi;
        break;
      }
    }
    if (foundGroupIdx !== null) break;
  }
  ...
  const group = groups[foundGroupIdx] as Record<string, unknown>;
  const groupHooks = (group.hooks as unknown[] | undefined) ?? [];
  if (groupHooks.length === 1) {
    if (deepEqual(group, srcGroup)) return false;
    if (force || update) {
  ...
  if (deepEqual(groupHooks[idx], ourHook)) return false;
  if (force || update) {
...
export function mergeHooks(
  ...
  const ourHooks = JSON.parse(readFileSync(srcHooksPath, 'utf-8')) as Record<string, unknown>;
  ...
  const ourFirstHook = ourSubagentHooks[0] as Record<string, unknown>;
  const ourCommand = (ourFirstHook.command as string | undefined) ?? '';

  if (existsSync(dstHooksPath)) {
    const existing = JSON.parse(readFileSync(dstHooksPath, 'utf-8')) as Record<string, unknown>;
    ...
    for (let i = 0; i < existingSubagent.length; i += 1) {
      const hook = existingSubagent[i] as Record<string, unknown>;
    ...
    if (foundIdx !== null) {
      if (force || update) {
        ...
        if (!('version' in existing)) existing.version = 1;
...
const CURSOR_LOGGER_EVENTS = ['afterMCPExecution', 'afterShellExecution', 'afterFileEdit'] as const;

export function mergeCursorLoggerHooks(
  srcHooksPath: string,
  dstHooksPath: string,
  opts: MergeOptions = {},
): void {
  ...
  const dryRun = Boolean(opts.dryRun);
  if (!existsSync(dstHooksPath)) return;

  const src = JSON.parse(readFileSync(srcHooksPath, 'utf-8')) as Record<string, unknown>;
  ...
  for (const ev of CURSOR_LOGGER_EVENTS) {
    const srcArr = Array.isArray(srcHooks[ev]) ? (srcHooks[ev] as unknown[]) : [];
    const ourHook = srcArr.find((h) =>
      isToolCallLoggerCommand((h as Record<string, unknown> | null)?.command),
    );
    if (ourHook === undefined) continue;
    const dstArr = setdefaultArray(existingHooks, ev);
    const foundIdx = dstArr.findIndex((h) =>
      isToolCallLoggerCommand((h as Record<string, unknown> | null)?.command),
    );
    if (foundIdx === -1) {
      dstArr.push(ourHook);
  ...
  writeFileSync(dstHooksPath, `${JSON.stringify(existing, null, 2)}\n`, 'utf-8');
  process.stdout.write(
    `  ${replaced.length > 0 ? 'UPDATE' : 'MERGE'} hooks.json (tool-call-logger: ${detail})\n`,
  );
}

function mergeCursorFamilyHook(
  srcHooksPath: string,
  dstHooksPath: string,
  event: string,
  isFamily: (cmd: unknown) => boolean,
  ...
  const src = JSON.parse(readFileSync(srcHooksPath, 'utf-8')) as Record<string, unknown>;
  const srcHooks = (src.hooks as Record<string, unknown> | undefined) ?? {};
  const srcArr = Array.isArray(srcHooks[event]) ? (srcHooks[event] as unknown[]) : [];
  const ourHook = srcArr.find((h) => isFamily((h as Record<string, unknown> | null)?.command));
  if (ourHook === undefined) return;

  const existing = JSON.parse(readFileSync(dstHooksPath, 'utf-8')) as Record<string, unknown>;
  const existingHooks = setdefaultObject(existing, 'hooks');
  const dstArr = setdefaultArray(existingHooks, event);
  const foundIdx = dstArr.findIndex((h) => isFamily((h as Record<string, unknown> | null)?.command));
...
export function mergeCursorFileGuardHook(
  srcHooksPath: string,
  dstHooksPath: string,
  opts: MergeOptions = {},
): void {
...
export function mergeCursorSpawnThrottleHook(
  srcHooksPath: string,
  dstHooksPath: string,
  opts: MergeOptions = {},
): void {
  mergeCursorFamilyHook(
    srcHooksPath,
    dstHooksPath,
    'subagentStart',
    isSpawnThrottleCommand,
    'spawn-throttle',
    opts,
  );
}

export function normalizeCursorHookShell(
  dstHooksPath: string,
  ...
  try {
    existing = JSON.parse(readFileSync(dstHooksPath, 'utf-8')) as Record<string, unknown>;
  } catch {
    return;
  }
  ...
  const normalize = (cmd: string): string => {
    ...
    if (!bare.startsWith(`${CLI_NAME} `)) return cmd;
  ...
  for (const ev of Object.keys(hooks)) {
    ...
    for (const h of arr) {
      if (h && typeof h === 'object' && typeof (h as Record<string, unknown>).command === 'string') {
        ...
        const next = normalize(rec.command as string);
        if (next !== rec.command) {
          rec.command = next;
          changed = true;
        }
  ...
  writeFileSync(dstHooksPath, `${JSON.stringify(existing, null, 2)}\n`, 'utf-8');
  process.stdout.write(
    `  UPDATE  hooks.json (${isWin ? 'wrapped riglane hooks with `cmd /c` (Windows no-shell fix)' : 'unwrapped riglane hooks (non-Windows)'})\n`,
  );
}
...
export function mergeMcpConfig(
  ...
  const ourConfig = JSON.parse(readFileSync(srcMcpPath, 'utf-8')) as Record<string, unknown>;
  const ourServers = (ourConfig.mcpServers as Record<string, unknown> | undefined) ?? {};
  ...
  if (existsSync(dstMcpPath)) {
    const existing = JSON.parse(readFileSync(dstMcpPath, 'utf-8')) as Record<string, unknown>;
...
export function mergeClaudeSettings(
  srcSettingsPath: string,
  dstSettingsPath: string,
  ...
  const src = JSON.parse(readFileSync(srcSettingsPath, 'utf-8')) as Record<string, unknown>;
  ...
  const srcGroup = srcSubagent[0] as Record<string, unknown>;
  const srcHooksList = (srcGroup.hooks as unknown[] | undefined) ?? [];
  ...
  const ourHook = srcHooksList[0] as Record<string, unknown>;
  const ourCommand = (ourHook.command as string | undefined) ?? '';
  ...
  if (existsSync(dstSettingsPath)) {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(
        `ERROR: ${dstSettingsPath} is not valid JSON: ${msg}\n       Refusing to overwrite to avoid data loss. Fix the file manually or remove it, then re-run init.`,
      );
    }
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
      throw new Error(
        `ERROR: ${dstSettingsPath} top-level is not a JSON object. Refusing to overwrite.`,
      );
    }
  ...
  for (let gi = 0; gi < subagentGroups.length; gi += 1) {
    const group = subagentGroups[gi] as Record<string, unknown>;
    const groupHooks = (group.hooks as unknown[] | undefined) ?? [];
    for (let hi = 0; hi < groupHooks.length; hi += 1) {
      const hk = groupHooks[hi] as Record<string, unknown>;
  ...
  let changed = false;
  if (foundGroupIdx !== null) {
    const group = subagentGroups[foundGroupIdx] as Record<string, unknown>;
    const groupHooks = (group.hooks as unknown[] | undefined) ?? [];
    if (groupHooks.length === 1) {
  ...
  if (srcPostToolUse.length > 0) {
    loggerChanged = mergeHookGroupByFamily(
      settings,
      'PostToolUse',
      srcPostToolUse[0] as Record<string, unknown>,
      isToolCallLoggerCommand,
      force,
      update,
    );
  }
  ...
  if (srcPreToolUse.length > 0) {
    fileGuardChanged = mergeHookGroupByFamily(
      settings,
      'PreToolUse',
      srcPreToolUse[0] as Record<string, unknown>,
      isFileGuardCommand,
      force,
      update,
    );
  ...
  if (srcSubagentStart.length > 0) {
    spawnThrottleChanged = mergeHookGroupByFamily(
      settings,
      'SubagentStart',
      srcSubagentStart[0] as Record<string, unknown>,
      isSpawnThrottleCommand,
      force,
      update,
    );
  ...
  if (dryRun) {
    ...
    const detail: string[] = [];
    if (changed) detail.push(`hook: ${ourCommand}`);
    if (loggerChanged) detail.push('PostToolUse: riglane tool-call-logger');
    if (fileGuardChanged) detail.push('PreToolUse: riglane file-guard');
    if (spawnThrottleChanged) detail.push('SubagentStart: riglane spawn-throttle');
    if (envChanged) detail.push(`MAX_MCP_OUTPUT_TOKENS=${mcpTokenLimit}`);
...
export function mergeGeminiSettings(
  ...
  const srcHooks = (src.hooks ?? {}) as Record<string, unknown>;
  for (const [event, groups] of Object.entries(srcHooks)) {
    ...
    for (const groupRaw of groups) {
      ...
      const cmd =
        (((group.hooks as unknown[] | undefined)?.[0] as Record<string, unknown> | undefined)
  ...
  const srcContext = (src.context ?? {}) as Record<string, unknown>;
  const srcFileNames = Array.isArray(srcContext.fileName)
    ? (srcContext.fileName as unknown[]).filter((x): x is string => typeof x === 'string')
    : [];
  if (srcFileNames.length > 0) {
    ...
    if (list.length !== beforeLegacyStrip) contextChanged = true;
    if (list.length === 0) {
      ctx.fileName = [...srcFileNames];
      contextChanged = true;
    } else {
      for (const name of srcFileNames) {
        if (!name.startsWith('riglane-')) continue;
        if (!list.includes(name)) {
          list.push(name);
          contextChanged = true;
        }
      }
      if (contextChanged) ctx.fileName = list;
    }
  }
  ...
  const detail: string[] = [...changedFamilies];
  if (mcpChanged) detail.push('mcpServers: workflow_engine + workflow_tools');
  if (contextChanged) detail.push('context.fileName: riglane-workflow-context.md');
  ...
  for (const d of detail) process.stdout.write(`          ${d}\n`);
}
...
export function mergeCodexConfig(
  ...
  const src = parseToml(srcText) as Record<string, unknown>;
  const srcServers = (src.mcp_servers as Record<string, unknown> | undefined) ?? {};
  const srcHookGroup = firstHookGroup(src, 'SubagentStop');
  const ourHandler = hookHandlers(srcHookGroup)[0] ?? null;
  const ourHookCommand = (ourHandler?.command as string | undefined) ?? '';
  const srcPostGroup = firstHookGroup(src, 'PostToolUse');
  const ourPostHandler = hookHandlers(srcPostGroup)[0] ?? null;
  const ourPostCommand = (ourPostHandler?.command as string | undefined) ?? '';
  ...
  try {
    dst = parseToml(dstText) as Record<string, unknown>;
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    throw new Error(
      `ERROR: ${dstConfigPath} is not valid TOML: ${msg}\n       Refusing to overwrite to avoid data loss. Fix the file manually or remove it, then re-run.`,
    );
  }
  if (typeof dst !== 'object' || dst === null || Array.isArray(dst)) {
    throw new Error(
      `ERROR: ${dstConfigPath} top-level is not a TOML table. Refusing to overwrite.`,
    );
  }

  const existingServers = (dst.mcp_servers as Record<string, unknown> | undefined) ?? {};
  const serverAdds: string[] = [];
  const serverReplaces: string[] = [];
  for (const [name, cfg] of Object.entries(srcServers)) {
    if (name in existingServers) {
      if (deepEqual(existingServers[name], cfg)) continue;
      if (overwrite) serverReplaces.push(name);
    } else {
  ...
  if (srcHookGroup !== null) {
    ...
    if (matches.length === 0) hookAdd = true;
  ...
  if (srcPostGroup !== null) {
    ...
    if (matches.length === 0) postHookAdd = true;
    else if (overwrite && (matches.length > 1 || !deepEqual(matches[0], ourPostHandler)))
      postHookReplace = true;
  }

  const srcAgents = (src.agents as Record<string, unknown> | undefined) ?? {};
  const dstAgents = (dst.agents as Record<string, unknown> | undefined) ?? {};
  const agentAdds: string[] = [];
  const agentReplaces: string[] = [];
  for (const [key, val] of Object.entries(srcAgents)) {
    if (key in dstAgents) {
      if (deepEqual(dstAgents[key], val)) continue;
      if (overwrite) agentReplaces.push(key);
    } else {
      agentAdds.push(key);
    }
  }
  ...
  if (needReplace) {
    ...
    for (const name of [...serverReplaces, ...serverAdds]) servers[name] = srcServers[name];
    if (hookReplace)
      replaceHookHandlersByFamily(dst, 'SubagentStop', isGateCheckCommand, ourHandler);
    if (hookAdd) setdefaultArray(setdefaultObject(dst, 'hooks'), 'SubagentStop').push(srcHookGroup);
    if (postHookReplace)
      replaceHookHandlersByFamily(dst, 'PostToolUse', isToolCallLoggerCommand, ourPostHandler);
    if (postHookAdd) setdefaultArray(setdefaultObject(dst, 'hooks'), 'PostToolUse').push(srcPostGroup);
    if (agentReplaces.length > 0 || agentAdds.length > 0) {
      const agents = setdefaultObject(dst, 'agents');
      for (const key of [...agentReplaces, ...agentAdds]) agents[key] = srcAgents[key];
    }
  ...
  if (serverAdds.length > 0) {
    const addServers: Record<string, unknown> = {};
    for (const name of serverAdds) addServers[name] = srcServers[name];
    frag.mcp_servers = addServers;
  }
  if (hookAdd || postHookAdd) {
    const fragHooks: Record<string, unknown> = {};
    if (hookAdd) fragHooks.SubagentStop = [srcHookGroup];
    if (postHookAdd) fragHooks.PostToolUse = [srcPostGroup];
    frag.hooks = fragHooks;
  }
  if (agentAdds.length > 0) {
    const addAgents: Record<string, unknown> = {};
    for (const key of agentAdds) addAgents[key] = srcAgents[key];
...
export const OPENCODE_LEGACY_INSTRUCTIONS_ENTRIES: readonly string[] = ['.opencode/acp/*.md'];

export const OPENCODE_MCP_SERVERS = ['workflow_engine', 'workflow_tools'] as const;
...
export function mergeOpencodeConfig(
  ...
  const src = JSON.parse(readFileSync(srcTemplatePath, 'utf-8')) as Record<string, unknown>;
  const srcServers = (src.mcp as Record<string, unknown> | undefined) ?? {};
  const srcInstructions = (src.instructions as string[] | undefined) ?? [];
  ...
  let dst: Record<string, unknown>;
  try {
    dst = parseJsonLenient(dstText) as Record<string, unknown>;
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    throw new Error(
      `ERROR: ${dstConfigPath} is not valid JSON: ${msg}\n       Refusing to overwrite to avoid data loss. Fix the file manually or remove it, then re-run.`,
    );
  }
  if (typeof dst !== 'object' || dst === null || Array.isArray(dst)) {
    throw new Error(
      `ERROR: ${dstConfigPath} top-level is not a JSON object. Refusing to overwrite.`,
    );
  }

  const existingServers = (dst.mcp as Record<string, unknown> | undefined) ?? {};
  const serverAdds: string[] = [];
  const serverReplaces: string[] = [];
  for (const [name, cfg] of Object.entries(srcServers)) {
    if (name in existingServers) {
  ...
  const existingInstructions = Array.isArray(dst.instructions)
    ? (dst.instructions as unknown[])
    : [];
  const instructionAdds = srcInstructions.filter((e) => !existingInstructions.includes(e));
  const instructionRemoves = overwrite
    ? existingInstructions.filter(
        (e): e is string => typeof e === 'string' && OPENCODE_LEGACY_INSTRUCTIONS_ENTRIES.includes(e),
      )
    : [];
  ...
  if (schemaAdd) dst.$schema = src.$schema;
  if (serverAdds.length > 0 || serverReplaces.length > 0) {
    const servers = setdefaultObject(dst, 'mcp');
    for (const name of [...serverReplaces, ...serverAdds]) servers[name] = srcServers[name];
  }
  if (instructionRemoves.length > 0) {
    dst.instructions = existingInstructions.filter(
      (e) => !(typeof e === 'string' && instructionRemoves.includes(e)),
    );
  }
  ...
  if (hasForeignKeys || hadJsoncisms) {
    process.stdout.write(
      `  NOTE  ${label} was re-serialized (2-space JSON) — comments/formatting in the original file were not preserved\n`,
    );
  }
}


function firstHookGroup(obj: Record<string, unknown>, event: string): Record<string, unknown> | null {
...
function hookHandlers(group: Record<string, unknown> | null): Record<string, unknown>[] {
  const h = group?.hooks;
...
function findHookHandlersByFamily(
  ...
  for (const group of readArrayPath(obj, ['hooks', event])) {
    ...
    for (const handler of hookHandlers(group as Record<string, unknown>)) {
      if (isFamily(handler.command)) out.push(handler);
    }
  }
  return out;
}

function replaceHookHandlersByFamily(
  obj: Record<string, unknown>,
  event: string,
  isFamily: (cmd: unknown) => boolean,
  ...
  const hooksTable = obj.hooks as Record<string, unknown> | undefined;
  if (!hooksTable || typeof hooksTable !== 'object') return;
  ...
  for (const group of groups) {
    ...
    for (const h of handlers) {
      if (h && typeof h === 'object' && isFamily((h as Record<string, unknown>).command)) {
        if (!placed) {
  ...
  if (keptGroups.length > 0) hooksTable[event] = keptGroups;
  else delete hooksTable[event];
  if (Object.keys(hooksTable).length === 0) delete obj.hooks;
}
...
function setdefaultObject(parent: Record<string, unknown>, key: string): Record<string, unknown> {
  let val = parent[key];
...
function setdefaultArray(parent: Record<string, unknown>, key: string): unknown[] {
  let val = parent[key];
