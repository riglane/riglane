...
import { type WorkflowInventory, buildWorkflowInventory } from '../../catalog/inventory.js';
...
export function isInstalledCommunityWorkflow(cwd: string | undefined, id: string): boolean {
  return existsSync(join(defaultPaths(cwd).communityDir, id, 'workflow.yaml'));
}

export async function runUpdateEntryCli(argv: readonly string[], deps: UpdateEntryCliDeps = {}): Promise<number> {
  ...
  try {
    if (localEntryDir !== undefined) {
      ...
      if (entry.id !== id) throw new EntryError(`the entry dir describes '${entry.id}', not '${id}'`);
      const lockPath = join(localEntryDir, 'entry.lock.yaml');
      if (!existsSync(lockPath)) throw new EntryError(`${localEntryDir} has no entry.lock.yaml`);
  ...
  try {
    ...
    try {
      newWorkflow = loadYaml<Workflow>(join(workflowDir, 'workflow.yaml'));
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      process.stderr.write(`update: fetched workflow.yaml does not parse: ${msg}\nNothing was changed.\n`);
      return 1;
    }
    if ((newWorkflow as { name?: unknown }).name !== id) {
      process.stderr.write(`update: the fetched workflow is named '${String((newWorkflow as { name?: unknown }).name)}', not '${id}'. Nothing was changed.\n`);
      return 1;
    }
    ...
    const confirmed = await confirmByTypingId(
      id,
      confirmationPrompt(id, 'update it'),
      deps.prompt,
    );
    if (!confirmed.ok) {
      process.stderr.write(
    ...
    process.stdout.write(
      `Updated '${id}' to ${entry.source.sha.slice(0, 12)}… — it is SWITCHED OFF again.\n` +
...
async function fetchCurrentEntry(id: string, deps: UpdateEntryCliDeps): Promise<PerEntryDocument | null> {
  ...
  if (revokedRaw !== null) {
    ...
    if (verdict.revoked) {
      process.stderr.write(
        `update: '${id}' has been REVOKED by the catalog${verdict.reason ? ` — ${verdict.reason}` : ''}.\n` +
...
function readInstalledPin(installDir: string): string | null {
  try {
...
function readInstalledInventory(installDir: string): WorkflowInventory {
  try {
    return buildWorkflowInventory(installDir, loadYaml<Workflow>(join(installDir, 'workflow.yaml')));
  } catch {
    const lockRaw = readIfExists(join(installDir, 'entry.lock.yaml'));
    if (lockRaw !== null) {
      try {
        return parseYamlText(lockRaw) as WorkflowInventory;
      } catch {
      }
    }
    return {
...
function printUpdateDiff(
  ...
  oldSha: string | null,
  newSha: string,
  oldInv: WorkflowInventory,
  newInv: WorkflowInventory,
): void {
  ...
  if (!d.shellSurfaceChanged) {
    ...
    for (const t of d.tools.added) w(`    + [tools.${t.name}]  ${t.command}`);
    for (const t of d.tools.removed) w(`    - [tools.${t.name}]  ${t.command}`);
    for (const c of d.tools.changed) {
      w(`    ~ [tools.${c.key}] command changed:`);
      w(`        - ${c.before}`);
      w(`        + ${c.after}`);
    }
    for (const dd of d.deciders.added) w(`    + [${dd.field} @ ${dd.at}]  ${dd.command}`);
    for (const dd of d.deciders.removed) w(`    - [${dd.field} @ ${dd.at}]  ${dd.command}`);
    for (const c of d.deciders.changed) {
      w(`    ~ [${c.key}] command changed:`);
      w(`        - ${c.before}`);
      w(`        + ${c.after}`);
    }
    for (const b of d.executables.added) w(`    + ${b.path}  (${b.bytes} bytes, sha256 ${b.sha256.slice(0, 12)}…)`);
    for (const b of d.executables.removed) w(`    - ${b.path}`);
    ...
    if (d.newFlags.length > 0) {
      ...
      for (const f of d.newFlags) w(`    + ${f.flag}: '${f.match}' in ${f.where}`);
    }
  }
  w('');
...
function serializeEntryProvenance(id: string, source: EntrySource): string {
  const lines = [
