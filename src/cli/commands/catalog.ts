...
export function runCatalogCli(argv: readonly string[]): number {
  const sub = argv[0];
  if (sub === 'pack') return runPack(argv.slice(1));
  process.stderr.write(sub === undefined ? USAGE : `catalog: unknown subcommand '${sub}'\n${USAGE}`);
  return 2;
}

function runPack(argv: readonly string[]): number {
  const toStdout = argv.includes('--stdout');
  let outPath: string | null = null;
  const positionals: string[] = [];
  for (let i = 0; i < argv.length; i += 1) {
    const a = argv[i] as string;
    if (a === '--stdout') continue;
    if (a === '--out') {
      const v = argv[i + 1];
