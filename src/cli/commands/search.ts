...
import { type CatalogIndexRow, EntryError, catalogIndexUrl, validateCatalogIndex } from '../../catalog/entry.js';
...
export async function runSearchCli(argv: readonly string[], deps: SearchCliDeps = {}): Promise<number> {
  ...
  let rows: readonly CatalogIndexRow[];
  try {
  ...
  for (const r of matches) {
    ...
    const shell =
      r.script_tools === 0 && r.deciders === 0 ? 'no shell         ' : `tools:${r.script_tools} deciders:${r.deciders}`.padEnd(17);
...
async function defaultFetchJson(url: string): Promise<unknown | null> {
  ...
  if (!res.ok) throw new EntryError(`${url} → HTTP ${res.status}`);
