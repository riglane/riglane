...
export function load(): RegistryFile {
  ...
  if (!existsSync(p)) return { version: SCHEMA_VERSION, projects: [] };
  try {
...
export function save(reg: RegistryFile): void {
  const p = registryPath();
...
export function findByPath(reg: RegistryFile, absPath: string): Entry | undefined {
  const canon = canonicalize(absPath);
  return reg.projects.find((e) => e.path === canon);
}

export function findById(reg: RegistryFile, id: string): Entry | undefined {
  if (id.length === 0) return undefined;
  return reg.projects.find((e) => e.id === id);
}

export function list(reg?: RegistryFile): Entry[] {
  const r = reg ?? load();
...
export function register(input: {
  ...
  const existing =
    (input.id !== undefined && input.id.length > 0 ? findById(reg, input.id) : undefined) ??
...
export function unregister(absPath: string): boolean {
  const reg = load();
  const canon = canonicalize(absPath);
...
export function isTemporaryLocation(p: string): boolean {
...
function nativeCanonical(p: string): string {
...
function migrateV1Entry(e: unknown): unknown {
  if (e === null || typeof e !== 'object') return e;
  const r = e as Record<string, unknown>;
  if (Array.isArray(r.adapters)) return r;
  const { adapter, ...rest } = r;
  let adapters: AdapterId[];
  if (adapter === 'both') adapters = ['cursor', 'claude'];
  else if (adapter === 'claude') adapters = ['claude'];
  else if (adapter === 'cursor') adapters = ['cursor'];
  else return e;
...
function isValidEntry(e: unknown): e is Entry {
  ...
  const r = e as Record<string, unknown>;
  if (r.id !== undefined && (typeof r.id !== 'string' || r.id.length === 0)) return false;
  return (
    typeof r.slug === 'string' &&
    typeof r.path === 'string' &&
    Array.isArray(r.adapters) &&
    r.adapters.length > 0 &&
    r.adapters.every((a) => (SELECTABLE_ADAPTERS as readonly string[]).includes(a as string)) &&
...
function computeSlug(absPath: string, existing: readonly Entry[]): string {
...
export function pathExistsAsDir(p: string): boolean {
  try {
