...
function parseMarker(raw: string): MarkerPayload | null {
  const trimmed = raw.trim();
  if (trimmed.length === 0 || trimmed[0] !== '{') return null;
  try {
    const obj = JSON.parse(trimmed) as Partial<MarkerPayload>;
...
export function probe(entry: Entry): ProbeResult {
  return { ...probeDrift(entry), temporary: isTemporaryLocation(entry.path) };
}

function probeDrift(entry: Entry): Omit<ProbeResult, 'temporary'> {
