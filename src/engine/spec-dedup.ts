...
export const SPEC_DEDUP_WEIGHTS = { title: 0.4, applies_to: 0.25, summary: 0.15 } as const;
export const SPEC_DEDUP_THRESHOLD = 0.72;
export const SPEC_DEDUP_NEAR_CERTAIN = 0.95;
export const DOMAIN_DEDUP_WEIGHTS = { name: 0.6, description: 0.4 } as const;
...
export function scoreSpecPair(a: SpecLike, b: SpecLike): SpecPairScore {
  ...
  let score =
    SPEC_DEDUP_WEIGHTS.title * titleSim +
  ...
  if (titleExact) signals.push('same title');
  else if (titleSim >= 0.7) signals.push(`similar title (${titleSim.toFixed(2)})`);
  if (appliesSim >= 0.5) signals.push(`shared applies_to (${appliesSim.toFixed(2)})`);
  if (summarySim >= 0.7) signals.push(`similar summary (${summarySim.toFixed(2)})`);
...
export function findSpecDuplicates(
  candidate: SpecLike,
  existing: readonly SpecIndexEntry[],
  opts?: { crossDomain?: boolean },
): SpecMatch[] {
...
export function dedupMessage(candidateTitle: string, match: SpecMatch): string {
  const why = match.signals.join(', ');
  return (
    `'${candidateTitle}' looks like existing spec ${match.spec_id} '${match.title}' — ${why}. ` +
...
export function findDomainDuplicates(
  candidate: { name: string; description: string },
  existing: readonly DomainEntry[],
): DomainMatch[] {
  return existing
    ...
    .map((d): DomainMatch => {
      ...
      const score =
        DOMAIN_DEDUP_WEIGHTS.name * nameSim + DOMAIN_DEDUP_WEIGHTS.description * descSim;
      const signals: string[] = [];
      if (nameSim >= 0.6) signals.push(`similar name (${nameSim.toFixed(2)})`);
      if (descSim >= 0.6) signals.push(`similar description (${descSim.toFixed(2)})`);
