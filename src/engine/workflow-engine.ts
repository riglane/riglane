...
import type {
  AgentNotesSearchInput,
  AgentNotesSearchOutput,
  AgentNotesSummary,
  AgentNotesWriteInput,
  AgentNotesWriteOutput,
  EngineRunOverrides,
  StepBeginDynamicInput,
  StepBeginDynamicOutput,
  StepCollectResultDynamicInput,
  StepCollectResultDynamicOutput,
  StepCompleteDynamicInput,
  StepCompleteDynamicOutput,
  WorkflowFinalizeDynamicInput,
  WorkflowFinalizeDynamicOutput,
  WorkflowInitInput,
  WorkflowInitOutput,
  WorkflowInvokeDynamicInput,
  WorkflowInvokeDynamicOutput,
  WorkflowInvokeDynamicSuccess,
  WorkflowResolveInput,
  WorkflowResolveOutput,
  WorkflowResolveStepEntry,
  WorkflowResumeInput,
  WorkflowResumeOutput,
...
import {
  AGENT_NOTES_VERSION,
  appendIndexEntry,
  assertSafeStepTemplate,
  composeNoteFile,
  ensureStepTemplateNotesDir,
  generateNoteFilename,
  isoDateLocal,
  noteFilePath,
  readIndex,
  stepTemplateNotesDir,
} from './agent-notes.js';
import type { NoteFrontmatter } from './agent-notes.js';
import type { Trace } from '../types/trace.js';
import { parse as parseYamlString } from 'yaml';
import type { ModelMode, PlanningRestrictions, Step, Workflow } from '../types/workflow.js';
...
import type { DynamicOriginMeta } from './dynamic-runs.js';
import {
  copyStructsToDynamicRuntime,
  dynamicChildWorkflowName,
  dynamicRuntimeDir,
  writeDynamicWorkflow,
} from './dynamic-runs.js';
...
import {
  clearUserActiveScope as scopeClearUserActiveScope,
  getScopeHint as scopeGetScopeHint,
  readUserActiveScope as scopeReadUserActiveScope,
  resolveActiveScope as scopeResolveActiveScope,
  scopeExists as scopeScopeExists,
  validateScopeId as scopeValidateScopeId,
  writeUserActiveScope as scopeWriteUserActiveScope,
} from '../scope/scope-context.js';
...
export function defaultPaths(cwd?: string): EnginePaths {
...
function withTraceLock(tracePath: string, fn: () => void): void {
  const fd = acquireTraceLockOrNull(tracePath);
  try {
    fn();
  } finally {
    if (fd !== null) releaseFileLock(fd, `${tracePath}.lock`);
  }
}

function acquireTraceLockOrNull(tracePath: string): number | null {
...
function logEngine(message: string): void {
...
function collectAllSequences(
  workflow: Workflow,
): Array<{ steps: ReadonlyArray<Record<string, unknown>>; label: string }> {
  const seqs: Array<{ steps: ReadonlyArray<Record<string, unknown>>; label: string }> = [];
  const visit = (steps: ReadonlyArray<Record<string, unknown>> | undefined, label: string): void => {
    seqs.push({ steps: steps ?? [], label });
    for (const s of steps ?? []) {
      const routes = s.routes as
        | { define?: ReadonlyArray<{ id?: string; steps?: ReadonlyArray<Record<string, unknown>> }> }
        | undefined;
      if (routes && Array.isArray(routes.define)) {
        for (const r of routes.define) visit(r.steps, `${label} → route '${r.id ?? '<unnamed>'}'`);
      }
      const lanes = s.lanes as
        | { define?: ReadonlyArray<{ id?: string; steps?: ReadonlyArray<Record<string, unknown>> }> }
        | undefined;
      if (lanes && Array.isArray(lanes.define)) {
        for (const l of lanes.define) visit(l.steps, `${label} → lane '${l.id ?? '<unnamed>'}'`);
      }
    }
  };
  visit(workflow.steps as unknown as ReadonlyArray<Record<string, unknown>>, 'main');
  return seqs;
}

function collectAllRoutes(
  workflow: Workflow,
): Array<{ id: string; steps: ReadonlyArray<Record<string, unknown>>; ownerStep: string }> {
  const routes: Array<{ id: string; steps: ReadonlyArray<Record<string, unknown>>; ownerStep: string }> = [];
  for (const s of collectAllSteps(workflow)) {
    const r = s.routes as
      | { define?: ReadonlyArray<{ id?: string; steps?: ReadonlyArray<Record<string, unknown>> }> }
      | undefined;
    if (!r || !Array.isArray(r.define)) continue;
    const ownerStep = (s.name as string | undefined) ?? '<unnamed>';
    for (const def of r.define) {
      routes.push({
        id: (def.id as string | undefined) ?? '<unnamed>',
        steps: (def.steps ?? []) as ReadonlyArray<Record<string, unknown>>,
        ownerStep,
      });
    }
  }
...
function resolveActiveSequence(
  workflow: Workflow | { steps?: ReadonlyArray<Record<string, unknown>> },
  manifest: Record<string, unknown>,
): ReadonlyArray<Record<string, unknown>> {
  const stack = (manifest.route_stack as ReadonlyArray<{ route_id: string }> | undefined) ?? [];
  const mainSteps = (workflow.steps ?? []) as unknown as ReadonlyArray<Record<string, unknown>>;
  if (stack.length === 0) return mainSteps;
  const topId = stack[stack.length - 1]?.route_id;
  for (const r of collectAllRoutes(workflow as Workflow)) {
    if (r.id === topId) return r.steps;
  }
  return mainSteps;
}

function computeProceedTarget(
  workflow: Workflow | { steps?: ReadonlyArray<Record<string, unknown>> },
  manifest: Record<string, unknown>,
  stepName: string,
): string | null {
  const seq = resolveActiveSequence(workflow, manifest);
  const names = seq.map((s) => (s.name as string | undefined) ?? '');
  const idx = names.indexOf(stepName);
  if (idx >= 0 && idx + 1 < names.length) return names[idx + 1] ?? null;
  const stack =
    (manifest.route_stack as ReadonlyArray<{ return_to: string | null }> | undefined) ?? [];
  if (stack.length > 0) return stack[stack.length - 1]?.return_to ?? null;
  return null;
}


interface LaneCtx {
  readonly forkStep: string;
  readonly laneId: string;
  readonly laneSteps: ReadonlyArray<Record<string, unknown>>;
}

function laneContextOf(
  workflow: Workflow | { steps?: ReadonlyArray<Record<string, unknown>> },
  stepName: string,
): LaneCtx | null {
  let found: LaneCtx | null = null;
  const visit = (steps: ReadonlyArray<Record<string, unknown>> | undefined, ctx: LaneCtx | null): void => {
    for (const s of steps ?? []) {
      if (s.name === stepName && ctx) found = ctx;
      const routes = s.routes as
        | { define?: ReadonlyArray<{ steps?: ReadonlyArray<Record<string, unknown>> }> }
        | undefined;
      if (routes && Array.isArray(routes.define)) {
        for (const r of routes.define) visit(r.steps, ctx);
      }
      const lanes = s.lanes as
        | { define?: ReadonlyArray<{ id?: string; steps?: ReadonlyArray<Record<string, unknown>> }> }
        | undefined;
      if (lanes && Array.isArray(lanes.define)) {
        const owner = (s.name as string | undefined) ?? '<unnamed>';
        for (const l of lanes.define) {
          visit(l.steps, {
            forkStep: owner,
            laneId: (l.id as string | undefined) ?? '<unnamed>',
            laneSteps: (l.steps ?? []) as ReadonlyArray<Record<string, unknown>>,
          });
        }
      }
    }
  };
  visit(workflow.steps as unknown as ReadonlyArray<Record<string, unknown>>, null);
  return found;
}

function staticSequenceOf(
  workflow: Workflow | { steps?: ReadonlyArray<Record<string, unknown>> },
  stepName: string,
): ReadonlyArray<Record<string, unknown>> {
  let found: ReadonlyArray<Record<string, unknown>> | null = null;
  const visit = (steps: ReadonlyArray<Record<string, unknown>> | undefined): void => {
    const list = steps ?? [];
    for (const s of list) {
      if (s.name === stepName && !found) found = list;
      const routes = s.routes as
        | { define?: ReadonlyArray<{ steps?: ReadonlyArray<Record<string, unknown>> }> }
        | undefined;
      if (routes && Array.isArray(routes.define)) for (const r of routes.define) visit(r.steps);
      const lanes = s.lanes as
        | { define?: ReadonlyArray<{ steps?: ReadonlyArray<Record<string, unknown>> }> }
        | undefined;
      if (lanes && Array.isArray(lanes.define)) for (const l of lanes.define) visit(l.steps);
    }
  };
  visit(workflow.steps as unknown as ReadonlyArray<Record<string, unknown>>);
  return found ?? ((workflow.steps ?? []) as unknown as ReadonlyArray<Record<string, unknown>>);
}

type StepContainer =
  | { readonly kind: 'main' }
  | { readonly kind: 'route'; readonly ownerStep: string; readonly routeId: string }
  | { readonly kind: 'lane'; readonly forkStep: string; readonly laneId: string };

function containerOf(
  workflow: Workflow | { steps?: ReadonlyArray<Record<string, unknown>> },
  stepName: string,
): StepContainer {
  let found: StepContainer | null = null;
  const visit = (steps: ReadonlyArray<Record<string, unknown>> | undefined, ctx: StepContainer): void => {
    for (const s of steps ?? []) {
      if (s.name === stepName && !found) found = ctx;
      const owner = (s.name as string | undefined) ?? '<unnamed>';
      const routes = s.routes as
        | { define?: ReadonlyArray<{ id?: string; steps?: ReadonlyArray<Record<string, unknown>> }> }
        | undefined;
      if (routes && Array.isArray(routes.define)) {
        for (const r of routes.define) {
          visit(r.steps, { kind: 'route', ownerStep: owner, routeId: (r.id as string | undefined) ?? '<unnamed>' });
        }
      }
      const lanes = s.lanes as
        | { define?: ReadonlyArray<{ id?: string; steps?: ReadonlyArray<Record<string, unknown>> }> }
      ...
      if (lanes && Array.isArray(lanes.define)) {
        for (const l of lanes.define) {
          visit(l.steps, { kind: 'lane', forkStep: owner, laneId: (l.id as string | undefined) ?? '<unnamed>' });
        }
      }
    }
  };
  visit(workflow.steps as unknown as ReadonlyArray<Record<string, unknown>>, { kind: 'main' });
  return found ?? { kind: 'main' };
}

function staticNextOf(
  workflow: Workflow | { steps?: ReadonlyArray<Record<string, unknown>> },
  stepName: string,
): string | null {
...
function laneStateOf(manifest: Record<string, unknown>, forkStep: string): LaneState | null {
  const stepData = (manifest.steps as Record<string, Record<string, unknown>> | undefined)?.[forkStep];
  const ls = stepData?.lane_state as LaneState | undefined;
  return ls && typeof ls === 'object' && ls.lanes ? ls : null;
}

function activeLaneOwners(manifest: Record<string, unknown>): string[] {
  const raw = manifest.active_lanes;
  return Array.isArray(raw) ? raw.filter((x): x is string => typeof x === 'string') : [];
}

function admissibleLaneCursors(
  manifest: Record<string, unknown>,
): Array<{ forkStep: string; laneId: string; cursor: string }> {
  const owners = new Set(activeLaneOwners(manifest));
  const out: Array<{ forkStep: string; laneId: string; cursor: string }> = [];
  for (const forkStep of owners) {
    ...
    for (const [laneId, entry] of Object.entries(ls.lanes)) {
      ...
      if (typeof entry.cursor !== 'string' || !entry.cursor) continue;
      if (owners.has(entry.cursor)) continue;
...
function ruleNoBranchLiteralInParallelOutputs(
  workflow: Workflow,
): string[] {
  const errors: string[] = [];
  ...
  for (const step of steps) {
    ...
    const stepName = (step.name as string | undefined) ?? '<unnamed>';
    const outputs = (step.outputs ?? []) as ReadonlyArray<unknown>;
    for (const out of outputs) {
      const opath =
        typeof out === 'string'
          ? out
          : ((out as Record<string, unknown>)?.path as string | undefined) ?? '';
      if (!opath) continue;
      if (/(^|\/)_branch_/.test(opath)) {
        errors.push(
          `step '${stepName}' outputs.path contains a literal '_branch_*' segment: ` +
...
function ruleNoEnginePromptsNamespace(workflow: Workflow): string[] {
  const errors: string[] = [];
  const inNamespace = (p: string): boolean => p === 'prompts' || p.startsWith('prompts/');
  for (const step of collectAllSteps(workflow)) {
    const stepName = (step.name as string | undefined) ?? '<unnamed>';
    for (const [kind, list] of [
      ['inputs', (step.inputs ?? []) as ReadonlyArray<unknown>],
      ['outputs', (step.outputs ?? []) as ReadonlyArray<unknown>],
    ] as const) {
      for (const raw of list) {
        const path =
          typeof raw === 'string'
            ? raw
            : ((raw as Record<string, unknown>)?.path as string | undefined) ?? '';
        if (path && inNamespace(path)) {
          errors.push(
            `step '${stepName}' ${kind} path '${path}' is under the run-root 'prompts/' ` +
...
function ruleParallelOutputPaths(workflow: Workflow): string[] {
  const errors: string[] = [];
  const validParams = new Set<string>(['run_id', 'iteration']);
  const params = (workflow.params ?? []) as ReadonlyArray<{ name?: unknown }>;
  for (const p of params) {
    if (typeof p?.name === 'string') {
      validParams.add(p.name);
      if (p.name === 'parallel_key') {
        errors.push(
          `workflow param 'parallel_key' is a reserved namespace — it is used by ` +
    ...
    for (const out of outputs) {
      ...
      for (const m of opath.matchAll(/\{([^}]*)\}/g)) {
        ...
        if (!validParams.has(token)) {
          errors.push(
            `step '${stepName}' parallel output '${opath}' uses '{${token}}', which is ` +
      ...
      if (!hasParallelKey && !hasGlob) {
        errors.push(
          `step '${stepName}' parallel output '${opath}' is the same for every branch ` +
...
function ruleNonParallelOutputPaths(workflow: Workflow): string[] {
  const errors: string[] = [];
  const validParams = new Set<string>(['run_id', 'iteration']);
  const params = (workflow.params ?? []) as ReadonlyArray<{ name?: unknown }>;
  for (const p of params) {
    if (typeof p?.name === 'string') validParams.add(p.name);
  }
  for (const step of collectAllSteps(workflow)) {
    if (step.parallel) continue;
    const stepName = (step.name as string | undefined) ?? '<unnamed>';
    ...
    for (const out of outputs) {
      ...
      for (const m of opath.matchAll(/\{([^}]*)\}/g)) {
        ...
        if (token === '') {
          errors.push(
            `step '${stepName}' output '${opath}' uses '{}' (branch-index slot), which is only ` +
        ...
        if (token === 'parallel_key' || token.startsWith('parallel_key.')) {
          errors.push(
            `step '${stepName}' output '${opath}' uses '{${token}}' (parallel_key namespace), ` +
        ...
        if (!validParams.has(token)) {
          errors.push(
            `step '${stepName}' output '${opath}' uses '{${token}}', which is not a declared ` +
...
function ruleStructSchemasExist(workflow: Workflow, definitionDir: string): string[] {
  const structRefs = new Map<string, string[]>();
  const recordRef = (name: string, location: string): void => {
  ...
  for (const step of steps) {
    ...
    for (const section of ['inputs', 'outputs'] as const) {
      const items = (step[section] ?? []) as ReadonlyArray<unknown>;
      for (const item of items) {
        if (typeof item !== 'object' || item === null) continue;
        const structName = (item as Record<string, unknown>).struct as string | undefined;
        if (structName) recordRef(structName, `step '${stepName}' ${section}`);
      }
    }
  ...
  for (const [structName, locations] of structRefs) {
    ...
    try {
      ...
      if (!st.isFile()) {
        errors.push(
          `struct '${structName}' (referenced at: ${locations.join(', ')}) — exists but is not a file: ${schemaPath}`,
        );
        continue;
      }
    } catch (e) {
      ...
      errors.push(
        `struct '${structName}' (referenced at: ${locations.join(', ')}) — expected at: ${schemaPath}`,
      );
      continue;
    }
    let parsed: unknown;
    try {
      ...
      errors.push(
        `struct '${structName}' (referenced at: ${locations.join(', ')}) — ${schemaPath} is not valid YAML: ` +
    ...
    if (unknownKeys.length > 0 && !structHasFieldContainer(parsed)) {
      ...
      errors.push(
        `struct '${structName}' (referenced at: ${locations.join(', ')}) — ${schemaPath} has ` +
...
function ruleDeciderScriptsExist(workflow: Workflow, definitionDir: string): string[] {
  ...
  const errors: string[] = [];
  const checkCommand = (cmd: unknown, where: string): void => {
    ...
    for (const rawTok of cmd.split(/\s+/)) {
      ...
      if (isAbs(tok)) continue;
      if (existsSync(join(projectRoot, tok))) continue;
      if (existsSync(join(definitionDir, tok))) {
        errors.push(
          `${where}: decider script '${tok}' will NOT be found at runtime — deciders run ` +
        ...
        errors.push(
          `${where}: decider script '${tok}' not found from the project root (deciders run ` +
  ...
  const wfHumanCfg = ((workflow.gate ?? {}) as { human?: { script?: unknown } }).human;
  if (typeof wfHumanCfg === 'object' && wfHumanCfg !== null) {
    checkCommand(wfHumanCfg.script, `workflow gate.human.script`);
  }
  for (const s of collectAllSteps(workflow)) {
    const stepName = (s.name as string | undefined) ?? '<unnamed>';
    const lb = s.loop_back as { when?: { script?: unknown } } | undefined;
    if (lb?.when) checkCommand(lb.when.script, `step '${stepName}' loop_back.when.script`);
    const rt = s.routes as { when?: { script?: unknown } } | undefined;
    if (rt?.when) checkCommand(rt.when.script, `step '${stepName}' routes.when.script`);
    const gh = ((s.gate ?? {}) as { human?: { script?: unknown } }).human;
...
function rulePlanningStepTypes(
  workflow: Workflow,
  allowParallel: boolean,
  allowDelegation: boolean,
): string[] {
  const errors: string[] = [];
  const steps = (workflow.steps ?? []) as unknown as ReadonlyArray<Record<string, unknown>>;
  for (const step of steps) {
    const stepName = (step.name as string | undefined) ?? '<unnamed>';
    if (step.parallel === true && !allowParallel) {
      errors.push(
        `step '${stepName}' has 'parallel: true' but parent planning step has ` +
          `allow_parallel=false. Remove 'parallel: true' or split into separate substeps.`,
      );
    }
    if ('delegate_to' in step && !allowDelegation) {
      errors.push(
        `step '${stepName}' has 'delegate_to: ${String(step.delegate_to)}' but parent ` +
    ...
    if (step.type === 'planning') {
      errors.push(
        `step '${stepName}' has 'type: planning' — nested planning steps are forbidden ` +
          `(prevents infinite planner recursion).`,
      );
    }
    if (step.routes !== undefined) {
      errors.push(
        `step '${stepName}' declares a 'routes' block — conditional routing is forbidden ` +
    ...
    if (step.spec_authoring !== undefined) {
      errors.push(
        `step '${stepName}' declares 'spec_authoring' — spec capability flags are ` +
    ...
    if (step.spec_check !== undefined && step.spec_check !== false) {
      errors.push(
        `step '${stepName}' declares 'spec_check: true' — spec capability flags are ` +
          `forbidden in orchestrator-drafted planning children (the generic subagent ` +
...
function ruleSubstepCount(workflow: Workflow, max: number): string[] {
...
function ruleNoForbiddenTopLevel(workflow: Workflow): string[] {
  const errors: string[] = [];
  const w = workflow as unknown as Record<string, unknown>;
  if (w.tools !== undefined) {
    errors.push(
      `generated workflow declares top-level 'tools:' — forbidden. Child inherits ` +
        `parent's tools whitelist (security boundary).`,
    );
  }
  if (w.params !== undefined) {
    errors.push(
      `generated workflow declares top-level 'params:' — forbidden. Pass values via ` +
...
function ruleStepNameUniqueness(workflow: Workflow): string[] {
  const errors: string[] = [];
  const seen = new Set<string>();
  const steps = collectAllSteps(workflow);
  for (const step of steps) {
    const name = (step?.name as string | undefined) ?? '<unnamed>';
    if (seen.has(name)) {
      errors.push(
        `duplicate step name '${name}' — step names MUST be globally unique within a ` +
...
function rulePlanningStepNoOutputs(workflow: Workflow): string[] {
  const errors: string[] = [];
  const steps = collectAllSteps(workflow);
  for (const step of steps) {
    if (step.type !== 'planning') continue;
    const outputs = step.outputs as ReadonlyArray<unknown> | undefined;
    if (Array.isArray(outputs) && outputs.length > 0) {
      const stepName = (step.name as string | undefined) ?? '<unnamed>';
      errors.push(
        `planning step '${stepName}' declares 'outputs' — forbidden. Planning steps ` +
...
function rulePlanningStepNoStepGate(workflow: Workflow): string[] {
  const errors: string[] = [];
  const steps = collectAllSteps(workflow);
  for (const step of steps) {
    if (step.type !== 'planning') continue;
    if (step.gate !== undefined) {
      const stepName = (step.name as string | undefined) ?? '<unnamed>';
      errors.push(
        `planning step '${stepName}' declares a step-level 'gate:' — forbidden, it has ` +
...
function ruleLoopBackValid(workflow: Workflow): string[] {
  const errors: string[] = [];
  const allSteps = collectAllSteps(workflow);
  const anyLoopBack = allSteps.some((s) => s.loop_back !== undefined);
  if (anyLoopBack) {
    const declared = (workflow.params ?? []) as unknown as ReadonlyArray<Record<string, unknown>>;
    if (declared.some((p) => p.name === 'iteration')) {
      errors.push(
        `workflow declares a param named 'iteration' but also uses loop_back — ` +
    ...
    for (const s of allSteps) {
      const outputs = s.outputs as ReadonlyArray<Record<string, unknown>> | undefined;
      if (!Array.isArray(outputs)) continue;
      const sName = (s.name as string | undefined) ?? '<unnamed>';
      for (const out of outputs) {
        ...
        if (out.per_iteration === true || p.includes('{iteration}')) {
          errors.push(
            `step '${sName}' output '${p}' uses per_iteration/{iteration} but the ` +
  ...
  for (const seq of collectAllSequences(workflow)) {
    ...
    const names = steps.map((s) => (s.name as string | undefined) ?? '<unnamed>');
    const ranges: Array<{ from: number; to: number; owner: string }> = [];
    for (let i = 0; i < steps.length; i += 1) {
      const lb = steps[i]?.loop_back as { to?: string } | undefined;
      ...
      if (toIdx < 0) {
        errors.push(
          `step '${stepName}' loop_back.to references '${to}', which is not a step in the ` +
      ...
      if (toIdx > i) {
        errors.push(
          `step '${stepName}' loop_back.to ('${to}') is a LATER step — loop_back may ` +
      ...
      for (const prev of ranges) {
        ...
        const nested =
          (toIdx >= prev.from && i <= prev.to) || (prev.from >= toIdx && prev.to <= i);
        if (disjoint || nested) continue;
        crossed = true;
        errors.push(
          `step '${stepName}' loop range [${to} .. ${stepName}] partially overlaps the ` +
      ...
      for (let j = toIdx; j <= i; j += 1) {
        ...
        if (s?.parallel === true || 'delegate_to' in (s ?? {}) || s?.type === 'planning') {
          errors.push(
            `step '${sName}' is inside the loop range [${to} .. ${stepName}] but is not a ` +
              `plain regular step (parallel/delegation/planning) — forbidden in v1. ` +
              `Loop ranges may contain only regular steps.`,
          );
        } else if (!isOwner && (s as Record<string, unknown>)?.routes !== undefined) {
          errors.push(
            `step '${sName}' is inside the loop range [${to} .. ${stepName}] and declares a ` +
        ...
        } else if (!isOwner && (s as Record<string, unknown>)?.lanes !== undefined) {
          errors.push(
            `step '${sName}' is inside the loop range [${to} .. ${stepName}] and declares a ` +
...
function collectAllLanes(
  workflow: Workflow,
): Array<{ id: string; steps: ReadonlyArray<Record<string, unknown>>; ownerStep: string; join: { require?: string } }> {
  const lanes: Array<{ id: string; steps: ReadonlyArray<Record<string, unknown>>; ownerStep: string; join: { require?: string } }> = [];
  for (const s of collectAllSteps(workflow)) {
    const l = s.lanes as
      | { define?: ReadonlyArray<{ id?: string; steps?: ReadonlyArray<Record<string, unknown>> }>; join?: { require?: string } }
      | undefined;
    if (!l || !Array.isArray(l.define)) continue;
    const ownerStep = (s.name as string | undefined) ?? '<unnamed>';
...
function ruleLanesValid(workflow: Workflow): string[] {
  const errors: string[] = [];
  const allLanes = collectAllLanes(workflow);
  const seenId = new Map<string, string>();
  for (const r of collectAllRoutes(workflow)) seenId.set(r.id, `route in step '${r.ownerStep}'`);
  for (const l of allLanes) {
    if (l.id === 'proceed') {
      errors.push(
        `lane id 'proceed' (in step '${l.ownerStep}') is reserved. Rename the lane.`,
      );
    }
    const prev = seenId.get(l.id);
    if (prev !== undefined) {
      errors.push(
        `duplicate lane id '${l.id}' (in step '${l.ownerStep}'; also ${prev}). Lane ids ` +
  ...
  for (const s of collectAllSteps(workflow)) {
    if (s.lanes !== undefined && s.routes !== undefined) {
      errors.push(
        `step '${String(s.name)}' declares BOTH 'routes' and 'lanes' — the two blocks move ` +
  ...
  const byOwner = new Map<string, typeof allLanes>();
  for (const l of allLanes) {
  ...
  for (const [owner, siblings] of byOwner) {
    ...
    const paramOwner = new Map<string, string>();
    for (const lane of siblings) {
      for (const s of collectAllSteps({ steps: lane.steps as unknown[] })) {
        const outputs = s.outputs as ReadonlyArray<Record<string, unknown> | string> | undefined;
        if (Array.isArray(outputs)) {
          for (const out of outputs) {
            const p = typeof out === 'string' ? out : typeof out?.path === 'string' ? (out.path as string) : '';
            if (!p) continue;
            const prevLane = outputOwner.get(p);
            if (prevLane !== undefined && prevLane !== lane.id) {
              errors.push(
                `lanes '${prevLane}' and '${lane.id}' (fork step '${owner}') both declare the ` +
        ...
        const bindings = s.param_bindings as Record<string, unknown> | undefined;
        if (bindings && typeof bindings === 'object') {
          for (const paramName of Object.keys(bindings)) {
            const prevLane = paramOwner.get(paramName);
            if (prevLane !== undefined && prevLane !== lane.id) {
              errors.push(
                `lanes '${prevLane}' and '${lane.id}' (fork step '${owner}') both bind the ` +
...
function ruleRoutesValid(workflow: Workflow): string[] {
  const errors: string[] = [];
  const seenId = new Map<string, string>();
  for (const r of collectAllRoutes(workflow)) {
    if (r.id === 'proceed') {
      errors.push(
        `route id 'proceed' (in step '${r.ownerStep}') is reserved — it is the sentinel ` +
    ...
    if (prevOwner !== undefined) {
      errors.push(
        `duplicate route id '${r.id}' (in step '${r.ownerStep}'; also in step ` +
...
function ruleSpecAuthoringValid(workflow: Workflow): string[] {
  const errors: string[] = [];
  let hasPersist = false;
  for (const s of collectAllSteps(workflow)) {
    if (s.spec_authoring === 'persist') hasPersist = true;
    if (s.spec_authoring === 'persist' && s.parallel === true) {
      errors.push(
        `step '${String(s.name)}': spec_authoring: persist cannot be combined with ` +
  ...
  if (hasPersist) {
    const params = Array.isArray(workflow.params)
      ? (workflow.params as ReadonlyArray<Record<string, unknown>>)
      : [];
    if (!params.some((p) => p.name === 'scope')) {
      errors.push(
        `a step declares spec_authoring: persist but the workflow has no 'scope' ` +
...
export function fullValidateWorkflow(
  workflow: Workflow,
  opts: { readonly definitionDir?: string } = {},
): { ok: boolean; errors: string[]; warnings: LintWarning[] } {
  const errors: string[] = [];
  ...
  const warnings = lintWorkflow(
    ...
    opts.definitionDir !== undefined ? { definitionDir: opts.definitionDir } : {},
  );

  return { ok: errors.length === 0, errors, warnings };
}

function ruleParallelRequiresSubagent(workflow: Workflow): string[] {
  const errors: string[] = [];
  for (const s of collectAllSteps(workflow)) {
    if (s.parallel === true && s.subagent === false) {
      errors.push(
        `step '${String(s.name)}': parallel: true cannot be combined with subagent: false — ` +
...
function ruleParamBindingsReservedNames(workflow: Workflow): string[] {
  const errors: string[] = [];
  for (const s of collectAllSteps(workflow)) {
    const bindings = (s.param_bindings ?? {}) as Record<string, unknown>;
    for (const key of Object.keys(bindings)) {
      if (key === 'run_id' || key === 'iteration') {
        errors.push(
          `step '${String(s.name)}': param_bindings must not bind to '${key}' — it is an ` +
...
function ruleFromDelegatedValid(workflow: Workflow): string[] {
  const errors: string[] = [];
  for (const s of collectAllSteps(workflow)) {
    const outputs = (s.outputs ?? []) as ReadonlyArray<Record<string, unknown> | string>;
    for (const o of outputs) {
      if (typeof o !== 'object' || o === null || !('from_delegated' in o)) continue;
      const src = o.from_delegated;
      const stepName = String(s.name);
      if (typeof src !== 'string' || src.trim() === '') {
        errors.push(
          `step '${stepName}': from_delegated must be a non-empty string (a path in the ` +
      ...
      if (!('delegate_to' in s)) {
        errors.push(
          `step '${stepName}': from_delegated is only valid on a delegation step ` +
      ...
      if (/[*?[]/.test(dest)) {
        errors.push(
          `step '${stepName}': output path '${dest}' with from_delegated must be a CONCRETE ` +
...
function ruleStepToolsExist(workflow: Workflow): string[] {
  const errors: string[] = [];
  const declared = new Set(
    (Array.isArray(workflow.tools) ? (workflow.tools as ReadonlyArray<Record<string, unknown>>) : [])
      .map((t) => t.name)
      .filter((n): n is string => typeof n === 'string'),
  );
  for (const step of collectAllSteps(workflow)) {
    const filter = (step as { tools?: unknown }).tools;
    if (!Array.isArray(filter)) continue;
    const unknown = filter.filter((n) => typeof n === 'string' && !declared.has(n)) as string[];
    if (unknown.length === 0) continue;
    const stepName = ((step as { name?: unknown }).name as string | undefined) ?? '<unnamed>';
    const declaredList = declared.size > 0 ? [...declared].join(', ') : '(none declared)';
    errors.push(
      `step '${stepName}': tools: references undeclared tool(s): ${unknown
        .map((n) => `'${n}'`)
...
function ruleBranchProfilesValid(workflow: Workflow): string[] {
  const errors: string[] = [];
  for (const step of collectAllSteps(workflow)) {
    const profiles = stepBranchProfiles(step as unknown as Record<string, unknown>);
    if (!profiles) continue;
    const stepName = ((step as { name?: unknown }).name as string | undefined) ?? '<unnamed>';
    if ((step as { parallel?: unknown }).parallel !== true) {
      errors.push(
        `step '${stepName}': branch_profiles is declared but the step is not parallel — ` +
    ...
    const stepTools = new Set(
      (Array.isArray((step as { tools?: unknown }).tools)
        ? ((step as { tools: unknown[] }).tools as unknown[])
        : []
      ).filter((n): n is string => typeof n === 'string'),
    );
    const structOutputs = ((step as { outputs?: unknown }).outputs ?? []) as ReadonlyArray<unknown>;
    const structBearing = structOutputs.filter(
      (o) => o !== null && typeof o === 'object' && typeof (o as { struct?: unknown }).struct === 'string',
    ).length;
    for (const [pid, profile] of Object.entries(profiles)) {
      const outside = (profile.tools ?? []).filter((t) => typeof t === 'string' && !stepTools.has(t));
      if (outside.length > 0) {
        errors.push(
          `step '${stepName}' branch_profiles.${pid}: tools ${outside
            .map((t) => `'${t}'`)
            .join(', ')} are not in the step's own tools: list (${
            stepTools.size > 0 ? [...stepTools].join(', ') : 'none declared'
          }). A profile narrows the step grant, never widens it — the step's list is the ` +
      ...
      if (typeof profile.struct === 'string' && profile.struct.length > 0 && structBearing !== 1) {
        errors.push(
          `step '${stepName}' branch_profiles.${pid}: struct override requires the step to ` +
...
function ruleParallelKeyParses(workflow: Workflow): string[] {
  const errors: string[] = [];
  for (const step of collectAllSteps(workflow)) {
    const s = step as Record<string, unknown>;
    if (s.parallel !== true || typeof s.parallel_key !== 'string') continue;
    try {
...
function ruleHumanChannelValid(workflow: Workflow): string[] {
  const errors: string[] = [];
  const wfGate = (workflow.gate ?? {}) as Record<string, unknown>;
  const wfHuman = Boolean(wfGate.human);
  const steps = collectAllSteps(workflow);

  const resolvedStepHuman = (s: Record<string, unknown>): boolean => resolvesStepHuman(wfGate, s);

  if (Object.prototype.hasOwnProperty.call(wfGate, 'human_channel')) {
    const anyHuman = wfHuman || steps.some((s) => resolvedStepHuman(s as Record<string, unknown>));
    if (!anyHuman) {
      errors.push(
        `workflow gate: 'human_channel' is set but no gate in the workflow resolves ` +
  ...
  for (const step of steps) {
    const s = step as Record<string, unknown>;
    const name = (s.name as string | undefined) ?? '<unnamed>';
    const g = (s.gate ?? {}) as Record<string, unknown>;
    if (Object.prototype.hasOwnProperty.call(g, 'human_channel') && !resolvedStepHuman(s)) {
      errors.push(
        `step '${name}' gate: 'human_channel' is set but the step's resolved human gate is ` +
    ...
    for (const [key, label] of [
      ...
      const when = ((s[key] as { when?: Record<string, unknown> } | undefined) ?? {}).when;
      if (when && Object.prototype.hasOwnProperty.call(when, 'human_channel') && when.human !== true) {
        errors.push(
          `step '${name}' ${label}: 'human_channel' is set but 'human' is not true — the ` +
...
function ruleNameMatchesDirectory(workflow: Workflow, definitionDir: string): string[] {
  const dirName = pathBasename(definitionDir);
  const wfName = (workflow as unknown as Record<string, unknown>).name;
...
function validateAuthoringConventions(
  workflow: Workflow,
  workflowName: string,
  ...
  throw new Error(
    `Workflow '${workflowName}' has an invalid configuration ` +
...
export function validateDynamicWorkflow(
  ...
  try {
    workflow = parseYamlString(rawYaml) as Workflow;
  } catch (e) {
  ...
  const errors: string[] = [
    ...
    ...rulePlanningStepTypes(
      workflow,
      restrictions.allowParallel,
      restrictions.allowDelegation,
    ),
...
export function composePlanningProcedure(
...
export function toolWorkflowValidateDynamic(
  args: WorkflowValidateDynamicInput,
  paths: EnginePaths = defaultPaths(),
): WorkflowValidateDynamicOutput {
  const parentWorkflow = args.parent_workflow as unknown as string;
  const parentStep = args.parent_step as unknown as string;
  const yaml = args.workflow_yaml;
  if (typeof parentWorkflow !== 'string' || !parentWorkflow) {
    return {
      error:
        "workflow_validate_dynamic: missing required parameter 'parent_workflow' (workflow name).",
    };
  }
  if (typeof parentStep !== 'string' || !parentStep) {
    return {
      error:
        "workflow_validate_dynamic: missing required parameter 'parent_step' (step name).",
    };
  }
  if (typeof yaml !== 'string') {
    return {
      error:
        "workflow_validate_dynamic: missing required parameter 'workflow_yaml' (raw YAML text of drafted child workflow).",
    };
  }

  let resolved: { definitionDir: string; workflow: Workflow };
  try {
  ...
  const stepConfig: Record<string, unknown> | null = findStepConfig(parentWf, parentStep);
  if (!stepConfig) {
    return {
  ...
  if ((stepConfig.type as string | undefined) !== 'planning') {
    return {
      error:
        `workflow_validate_dynamic: step '${parentStep}' is not a planning step ` +
        `(type='${(stepConfig.type as string | undefined) ?? '(none)'}'). This MCP tool ` +
  ...
  try {
    ...
    return {
      error:
        `workflow_validate_dynamic: cannot read manifest at '${manifestPath}' — ${msg}. ` +
        `Ensure workflow_init has been called for '${parentWorkflow}' and step_begin ` +
        `has been called for '${parentStep}'.`,
    };
  }
  const stepState = (manifest.steps?.[parentStep] ?? null) as Record<string, unknown> | null;
  if (!stepState || stepState.status !== 'in_progress') {
    return {
      error:
        `workflow_validate_dynamic: step '${parentStep}' is not in_progress ` +
        `(current status: '${(stepState?.status as string | undefined) ?? '(none)'}'). ` +
        `Call step_begin for '${parentStep}' before validating a draft.`,
      parent_step: parentStep,
    };
  }
  const planningState = (stepState.planning ?? null) as Record<string, unknown> | null;
  if (!planningState) {
    return {
      error:
        `workflow_validate_dynamic: step '${parentStep}' has no planning state. ` +
  ...
  const currentPhase = (planningState.phase as string | undefined) ?? 'planning';
  if (currentPhase === 'executing' || currentPhase === 'completed') {
    return {
      error:
        `workflow_validate_dynamic: step '${parentStep}' planning phase is ` +
      ...
      parent_step: parentStep,
      attempts: (planningState.attempts as number | undefined) ?? 0,
    };
  }

  const restrictions = resolvePlanningRestrictions(stepConfig as unknown as Step);
  const attemptsBefore = (planningState.attempts as number | undefined) ?? 0;
  if (attemptsBefore >= restrictions.maxPlanAttempts) {
    return {
      error:
        `workflow_validate_dynamic: step '${parentStep}' has exhausted plan-draft ` +
...
export function toolWorkflowValidate(
  args: WorkflowValidateInput,
  paths: EnginePaths = defaultPaths(),
): WorkflowValidateOutput {
  const yaml = args.workflow_yaml;
  if (typeof yaml !== 'string') {
    return {
      error:
        "workflow_validate: missing required parameter 'workflow_yaml' (raw YAML text of the workflow to validate).",
    };
  }
  ...
  const wfName = args.workflow_name as unknown as string | undefined;
  ...
  const result = fullValidateWorkflow(
    workflow,
  ...
  } as unknown as WorkflowValidateOutput;
}

function composeValidateNextSteps(workflow: Workflow, onDisk: boolean): string {
  const lines: string[] = ['VALIDATION PASSED — next steps:'];
  ...
  if (!onDisk) {
    lines.push(
      `1. Write the files: .riglane/workflows/templates/my_workflows/${name}/workflow.yaml ` +
        `(+ structs/, scripts/ as declared).`,
      `2. Re-run workflow_validate with workflow_name="${name}" — THIS draft pass could not ` +
  ...
  const tools = (wf.tools ?? []) as ReadonlyArray<Record<string, unknown>>;
  if (tools.some((t) => t.type === 'script')) {
    lines.push(
      `• This workflow declares script tools → the user MUST run \`riglane init-workflow ${name}\` ` +
  ...
  if (allSteps.some((s) => s.loop_back !== undefined)) {
    lines.push(
      '• loop_back decider scripts run from the PROJECT ROOT — the when.script file must ' +
        'exist at its full root-relative path before the first run.',
    );
  }
  if (allSteps.some((s) => typeof s.delegate_to === 'string' && s.delegate_to !== '')) {
    lines.push(
      '• The delegate_to target runs as its own top-level workflow run, started by the ' +
        'orchestrator — make sure it is installed on the runtime machine.',
    );
  }

  const requiredParams = ((wf.params ?? []) as ReadonlyArray<Record<string, unknown>>)
    .filter((p) => p.required === true && !('default' in p))
    .map((p) => `--${String(p.name)} <value>`);
  lines.push(
    `• Hand off the invocation: /riglane-run-workflow ${name}` +
      (requiredParams.length > 0 ? ` ${requiredParams.join(' ')}` : ''),
  );
  return lines.join('\n');
}

export function toolDynamicWorkflowInit(
  args: {
  ...
  const runtimeDir = dynamicRuntimeDir(
    paths.agentDir,
    args.parent_run_id,
    args.parent_step,
  );
  const childName = dynamicChildWorkflowName(
    args.parent_workflow,
    args.parent_run_id,
    args.parent_step,
  );
  return toolWorkflowInit(
    {
      name: childName as unknown as WorkflowInitInput['name'],
...
export function toolWorkflowInvokeDynamic(
  args: WorkflowInvokeDynamicInput,
  paths: EnginePaths = defaultPaths(),
): WorkflowInvokeDynamicOutput {
  const parentWorkflow = args.parent_workflow as unknown as string;
  const parentStep = args.parent_step as unknown as string;
  ...
  if ((stepConfig.type as string | undefined) !== 'planning') {
    return {
      error:
        `workflow_invoke_dynamic: step '${parentStep}' is not a planning step ` +
        `(type='${(stepConfig.type as string | undefined) ?? '(none)'}').`,
      parent_step: parentStep,
    };
  }
  ...
  try {
    parentManifest = JSON.parse(readFileSync(parentManifestPath, 'utf-8')) as typeof parentManifest;
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    return {
  ...
  const stepState = (parentManifest.steps?.[parentStep] ?? null) as Record<string, unknown> | null;
  if (!stepState || stepState.status !== 'in_progress') {
    return {
      error:
        `workflow_invoke_dynamic: parent step '${parentStep}' is not in_progress ` +
        `(status: '${(stepState?.status as string | undefined) ?? '(none)'}').`,
      parent_step: parentStep,
    };
  }
  const planningState = (stepState.planning ?? null) as Record<string, unknown> | null;
  if (!planningState) {
    return {
      error:
        `workflow_invoke_dynamic: parent step '${parentStep}' has no planning state. ` +
  ...
  if (planningState.phase !== 'validating') {
    return {
      error:
        `workflow_invoke_dynamic: parent step '${parentStep}' planning phase is ` +
        `'${(planningState.phase as string | undefined) ?? '(none)'}' but must be 'validating'. ` +
  ...
  if (!validation.ok) {
    return {
      error:
        `workflow_invoke_dynamic: defensive re-validation failed. The submitted YAML ` +
  ...
  try {
    parsedDraft = parseYamlString(yaml) as Record<string, unknown>;
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    return { error: `workflow_invoke_dynamic: unexpected parse failure post-validation: ${msg}` };
  }

  const parentRunId = (parentManifest.run_id as string | undefined) ?? '';
  if (!parentRunId) {
    return {
      error: `workflow_invoke_dynamic: parent manifest has no run_id — cannot allocate dynamic dir.`,
    };
  }
  const parentGoal = resolvePlaceholders(
    (stepConfig.goal as string | undefined) ?? '',
    (parentManifest.params ?? {}) as Record<string, unknown>,
  );
  const origin: DynamicOriginMeta = {
  ...
  let writeResult: { path: string; childName: string };
  try {
    writeResult = writeDynamicWorkflow(
      paths.agentDir,
      parentWorkflow,
      parentRunId,
      parentStep,
  ...
  const inheritParams =
    (args.inherit_params as Record<string, unknown> | undefined) ?? {};
  ...
  const childRuntimeDir = dynamicRuntimeDir(
    paths.agentDir,
    parentRunId,
    parentStep,
  );
  ...
  if (parentRunId) {
    ...
    flushTraceSnapshot(
      parentRuntimeDir, parentRunId, parentManifest, parentStep, parentWf.workflow,
    );
  }
  ...
  return {
    ...
    child_workflow_name: writeResult.childName as unknown as WorkflowInvokeDynamicSuccess['child_workflow_name'],
    first_step: childInit.first_step,
    step_names: childInit.step_names,
    ...((childInit as unknown as Record<string, unknown>).next_begin !== undefined
      ? { next_begin: (childInit as unknown as Record<string, unknown>).next_begin }
      : {}),
  };
}

export function resolveDynamicChildContext(
  parentWorkflow: string,
  parentStep: string,
  paths: EnginePaths = defaultPaths(),
): { child_workflow_path: string; child_workflow_name: string } | { error: string } {
  if (typeof parentWorkflow !== 'string' || !parentWorkflow) {
  ...
  if (!planningState) {
    return {
      error:
        `dynamic wrapper: parent step '${parentStep}' has no planning state. ` +
  ...
  if (planningState.phase !== 'executing' && planningState.phase !== 'completed') {
    return {
      error:
        `dynamic wrapper: parent step '${parentStep}' planning phase is ` +
        `'${(planningState.phase as string | undefined) ?? '(none)'}'. Expected 'executing' ` +
        `(call workflow_invoke_dynamic first) or 'completed' (re-entering finished child).`,
    };
  }
  const childWorkflowPath = (planningState.child_workflow_path as string | undefined) ?? '';
...
export function toolStepBeginDynamic(
  args: StepBeginDynamicInput,
  paths: EnginePaths = defaultPaths(),
): StepBeginDynamicOutput {
  const parentWorkflow = args.parent_workflow as unknown as string;
  const parentStep = args.parent_step as unknown as string;
  ...
  const result = toolStepBegin(
    {
      name: ctx.child_workflow_name as unknown as StepBeginInput['name'],
      step: args.step,
    },
    paths,
    { runtimeDir: childRuntimeDir, workflowYamlPath: ctx.child_workflow_path },
  );
  try {
    const parentRuntimeDir = resolveRunRuntimeDir(paths, parentWorkflow);
    const parentManifestPath = join(parentRuntimeDir, 'manifest.json');
...
export function toolStepCollectResultDynamic(
  args: StepCollectResultDynamicInput,
  paths: EnginePaths = defaultPaths(),
): StepCollectResultDynamicOutput {
  const parentWorkflow = args.parent_workflow as unknown as string;
  const ctx = resolveDynamicChildContext(
    parentWorkflow,
    args.parent_step as unknown as string,
    paths,
  );
  if ('error' in ctx) return ctx;
  const childRuntimeDir = dirname(ctx.child_workflow_path);
  const collectArgs: StepCollectResultInput = {
    name: ctx.child_workflow_name as unknown as StepCollectResultInput['name'],
  ...
  if (result.passed && !('error' in result)) {
    try {
      const parentWf = resolveWorkflow(parentWorkflow, paths);
      const parentGate = ((parentWf.workflow.gate ?? {}) as Record<string, unknown>);
      ...
      if (result.needs_semantic_gate && !result.engine_instructions) {
        let stepConfigForSpec: Record<string, unknown> = {};
        try {
          const childWf = loadYaml<Workflow>(ctx.child_workflow_path);
          for (const s of childWf.steps ?? []) {
            const sObj = s as unknown as { name?: string };
            if (sObj.name === args.step) {
              stepConfigForSpec = s as unknown as Record<string, unknown>;
              break;
            }
          }
        } catch {
      ...
      if (
        result.needs_human_gate &&
        !String(result.engine_instructions ?? '').includes('HUMAN GATE')
      ) {
        result.engine_instructions = appendHumanGateInstructions(
          result.engine_instructions,
          resolveHumanChannel(parentGate as Record<string, unknown>, {}),
        );
      }
    } catch {
    }
  ...
  return result as StepCollectResultDynamicOutput;
}

function promoteLastStepOutputs(
  childRuntimeDir: string,
  parentRunDir: string,
  childWorkflowPath: string,
): void {
  try {
    const childWf = loadYaml<Workflow>(childWorkflowPath);
    const steps = (childWf.steps ?? []) as unknown as ReadonlyArray<Record<string, unknown>>;
    if (steps.length === 0) return;
    const lastOutputs = (steps[steps.length - 1]?.outputs ?? []) as ReadonlyArray<unknown>;
    if (lastOutputs.length === 0) return;

    let childParams: Record<string, unknown> = {};
    try {
      const m = JSON.parse(readFileSync(join(childRuntimeDir, 'manifest.json'), 'utf-8')) as {
    ...
    for (const outRaw of lastOutputs) {
      const opath =
        outRaw !== null && typeof outRaw === 'object' && !Array.isArray(outRaw)
          ? ((outRaw as Record<string, unknown>).path as string | undefined) ?? ''
          : String(outRaw);
      if (!opath) continue;
      const resolved = resolvePlaceholders(opath, childParams);
      if (isDottedRoot(resolved) || pathIsAbsolute(resolved)) continue;
      ...
      for (const src of srcs) {
        ...
        try {
          ...
          logEngine(
            `output promotion: failed to promote '${rel}': ${e instanceof Error ? e.message : String(e)}`,
          );
        }
      }
    }
...
export function toolWorkflowFinalizeDynamic(
  args: WorkflowFinalizeDynamicInput,
  paths: EnginePaths = defaultPaths(),
): WorkflowFinalizeDynamicOutput {
  const parentWorkflow = args.parent_workflow as unknown as string;
  const parentStep = args.parent_step as unknown as string;
  ...
  const childResult = toolWorkflowFinalize(
    {
      name: ctx.child_workflow_name as unknown as WorkflowFinalizeInput['name'],
    },
    paths,
    { runtimeDir: childRuntimeDir, workflowYamlPath: ctx.child_workflow_path },
  );
  ...
  const newPhase: 'completed' | 'failed' =
  ...
  const parentStepState = parentManifest.steps?.[parentStep] as Record<string, unknown> | undefined;
  if (parentStepState && parentStepState.planning) {
    const planning = parentStepState.planning as Record<string, unknown>;
...
export function toolWorkflowReplanDynamic(
  args: { parent_workflow: string; parent_step: string },
  paths: EnginePaths = defaultPaths(),
  ...
  try {
    parentManifest = JSON.parse(
      readFileSync(parentManifestPath, 'utf-8'),
    ) as typeof parentManifest;
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    return { error: `workflow_replan_dynamic: cannot read parent manifest — ${msg}` };
  }

  const stepState = parentManifest.steps?.[parentStep] as Record<string, unknown> | undefined;
  if (!stepState) {
    return { error: `workflow_replan_dynamic: step '${parentStep}' not found in manifest.` };
  }
  const planning = stepState.planning as Record<string, unknown> | undefined;
  if (!planning) {
    return { error: `workflow_replan_dynamic: step '${parentStep}' has no planning state.` };
  }
  const currentPhase = (planning.phase as string | undefined) ?? '';
  if (currentPhase !== 'completed' && currentPhase !== 'failed') {
    return {
      error:
        `workflow_replan_dynamic: planning phase is '${currentPhase}' but must be ` +
        `'completed' or 'failed'. Replan is only allowed after workflow_finalize_dynamic.`,
    };
  }

  const previousRuns = (planning.previous_child_runs ?? []) as string[];
  const prevChildRunId = planning.child_run_id as string | null | undefined;
  if (typeof prevChildRunId === 'string' && prevChildRunId.length > 0) {
    previousRuns.push(prevChildRunId);
  }

  const currentAttempts = (planning.attempts as number | undefined) ?? 0;
...
export function toolAgentNotesWrite(
  args: AgentNotesWriteInput,
  paths: EnginePaths = defaultPaths(),
): AgentNotesWriteOutput {
  const stepTemplate = args.step_template as unknown as string;
  const topic = args.topic as unknown as string;
  const runId = args.run_id as unknown as string;
  const body = args.body as unknown as string;
  ...
  const status = args.status as unknown as string;
  const confidence = args.confidence as unknown as string;
  const STATUSES: ReadonlyArray<string> = ['success', 'partial', 'failed', 'experimental'];
  const CONFIDENCES: ReadonlyArray<string> = ['high', 'medium', 'low'];
  if (!STATUSES.includes(status)) {
    return {
      error:
        `agent_notes_write: invalid 'status' value '${status ?? '(none)'}'. ` +
  ...
  if (!CONFIDENCES.includes(confidence)) {
    return {
      error:
        `agent_notes_write: invalid 'confidence' value '${confidence ?? '(none)'}'. ` +
  ...
  const tags = (args.tags ?? []) as readonly string[];
  const relatedRuns = (args.related_runs ?? []) as readonly string[];
  const fm: NoteFrontmatter = {
    topic,
    step_template: stepTemplate,
    status: status as NoteFrontmatter['status'],
    confidence: confidence as NoteFrontmatter['confidence'],
    project,
    run_id: runId,
  ...
  try {
    ...
    return {
      error:
        `agent_notes_write: note file written at '${filePath}' but index update failed — ${msg}. ` +
...
export function toolAgentNotesSearch(
  args: AgentNotesSearchInput,
  paths: EnginePaths = defaultPaths(),
): AgentNotesSearchOutput {
  const stepTemplate = args.step_template as unknown as string;
  if (typeof stepTemplate !== 'string' || !stepTemplate) {
  ...
  const statusFilter = (args.status ?? ['success', 'partial']) as ReadonlyArray<string>;
  const confidenceFilter = (args.confidence ?? ['high', 'medium']) as ReadonlyArray<string>;
  ...
  const filtered = index.entries.filter((e) => {
    ...
    if (!confidenceFilter.includes(e.confidence)) return false;
  ...
  const matches: AgentNotesSummary[] = clamped.map((e) => ({
    file: e.file,
...
function inboxWebhookUrlSafe(): string {
  try {
...
import {
  composeElicitation,
  composeTerminalPresentation,
  expectsAnswer,
  findStepMessages,
  flushPendingDeliveries,
  mapElicitationContent,
  messageState,
  postMessage as inboxPostMessage,
  readMessage as inboxReadMessage,
  respondMessage as inboxRespondMessage,
  type InboxKind,
  type InboxMessage,
...
const POST_ADVISORY_KEYS = ['channel_warning', 'delivery_warning'] as const;

function postAdvisories(posted: Record<string, unknown>): Record<string, unknown> {
  const out: Record<string, unknown> = {};
  for (const k of POST_ADVISORY_KEYS) {
    if (posted[k] !== undefined) out[k] = posted[k];
  }
  return out;
}

function supersededRefusal(
  messageId: string,
  ...
  return {
    error:
      `Message '${messageId}' was superseded by '${live}': its step asked again (the run ` +
      `continued after a stop), so this question can no longer be answered and waiting on it ` +
      `would never end.`,
    message_id: messageId,
    superseded_by: replacement ?? null,
    engine_instructions:
      `This is NOT a failure of the step and NOT a reason to clear or finalize the run. Work ` +
...
function inboxPageUrl(): string | null {
  const base = currentLocalServerBase();
  return base === null ? null : withServeToken(`${base}/tools/workflow-studio.html`);
}

function inboxDeliveryOpts(manifest: Record<string, unknown> | null): {
  webhookUrl: string | null;
  respondUrl: string | null;
} {
  const runWebhook = (manifest as { inbox_webhook_url?: unknown } | null)?.inbox_webhook_url;
  const webhookUrl =
    typeof runWebhook === 'string' && runWebhook ? runWebhook : inboxWebhookUrlSafe();
  const base = currentLocalServerBase();
  return {
...
function loadManifest(runtimeDir: string): Record<string, unknown> | null {
  try {
    return JSON.parse(readFileSync(join(runtimeDir, 'manifest.json'), 'utf-8')) as Record<
      string,
      unknown
    >;
  } catch {
...
function composeInboxMessageRules(stepName: string): string {
  return instruction('engine/inbox-message-rules', { stepName });
}

export function toolInboxRules(
  args: InboxRulesInput,
  paths: EnginePaths = defaultPaths(),
): Record<string, unknown> {
  const workflowName = (args.name ?? args.workflow_name) as string | undefined;
  ...
  if (!manifest) return { error: `No manifest for workflow '${workflowName}'` };
  const own = checkRunOwnership(manifest as unknown as Record<string, unknown>, 'inbox_rules');
  ...
  const stepState = (manifest.steps as Record<string, Record<string, unknown>>)[stepName] ?? {};
  stepState.inbox_rules_at = nowIsoLocal();
  (manifest.steps as Record<string, Record<string, unknown>>)[stepName] = stepState;
  manifest.updated_at = nowIsoLocal();
...
function resolvePostedMessageChannel(
  workflowName: string,
  stepName: string,
  kind: string,
  paths: EnginePaths,
  ...
  try {
    ...
    const stepCfg = (findStepConfig(workflow, stepName) ?? {}) as Record<string, unknown>;
    if (kind === 'loop_decision') {
      const when = ((stepCfg.loop_back as Record<string, unknown> | undefined)?.when ?? {}) as {
    ...
    if (kind === 'route_decision') {
      const when = ((stepCfg.routes as Record<string, unknown> | undefined)?.when ?? {}) as {
        human_channel?: string;
      };
      return when.human_channel === 'external' || when.human_channel === 'both'
        ? when.human_channel
    ...
    return resolveHumanChannel(
      (workflow.gate ?? {}) as Record<string, unknown>,
      ((stepCfg.gate as Record<string, unknown> | undefined) ?? {}) as Record<string, unknown>,
    );
  } catch {
    return 'terminal';
  }
...
function composeVerifiedContext(
  workflowName: string,
  stepName: string,
  runtimeDir: string,
  params: Record<string, unknown>,
  ...
  try {
    ...
    const entries: VerifiedOutput[] = [];
    for (const raw of outputs as Array<Record<string, unknown> | string>) {
      const rawPath =
        typeof raw === 'string' ? raw : String((raw as { path?: unknown }).path ?? '');
      if (!rawPath) continue;
      ...
      for (const abs of resolved) {
        ...
        const clipped =
          typeof preview.value_preview === 'string' &&
...
export async function toolInboxPost(
  args: InboxPostInput,
  paths: EnginePaths = defaultPaths(),
  ...
  const manifest = loadManifest(runtimeDir);
  if (!manifest) return { error: `No manifest for workflow '${workflowName}'` };
  const own = checkRunOwnership(manifest as unknown as Record<string, unknown>, 'inbox_post');
  if (own) return own as Record<string, unknown>;
  if (!(manifest.steps && Object.prototype.hasOwnProperty.call(manifest.steps, stepName))) {
    return { error: `Step '${stepName}' is not part of this run` };
  }
  {
    const stepState =
      ((manifest.steps as Record<string, Record<string, unknown>>)[stepName] ?? {}) as {
        inbox_rules_at?: string;
        started_at?: string;
      };
    const rulesAt = stepState.inbox_rules_at;
    const startedAt = stepState.started_at;
    const rulesFresh =
      typeof rulesAt === 'string' &&
      rulesAt.length > 0 &&
      (!startedAt || Date.parse(rulesAt) >= Date.parse(startedAt));
    if (!rulesFresh) {
      return {
        error:
          `The inbox message rules were not requested for this pass of step '${stepName}'. ` +
  ...
  const runWebhook = (manifest as { inbox_webhook_url?: unknown }).inbox_webhook_url;
  const webhookUrl =
    typeof runWebhook === 'string' && runWebhook ? runWebhook : inboxWebhookUrlSafe();
  ...
  if (channel !== 'terminal' || webhookUrl) {
    try {
      ...
      if (base) respondUrl = `${base}/api/inbox/respond`;
    } catch {
      respondUrl = null;
    }
  }
  ...
  if (channel === 'external' && respondUrl === null && awaitsAnswer) {
    return {
      error:
        `Cannot post this message: the step's human gate is answered OUTSIDE the terminal ` +
  ...
  const verifiedContext =
    messageKind === 'human_gate'
      ? composeVerifiedContext(
          workflowName,
          stepName,
          runtimeDir,
      ...
      : null;
  const posted = await inboxPostMessage(
    runtimeDir,
    {
      run_id: (manifest.run_id as string | undefined) ?? '',
      workflow: workflowName,
      step: stepName,
    },
    args.message as InboxMessageContent,
    {
      respondUrl,
      webhookUrl: webhookUrl || null,
      stepStartedAt:
        ((manifest.steps as Record<string, { started_at?: string }> | undefined)?.[stepName]
          ?.started_at) ?? null,
      verifiedContext,
    },
  );
  if ('errors' in posted) {
    return {
      error:
        `Message content rejected by the inbox schema:\n- ${posted.errors.join('\n- ')}\n` +
  ...
  return {
    ...
    ...(posted.message.delivery !== undefined && posted.message.delivery.delivered_at === undefined
      ? {
          delivery_warning:
            `The webhook consumer at ${posted.message.delivery.url} did not accept the ` +
      ...
      : {}),
    engine_instructions:
      `Message '${posted.message.message_id}' is stored and visible to the user (Agent ` +
...
export async function toolInboxCheck(
  args: InboxCheckInput,
  paths: EnginePaths = defaultPaths(),
): Promise<Record<string, unknown>> {
  const workflowName = (args.name ?? args.workflow_name) as string | undefined;
  ...
  for (;;) {
    ...
    if (!msg) return { error: `Unknown message '${messageId}'` };
    ...
    if (messageState(msg) === 'answered') {
      return {
        ...
        engine_instructions:
          'The question is answered — never relay or re-ask it now (a relayed ' +
    ...
    if (Date.now() >= deadline) {
      return {
        ...
        message_id: messageId,
        engine_instructions:
          "No response yet. Wait (do not busy-loop), then call inbox(op:'check') again — the run " +
...
export interface InboxRespondInput {
  ...
  readonly items?: Record<string, { type: string; text?: string; args?: Record<string, unknown> }>;
}

export async function toolInboxRespond(
  args: InboxRespondInput,
  paths: EnginePaths = defaultPaths(),
): Promise<Record<string, unknown>> {
  const workflowName = (args.name ?? args.workflow_name) as string | undefined;
  if (!workflowName || typeof workflowName !== 'string') {
  ...
  const manifest = loadManifest(runtimeDir);
  if (!manifest) return { error: `No manifest for workflow '${workflowName}'` };
  const own = checkRunOwnership(manifest as unknown as Record<string, unknown>, 'inbox_respond');
  if (own) return own as Record<string, unknown>;
  const result = await inboxRespondMessage(
    runtimeDir,
    args.message_id,
    {
      type: args.type,
  ...
  return {
    ...
    engine_instructions:
      'Recorded. Act on the answer IMMEDIATELY — do the step work and call ' +
...
function resolveMessageKind(message: unknown): InboxKind {
  const declared = (message as { kind?: unknown } | null)?.kind;
  return declared === 'loop_decision' || declared === 'route_decision' || declared === 'info'
    ? declared
    : 'human_gate';
}

function openPassQuestion(
  runtimeDir: string,
  step: string,
  kind: InboxKind,
): InboxMessage | null {
  if (!expectsAnswer(kind)) return null;
  const manifest = loadManifest(runtimeDir);
  const startedAt = Date.parse(
    (manifest?.steps as Record<string, { started_at?: string }> | undefined)?.[step]?.started_at ??
      '',
  );
...
export async function toolInboxAsk(
  args: InboxInput,
  paths: EnginePaths = defaultPaths(),
): Promise<Record<string, unknown>> {
  const workflowName = (args.name ?? args.workflow_name) as string | undefined;
  ...
  let postedPresentation: string | null = null;
  let advisories: Record<string, unknown> = {};
  if (typeof args.message_id === 'string' && args.message_id.length > 0) {
    messageId = args.message_id;
  } else {
    const adopted =
      typeof args.step === 'string' && args.step.length > 0
        ? openPassQuestion(runtimeDir, args.step, resolveMessageKind(args.message))
        : null;
    if (adopted !== null) {
      messageId = adopted.message_id;
      advisories = {
        reused_open_question:
          `Step '${adopted.step}' ALREADY has an unanswered question from this pass — ` +
      ...
      const posted = await toolInboxPost(args as InboxPostInput, paths);
      if (typeof posted.error === 'string') return posted;
      messageId = posted.message_id as string;
      postedPresentation = (posted.terminal_presentation as string | undefined) ?? null;
      advisories = postAdvisories(posted);
    }
  }
  const withContext = (body: Record<string, unknown>): Record<string, unknown> => {
  ...
  let relayPresentation: string | null = null;
  if (channel === 'both' && !canElicit) {
    const externalAlive = await (async (): Promise<boolean> => {
      try {
    ...
    if (!externalAlive) {
      ...
      return withContext({
        ...
        engine_instructions:
          `This host does not render engine dialogs (no elicitation capability), so the ` +
    ...
    emitCallProgress(
      `HUMAN QUESTION (answer on the inbox page or via the API):\n${relayPresentation}`,
      0,
    );
  }
  ...
  const elicitBox: { req: OutgoingRequest | null } = { req: null };
  let holdOver = false;
  if (canElicit && bridge) {
    void (async (): Promise<void> => {
      let refusal = '';
      for (let attempt = 0; attempt < 3 && !holdOver; attempt++) {
        const form = composeElicitation(msg as InboxMessage);
        ...
        elicitBox.req = null;
        const res = raw as { action?: string; content?: Record<string, unknown> } | null;
        if (res?.action !== 'accept') return;
        const mapped = mapElicitationContent(msg as InboxMessage, res.content ?? {});
        if ('error' in mapped) {
          refusal = `Your previous submission was not usable: ${mapped.error}`;
          continue;
        }
        await inboxRespondMessage(
          runtimeDir,
          messageId,
          mapped.response,
          'terminal',
          inboxDeliveryOpts(loadManifest(runtimeDir)),
        );
        return;
      }
    })();
  ...
  try {
    for (;;) {
      ...
      if (signal?.aborted) return withContext({ status: 'cancelled', message_id: messageId });
      ...
      if (elapsed >= holdMs) {
        return withContext({
          ...
          engine_instructions:
            `Still unanswered after ${Math.round(elapsed / 1000)}s of holding. Call ` +
      ...
      if (elapsed - lastProgressAt >= progressEveryMs) {
        ...
        emitCallProgress(
          `Waiting for the human answer to '${msg.title}' (${Math.round(elapsed / 1000)}s)`,
...
function inboxAskMaxHoldMsSafe(): number {
  try {
...
export async function toolInbox(
  args: InboxInput,
  paths: EnginePaths = defaultPaths(),
): Promise<Record<string, unknown>> {
  switch (args.op) {
    ...
    default:
      return {
        error:
          `inbox requires 'op' — one of: rules (fetch the message rules; unlocks posting), ` +
...
export function toolStepCompleteDynamic(
  args: StepCompleteDynamicInput,
  paths: EnginePaths = defaultPaths(),
): StepCompleteDynamicOutput {
  const parentWorkflow = args.parent_workflow as unknown as string;
  const parentStep = args.parent_step as unknown as string;
  ...
  const result = toolStepComplete(
    {
      name: ctx.child_workflow_name as unknown as StepCompleteInput['name'],
      step: args.step,
...
export function resolveWorkflow(
  name: string,
  paths: EnginePaths = defaultPaths(),
): { definitionDir: string; workflow: Workflow } {
  ...
  for (const path of searchPaths) {
    ...
    const workflow = loadYaml<Workflow>(path);
    validateAuthoringConventions(workflow, name, definitionDir);
    normalizePerIterationOutputs(workflow as unknown as Record<string, unknown>);
    injectSpecAuthoringOutputs(workflow as unknown as Record<string, unknown>);
    workflowCache.set(name, { definitionDir, workflow, mtime: st.mtimeMs });
    return { definitionDir, workflow };
  }

  const _err = new Error(
    `Workflow '${name}' not found. Searched: ${searchPaths.join(', ')}`,
...
import {
  ...
  assertBranchPathsUnique,
  resolveBranchPath,
} from './branch-paths.js';
import { OutputPathError, resolveConcreteOutputPath } from './output-paths.js';

function resetSpawnThrottleState(runtimeDir: string, stepName: string): void {
...
export function isPurePlaceholderSegment(segment: string): boolean {
  return (
    segment.startsWith('{') &&
    segment.endsWith('}') &&
...
export function readJsonField(data: unknown, fieldPath: string): unknown {
...
export function parseParallelKey(parallelKey: string): ParsedParallelKey {
  const bare = parallelKey.endsWith('[*]') ? parallelKey.slice(0, -3) : parallelKey;
  const m = /^([^.]+)\.(.+?)(?:\[(\w+)=(\w+)\])?$/.exec(bare);
  const guidance =
    `Expected '<struct>.<field.path>' with an optional terminal '[*]' (all items) ` +
  ...
  if (/[[\]]/.test(structName) || /[[\]]/.test(fieldPath)) {
    throw new Error(
      `Cannot parse parallel_key '${parallelKey}': brackets are reserved for a terminal ` +
...
export function resolveParallelBranches(
  parallelKey: string,
  runtimeDir: string,
  workflow?: { steps?: ReadonlyArray<Record<string, unknown>> },
  params?: Record<string, unknown>,
): unknown[] {
  ...
  try {
    ...
    throw new Error(
      `Parallel key references struct '${structName}' but data file not found: ${dataFile}`,
    );
  }
  ...
  if (filterKey !== undefined && filterVal !== undefined) {
    return items.filter(
      (it): boolean =>
        ...
        typeof it === 'object' &&
        !Array.isArray(it) &&
        String((it as Record<string, unknown>)[filterKey]) === filterVal,
    );
  }
  return items;
}
...
export function normalizeForMcp(name: string | null | undefined): string {
...
function ccMcpName(server: string, registered: string): string {
  return `mcp__${normalizeForMcp(server)}__${registered}`;
}

function platformPythonNames(): { py: string; otherPy: string } {
  const py = process.platform === 'win32' ? 'python' : 'python3';
  return { py, otherPy: py === 'python' ? 'python3' : 'python' };
}

function scriptInvokeHint(host: Host | null, registered: string, command: string): string {
...
export function renderToolDocsBrief(
  activeTools: readonly ActiveToolDescriptor[],
  workflowName: string,
  host: Host | null = null,
): string {
  if (activeTools.length === 0) return '';
  const isOc = host === 'opencode';
  const lines: string[] = ['**Available tools for this step:**'];
  ...
  lines.push(
    'Prefer these MCP tools over running the underlying scripts via Bash — they provide structured tracing and schema validation.',
  );
  lines.push(
    'If a listed tool is not callable yet it is DEFERRED (load its schema, e.g. `ToolSearch`), NOT missing — do not fall back to Bash for a deferred tool.',
  );
  return lines.join('\n');
}

export function renderToolDocsFull(
  activeTools: readonly ActiveToolDescriptor[],
  workflowName: string,
  host: Host | null = null,
): string {
  if (activeTools.length === 0) return '';
  const sections: string[] = ['## Available tools for this step', ''];
  for (const t of activeTools) {
    const name = t.name ?? '';
    const desc = t.description ?? '';
    const ttype = t.type ?? 'script';
    if (ttype === 'script') {
      ...
      sections.push('');
      sections.push(
        `**Fallback command** (shell — use ONLY if the MCP tool is genuinely unavailable, per the note above): \`${cmd}\``,
      );
      ...
      if (expected.length > 0) {
        sections.push('');
        sections.push(
          `**Tools — find each under the \`${name}\` MCP server and call it by the name your host shows:**`,
        );
      ...
      sections.push('');
      sections.push(
        `These are MCP tools (not scripts — do not shell). Ensure the \`${name}\` server is connected; call each by the EXACT name your host shows for that server (do NOT assume a prefix format).`,
      );
      sections.push('');
    }
  }
...
export function checkToolsAvailability(
  ...
  for (const tool of toolsConfig) {
    ...
    if (toolType === 'script') {
      ...
      if (hasConfig) entry.server_config = tool.server_config;
...
const _mcpCallLog: McpCallLogEntry[] = [];

export function mcpCallLog(): readonly McpCallLogEntry[] {
  return _mcpCallLog;
}

export function appendMcpCall(entry: McpCallLogEntry): void {
  _mcpCallLog.push(entry);
}

export function clearMcpCallLog(): void {
  _mcpCallLog.length = 0;
}


export function toolWorkflowResolve(
  args: WorkflowResolveInput,
  paths: EnginePaths = defaultPaths(),
): WorkflowResolveOutput {
...

  {
    ...
    if (activeWorkflow && activeWorkflow !== resolvedName) {
      const hasScriptTools = (workflow.tools ?? []).some(
        (t) => (t.type ?? 'script') === 'script',
      );
      if (hasScriptTools) {
        throw new Error(
          `Active workflow is '${activeWorkflow}', but you are resolving ` +
  ...
  const stepsInfo: WorkflowResolveStepEntry[] = [];
  for (const s of workflow.steps ?? []) {
    const info: Record<string, unknown> = { name: s.name };
    if ('delegate_to' in s) info.delegate_to = s.delegate_to;
    if (s.parallel) info.parallel = true;
    if (s.gate) info.gate_override = s.gate;
    if (s.model) info.model = s.model;
    stepsInfo.push(info as unknown as WorkflowResolveStepEntry);
  }

  const toolsConfig = workflow.tools ?? [];
  const toolsInfo: Record<string, unknown>[] = [];
  for (const t of toolsConfig) {
    ...
    if ((t as { description?: string }).description) {
      ti.description = (t as { description?: string }).description;
    }
    const _requiredVal = (t as { required?: unknown }).required;
    if (_requiredVal != null) {
      ti.required = _requiredVal as boolean;
    }
    toolsInfo.push(ti);
  }
  ...
  if (toolsInfo.length > 0) result.tools = toolsInfo;
  return result as unknown as WorkflowResolveOutput;
}


function uuid4Full(): string {
  return randomUUID();
}

function resolveRunRuntimeDir(paths: EnginePaths, workflowName: string): string {
  let rid = getCurrentRunId();
  if (rid) {
    try {
      const held = JSON.parse(
        readFileSync(join(runDir(paths.agentDir, rid), 'manifest.json'), 'utf-8'),
      ) as { status?: string; workflow?: string };
      if (held.workflow !== workflowName || held.status !== 'in_progress') rid = null;
    } catch {
      rid = null;
    }
  }
  if (!rid) {
    ...
    if (typeof envRid === 'string' && isValidRunId(envRid)) {
      try {
        const m = JSON.parse(
          readFileSync(join(runDir(paths.agentDir, envRid), 'manifest.json'), 'utf-8'),
        ) as { status?: string; workflow?: string };
  ...
  if (!rid) {
    ...
    if (inProgress.length === 1) {
      ...
      throw new Error(
        `Multiple in-progress runs of workflow '${workflowName}' — ambiguous. ` +
  ...
  if (!rid) {
    throw new Error(
      `No active run for workflow '${workflowName}'. Call workflow_init first ` +
...
let _pendingDelegation: PendingDelegation | null = null;

export function _resetPendingDelegation(): void {
  _pendingDelegation = null;
}

function resolveDelegatedRunDir(
  paths: EnginePaths,
  ...
  | { dir: string; source: 'linkage' | 'backlink' }
  | { dir: null; source: 'unresolved'; candidates: number } {
  ...
  if (parentRunId) {
    for (let i = runs.length - 1; i >= 0; i -= 1) {
      const rid = runs[i] as string;
      try {
        const m = JSON.parse(
          readFileSync(join(runDir(paths.agentDir, rid), 'manifest.json'), 'utf-8'),
        ) as { parent_run_id?: string; parent_step?: string };
...
function checkRunOwnership(
  manifest: Record<string, unknown>,
  tool: string,
): Record<string, unknown> | null {
  const owner = manifest.owner_instance_id as string | undefined;
  if (!owner || owner === SERVER_INSTANCE_ID) return null;
  return {
    error:
      `Cannot call '${tool}': this workflow run is orchestrated by a different session — ` +
...
function deciderErrorDirective(kind: 'loop' | 'route'): string {
  return '\n\n' + instruction('engine/decider-error-directive', { kind });
}

function composeModelInstruction(mode: unknown): string | null {
...
function composeStepBeginEngineInstructions(
  useSubagent: boolean,
  isRebegin: boolean,
  stepName: string,
  model?: unknown,
  ...
  const notes: string[] = [];
  if (useSubagent && getEngineHost() === 'codex') notes.push(CODEX_SPAWN_LIFECYCLE_NOTE);
  if (useSubagent && getEngineHost() === 'cursor') notes.push(CURSOR_SPAWN_TYPE_NOTE);
  if (useSubagent && getEngineHost() === 'gemini') notes.push(GEMINI_SPAWN_NOTE);
  if (useSubagent) {
  ...
  if (isRebegin) {
    notes.push(
      `NOTE — step '${stepName}' was already in progress: this step_begin is a RE-begin. ` +
...
function nowIsoLocal(d: Date = new Date()): string {
  return toIsoLocal(d);
}

function normalizeRunOverrides(overrides?: EngineRunOverrides): {
  runtimeDir: string | null;
  workflowYamlPath: string | null;
} {
...
export function toolWorkflowInit(
  args: WorkflowInitInput,
  paths: EnginePaths = defaultPaths(),
  overrides?: EngineRunOverrides,
): WorkflowInitOutput {
  const name =
    (args.name as unknown as string) ||
    ((args as unknown as Record<string, unknown>).workflow_name as string);
  let userParams: Record<string, unknown> = {};
  const rawParams = args.params;
  if (rawParams !== undefined && rawParams !== null) {
    if (typeof rawParams === 'string') {
      try {
        userParams = JSON.parse(rawParams) as Record<string, unknown>;
      } catch {
        userParams = {};
      }
    } else if (typeof rawParams === 'object' && !Array.isArray(rawParams)) {
      userParams = { ...(rawParams as Record<string, unknown>) };
    }
  }

  const rawModelOverride =
    (args as { model_override?: unknown }).model_override ?? process.env[ENV_MODEL_OVERRIDE];
  let modelOverride: ModelMode | undefined;
  if (rawModelOverride !== undefined && rawModelOverride !== null && rawModelOverride !== '') {
    if (!isModelMode(rawModelOverride)) {
      return {
        error:
          `Invalid model override '${String(rawModelOverride)}'. ` +
      ...
      } as unknown as WorkflowInitOutput;
    }
    modelOverride = rawModelOverride;
  }

  const rawInboxWebhook =
    (args as { inbox_webhook?: unknown }).inbox_webhook ??
    process.env[ENV_INBOX_WEBHOOK_OVERRIDE];
  let inboxWebhookOverride: string | undefined;
  if (rawInboxWebhook !== undefined && rawInboxWebhook !== null && rawInboxWebhook !== '') {
    if (typeof rawInboxWebhook !== 'string' || !/^https?:\/\//.test(rawInboxWebhook)) {
      return {
        error:
          `Invalid inbox webhook '${String(rawInboxWebhook)}' — it must be an http(s) URL ` +
  ...
  const rawTraceViewer =
    (args as { trace_viewer?: unknown }).trace_viewer ?? process.env[ENV_TRACE_VIEWER_OVERRIDE];
  let traceViewerOff = false;
  if (rawTraceViewer !== undefined && rawTraceViewer !== null && rawTraceViewer !== '') {
    if (rawTraceViewer !== 'off') {
      return {
        error:
          `Invalid trace viewer override '${String(rawTraceViewer)}' — the only ` +
  ...
  const { runtimeDir: runtimeDirOverride, workflowYamlPath: yamlPathOverride } =
    normalizeRunOverrides(overrides);
  let workflow: Workflow;
  let defnDir: string;
  if (yamlPathOverride !== null) {
    try {
      workflow = loadYaml<Workflow>(yamlPathOverride);
      normalizePerIterationOutputs(workflow as unknown as Record<string, unknown>);
      injectSpecAuthoringOutputs(workflow as unknown as Record<string, unknown>);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
  ...
  if (existingExists) {
    try {
      const prev = JSON.parse(readFileSync(existingManifestPath, 'utf-8')) as Manifest;
  ...
  for (const pdef of paramDefs) {
    const pname = pdef.name as unknown as string;
    if (pname in userParams && userParams[pname] !== null && userParams[pname] !== undefined) {
      ...
      resolvedParams[pname] = (pdef as { default: unknown }).default;
  ...
  if (hasLoopBack) resolvedParams.iteration = 0;

  const scopeParamRaw = (resolvedParams as Record<string, unknown>).scope;
  const scopeParam: string | null =
    typeof scopeParamRaw === 'string' && scopeParamRaw.length > 0 ? scopeParamRaw : null;
  const scopeManaged = scopeParam !== null;
  let preservedActiveScope: string | null = null;
  ...
  if (scopeParam !== null) {
    try {
      ...
      return {
        error:
          `Cannot manage workflow scope '${scopeParam}': ${msg}. ` +
  ...
  const stepNames = (workflow.steps ?? []).map((s) => s.name as unknown as string);
  const stepsDict: Record<string, { status: 'pending' }> = {};
  for (const sname of stepNames) stepsDict[sname] = { status: 'pending' };
  ...
  if (modelOverride) manifest.model_override = modelOverride;
  manifest.step_tools = freezeStepTools(
    workflow as unknown as ToolDeclaringWorkflow,
    collectAllSteps(workflow),
  );
  {
    const wfWebhook = (workflow as { inbox_webhook?: unknown }).inbox_webhook;
    const snapshotWebhook =
      inboxWebhookOverride ?? (typeof wfWebhook === 'string' && wfWebhook ? wfWebhook : undefined);
    if (snapshotWebhook) manifest.inbox_webhook_url = snapshotWebhook;
  }
  ...
  if (!hasRuntimeDirOverride && _pendingDelegation && _pendingDelegation.target === name) {
    try {
      ...
      const pSteps = (parentManifest.steps ?? {}) as Record<string, Record<string, unknown>>;
      const pStep = (pSteps[_pendingDelegation.parentStep] ?? {}) as Record<string, unknown>;
      ...
      safeWriteJson(parentManifestPath, parentManifest);
      manifest.parent_run_id = (parentManifest.run_id as string | undefined) ?? null;
      manifest.parent_step = _pendingDelegation.parentStep;
      safeWriteJson(existingManifestPath, manifest);
      logEngine(
        `delegation linkage: linked child run '${runId}' to parent step ` +
          `'${_pendingDelegation.parentStep}' (${parentManifest.run_id as string})`,
      );
    } catch (e) {
      logEngine(
        `delegation linkage: stamp failed (falling back to latest-by-name at ` +
  ...
  if (scopeParam !== null) {
    try {
      ...
      return {
        error:
          `Failed to write workflow scope '${scopeParam}' to ` +
  ...
  const trace: Record<string, unknown> = {
    ...
    steps: (workflow.steps ?? []).map((s, i) => {
      const su = s as unknown as Record<string, unknown>;
      const entry: Record<string, unknown> = {
        name: su.name ?? `step-${i}`,
        index: i,
        status: 'pending',
      ...
      if ('delegate_to' in su) entry.delegate_to = su.delegate_to;
      return entry;
    }),
  };

  if (workflow.description) trace.description = workflow.description;
  if (workflow.context) trace.context = workflow.context;
  ...
  const toolsConfig = (workflow.tools ?? []) as unknown as readonly ToolConfig[];
  ...
  if (toolsConfig.length > 0) {
    ...
    if (requiredMissing.length > 0) {
      ...
      toolsResult.block_message = [
        ...
        ...requiredMissing.map(
          (m) =>
            `  - ${m.name as string} (${m.type as string}): ${(m.reason as string) ?? 'not available'}`,
        ),
      ].join('\n');
    }
  }
  ...
  if (toolsResult) result.tools = toolsResult;
  ...
  if (scopeWarning) result.scope_warning = scopeWarning;
  if (modelOverride) result.model_override = modelOverride;
  if (traceViewerOff) result.trace_viewer = 'off';
  if (manifest.inbox_webhook_url) result.inbox_webhook = manifest.inbox_webhook_url;
  ...
  if (!toolsResult?.blocked && (stepNames[0] ?? null) !== null) {
    const firstBegin = composeNextBeginForAdvance({
      ...
      nextStep: stepNames[0] as string,
      paths,
    });
    if (firstBegin !== null) {
      result.engine_instructions =
        `next_begin below carries the FULL begin payload for the first step ` +
        `('${stepNames[0] as string}') — do NOT call step_begin; compose the subagent task ` +
  ...
  return result as unknown as WorkflowInitOutput;
}
...
function readRunHead(
  paths: EnginePaths,
  rid: string,
): { status?: string; workflow?: string; stopped?: unknown } | null {
  try {
    return JSON.parse(
      readFileSync(join(runDir(paths.agentDir, rid), 'manifest.json'), 'utf-8'),
    ) as { status?: string; workflow?: string; stopped?: unknown };
  } catch {
    return null;
  }
}

function isResumable(m: { status?: string; stopped?: unknown } | null): boolean {
...
function resolveResumeTarget(
...
): ResumeTarget | { error: string } {
  if (typeof requested === 'string' && requested.length > 0) {
    if (!isValidRunId(requested)) {
      return {
        error:
          `Invalid run_id '${requested}'. A run id looks like ` +
    ...
    if (m === null) {
      return {
        error:
          `No run '${requested}' in this project (its directory is missing or its ` +
    ...
    if (m.workflow !== name) {
      return {
        error:
          `Run '${requested}' belongs to workflow '${String(m.workflow)}', not '${name}'. ` +
    ...
    if (!isResumable(m)) {
      return {
        error:
          `Run '${requested}' is not resumable (status '${String(m.status)}'` +
  ...
  let latestTerminalStatus: string | null = null;
  for (let i = runs.length - 1; i >= 0; i -= 1) {
    const rid = runs[i] as string;
    const m = readRunHead(paths, rid);
    if (isResumable(m)) return { runId: rid, runtimeDir: runDir(paths.agentDir, rid) };
    if (m !== null) latestTerminalStatus = latestTerminalStatus ?? (m.status || 'unknown');
  }
  if (latestTerminalStatus !== null) {
    return {
      error:
        `No resumable run for '${name}': the latest run is terminal ` +
...
export function toolWorkflowResume(
  args: WorkflowResumeInput,
  paths: EnginePaths = defaultPaths(),
): WorkflowResumeOutput {
  const name = args.name;
  const _picked = resolveResumeTarget(paths, name, args.run_id);
  if ('error' in _picked) return _picked;
  ...
  {
    const _ownerId = (manifest as unknown as Record<string, unknown>).owner_instance_id;
    const _ownerPid = (manifest as unknown as Record<string, unknown>).owner_pid;
    if (
      typeof _ownerId === 'string' &&
      _ownerId.length > 0 &&
      ...
      return {
        error:
          `Cannot resume workflow '${name}': this run is owned by another LIVE ` +
  ...
  const _resumeManifest = manifest as unknown as Record<string, unknown>;
  if (_resumeManifest.scope_managed === true && manifest.status === 'in_progress') {
    const _resumeScope = (manifest.params as unknown as Record<string, unknown> | undefined)
      ?.scope;
    if (typeof _resumeScope === 'string' && _resumeScope.length > 0) {
      ...
      try {
        ...
        // eslint-disable-next-line no-console
        console.error(
          `[workflow-engine] Warning: workflow_resume could not reconcile ` +
  ...
  if (_resumeManifest.step_tools === undefined && manifest.status === 'in_progress') {
    try {
      ...
      _resumeManifest.step_tools = freezeStepTools(
        _resumeWf as unknown as ToolDeclaringWorkflow,
        collectAllSteps(_resumeWf),
      );
    } catch {
    }
  }
  ...
  setCurrentRunId((_resumeManifest.run_id as string | undefined) ?? null);

  const _retakenChildren: Array<{ parent_step: string; child_run_id: string | null }> = [];
  const _childWarnings: string[] = [];
  for (const [_sname, _sdata] of Object.entries(manifest.steps ?? {})) {
    const _planning = (_sdata as unknown as Record<string, unknown>).planning as
      | Record<string, unknown>
      | undefined;
    if (!_planning || _planning.phase !== 'executing') continue;
    const _childYaml = _planning.child_workflow_path;
    if (typeof _childYaml !== 'string' || !_childYaml) continue;
    const _childManifestPath = join(dirname(_childYaml), 'manifest.json');
    let _childManifest: Record<string, unknown>;
    try {
      _childManifest = JSON.parse(readFileSync(_childManifestPath, 'utf-8')) as Record<
        string,
        unknown
      >;
    } catch {
    ...
    if (
      ...
      _childWarnings.push(
        `dynamic child of step '${_sname}' is owned by another LIVE orchestrator ` +
    ...
    _retakenChildren.push({
      parent_step: _sname,
      child_run_id: (_childManifest.run_id as string | undefined) ?? null,
    });
  }

  const currentStep = manifest.current_step ?? null;
  const stepData = currentStep ? (manifest.steps[currentStep as unknown as string] ?? {}) : {};
  ...
  const result: Record<string, unknown> = {
    ...
    current_step_status: (stepData as { status?: string }).status ?? null,
  ...
  const _resumeLaneCursors = admissibleLaneCursors(manifest as unknown as Record<string, unknown>);
  ...
  try {
    if (typeof currentStep === 'string' && currentStep &&
        (stepData as { status?: string }).status === 'in_progress') {
      ...
      if (_answered.length > 0 || _pending.length > 0) {
        ...
        const _gateLines: string[] = [];
        if (_answered.length > 0) {
          const last = _answered[_answered.length - 1] as {
            message_id: string;
            response?: { type?: string; text?: string; responded_at?: string };
          };
          const r = last.response ?? {};
          _gateLines.push(
            `Step '${currentStep}' ALREADY HAS a recorded human answer: message ` +
        ...
        if (_pending.length > 0) {
          ...
          _gateLines.push(
            `Step '${currentStep}' has ${_pending.length} still-unanswered inbox ` +
  ...
  if (_retakenChildren.length > 0) result.dynamic_children_retaken = _retakenChildren;
  if (_childWarnings.length > 0) result.dynamic_child_warnings = _childWarnings;

  return result as unknown as WorkflowResumeOutput;
}
...
import {
  type SnapshotBranchInput,
  createSnapshot,
  deleteSnapshot,
  injectSpecAuthoringOutputs,
  normalizePerIterationOutputs,
  resolveOutputPath,
} from './output-validator.js';

function isDottedRoot(path: string): boolean {
...
function relpathPosix(target: string, from: string = process.cwd()): string {
  return toPosixSlashes(pathRelative(from, target));
}


interface BranchInputDescriptor {
  readonly path: string;
  readonly inject: string;
  readonly struct?: string;
  readonly glob_unmatched?: boolean;
}

interface BranchPromptDescriptor {
  readonly branch_index?: number;
  readonly branch_data?: unknown;
  readonly branch_dir?: string;
  readonly goal: string;
  readonly outputs_text: string;
  readonly output_schemas: readonly Record<string, unknown>[];
  readonly tool_docs: string;
  readonly params_text: string;
  readonly inputs: readonly BranchInputDescriptor[];
...
function composeBranchPrompt(
  branch: BranchPromptDescriptor,
  stepName: string,
  runtimeDir: string,
  defnDir: string | null,
): string {
...
function summarizeBranchData(branchData: unknown): string {
  const pyStr = (v: unknown): string => {
  ...
  if (branchData !== null && typeof branchData === 'object' && !Array.isArray(branchData)) {
    const obj = branchData as Record<string, unknown>;
    for (const key of ['name', 'domain', 'id', 'label', 'title', 'key']) {
      if (key in obj) return pyStr(obj[key]);
    }
    for (const [k, v] of Object.entries(obj)) {
      if (v !== null && (typeof v === 'object' || Array.isArray(v))) return k;
      return `${k}: ${pyStr(v)}`;
    }
  }
...
export function createStepSnapshot(
  ...
  const outputs = (stepConfig.outputs ?? []) as ReadonlyArray<unknown>;
  if (outputs.length === 0) return;

  try {
    const isParallel = stepConfig.parallel === true;
    const resolveForSnapshot = (p: string): string =>
      isParallel ? resolvePlaceholders(p, params) : resolveConcreteOutputPath(p, params);
    const resolvedOutputs: Array<Record<string, unknown> | string> = [];
    for (const out of outputs) {
      if (out !== null && typeof out === 'object' && !Array.isArray(out)) {
        const o = out as Record<string, unknown>;
        const entry: Record<string, unknown> = { ...o };
        entry.path = resolveForSnapshot((o.path as string | undefined) ?? '');
    ...
    createSnapshot(
      resolvedOutputs as unknown as Parameters<typeof createSnapshot>[0],
      runtimeDir,
      stepName,
      branches,
    );
    ...
    // eslint-disable-next-line no-console
    console.error(
      `[workflow-engine] WARN: snapshot creation failed for step '${stepName}': ${msg}. Proceeding without write_proof baseline.`,
    );
  }
}
...
function resolveStepInputs(
  ...
  const inputsInfo: Array<Record<string, unknown>> = [];
  const stepInputs = (stepConfig.inputs ?? []) as ReadonlyArray<unknown>;
  for (const inpRaw of stepInputs) {
    let path: string;
    let inject: string;
    let struct: string | undefined;
    if (inpRaw !== null && typeof inpRaw === 'object' && !Array.isArray(inpRaw)) {
      const inp = inpRaw as Record<string, unknown>;
      path = (inp.path as string | undefined) ?? '';
      inject = (inp.inject as string | undefined) ?? 'reference';
      struct = inp.struct as string | undefined;
    } else {
      path = String(inpRaw);
      inject = 'reference';
      struct = undefined;
    ...
    const hasGlob =
      resolvedPath.includes('*') || resolvedPath.includes('?') || resolvedPath.includes('[');
    ...
    if (hasGlob) {
      ...
      if (matched.length === 0 && inject === 'file_if_exists') continue;
      if (matched.length === 0) {
      ...
      for (const matchPath of matched) {
        ...
        const entry: Record<string, unknown> = { path: mp, inject };
        if (struct) entry.struct = struct;
      ...
      if (!isDottedRoot(displayPath) && !pathIsAbsolute(displayPath)) {
        ...
        if (!existsSync(candidate)) {
          ...
          if (existsSync(defnCandidate)) candidate = defnCandidate;
        }
        displayPath = relpathPosix(candidate);
      }
      const entry: Record<string, unknown> = { path: displayPath, inject };
      if (struct) entry.struct = struct;
      inputsInfo.push(entry);
    }
  }
  ...
  const seen = new Set<string>();
  const dedupedInputs: Array<Record<string, unknown>> = [];
  for (const inp of inputsInfo) {
    const key = `${inp.path as string}\u0000${inp.inject as string}`;
    if (!seen.has(key)) {
      seen.add(key);
...
function validateInputStructs(
  entries: ReadonlyArray<Record<string, unknown>>,
  stepName: string,
  defnDir: string,
): string | null {
  for (const inp of entries) {
    const structName = inp.struct as string | undefined;
    if (structName === undefined || structName === '') continue;
    if (inp.glob_unmatched === true) continue;
    ...
    if (!result.passed) {
      return (
        `Input '${p}' of step '${stepName}' failed struct validation ('${structName}'): ` +
        `${result.details.join('; ')}` +
...
function iterationForStep(
  workflow: Workflow,
  manifest: Record<string, unknown>,
  stepName: string,
): number | null {
  const steps = (manifest.steps ?? {}) as Record<string, Record<string, unknown>>;
  let best: { span: number; count: number } | null = null;
  for (const seq of collectAllSequences(workflow)) {
    const names = seq.steps.map((x) => (x.name as string | undefined) ?? '');
    const at = names.indexOf(stepName);
    if (at < 0) continue;
    for (let i = 0; i < seq.steps.length; i += 1) {
      const lb = seq.steps[i]?.loop_back as { to?: string } | undefined;
      if (!lb || typeof lb !== 'object') continue;
      const from = names.indexOf(lb.to ?? '');
      if (from < 0 || from > i) continue;
      if (at < from || at > i) continue;
      const owner = names[i] ?? '';
      const st = (steps[owner] ?? {}) as { loop_state?: { iterations?: number } };
      const count = typeof st.loop_state?.iterations === 'number' ? st.loop_state.iterations : 0;
      const span = i - from;
      if (best === null || span < best.span) best = { span, count };
    }
  }
...
interface StepBeginComposeContext {
  readonly workflowName: string;
  readonly stepName: string;
  readonly workflow: Workflow;
  readonly defnDir: string;
  readonly runtimeDir: string;
  readonly manifestPath: string;
  readonly manifest: MutableRunManifest;
  readonly stepConfig: Record<string, unknown>;
  readonly activeSeq: ReturnType<typeof resolveActiveSequence>;
...
interface StepBeginMaterial {
  readonly paramsText: string;
  readonly goalResolved: string;
  readonly dedupedInputs: readonly unknown[];
  readonly outputsText: string;
  readonly outputSchemas: ReadonlyArray<Record<string, unknown>>;
  readonly summariesText: string;
  readonly specGuidanceText: string;
  readonly constraintsText: string;
  readonly runTokenText: string;
  readonly runToken: string;
  readonly useSubagent: boolean;
  readonly activeTools: ReadonlyArray<ActiveToolDescriptor>;
  readonly isCcPerStep: boolean;
  readonly isOcPerStep: boolean;
  readonly isCopilotPerStep: boolean;
  readonly isGeminiPerStep: boolean;
  readonly toolDocsBlock: string;
  readonly subagentType: string | null;
  readonly profileSubagentTypeFor: (profileId: string) => string | null;
}

function composeStepMaterial(
  ctx: StepBeginComposeContext,
  params: Record<string, unknown>,
): StepBeginMaterial | StepBeginError {
  const {
    workflowName,
    stepName,
    workflow,
  ...
  const runToken = (manifest.run_token as unknown as string) ?? '';
  ...
  const goal = (stepConfig.goal as string | undefined) ?? '';
  ...
  if (inputStructError !== null) {
    return { error: inputStructError } as unknown as StepBeginError;
  }

  const outputsParts: string[] = [];
  const schemaSections: string[] = [];
  const outputSchemas: Array<Record<string, unknown>> = [];
  const stepOutputs = (stepConfig.outputs ?? []) as ReadonlyArray<unknown>;
  for (const outRaw of stepOutputs) {
    let opath: string;
    let ostruct: string | undefined;
    if (outRaw !== null && typeof outRaw === 'object' && !Array.isArray(outRaw)) {
      const out = outRaw as Record<string, unknown>;
      opath = (out.path as string | undefined) ?? '';
      ostruct = out.struct as string | undefined;
    } else {
      opath = String(outRaw);
      ostruct = undefined;
    }
    let opathResolved: string;
    if (stepConfig.parallel === true) {
      ...
      try {
        ...
        if (e instanceof OutputPathError) {
          return {
            error:
              `Output path error in step '${stepName}' (output '${opath}'): ${e.message}` +
              AUTHORING_ERROR_DIRECTIVE,
            action: 'STOP_WORKFLOW',
          } as unknown as StepBeginError;
        }
        throw e;
      }
    }
    let opathForAgent: string;
    ...
    if (ostruct) {
      ...
      try {
        ...
        schemaSections.push(
          `**Struct \`${ostruct}\`** (output: \`${opathForAgent}\`):\n\`\`\`yaml\n${schemaContent}\n\`\`\`\nWrite as **${fmt}** matching this schema exactly.`,
        );
      } catch (e) {
        if ((e as NodeJS.ErrnoException)?.code !== 'ENOENT') throw e;
        schemaSections.push(
          `**Struct \`${ostruct}\`**: ⚠ schema file not found at \`${schemaPath}\` — ask the user for guidance.`,
        );
      }
      outputSchemas.push({
        path: opathForAgent,
  ...
  let summariesText = '';
  const wfCarry = ((workflow.context ?? {}) as Record<string, unknown>).carry_forward ?? 'none';
  const stepCarry = stepConfig.carry_forward;
  let shouldCarry = false;
  if (stepCarry === false) shouldCarry = false;
  else if (stepCarry === true) shouldCarry = true;
  else if (wfCarry === 'summary') shouldCarry = true;

  if (shouldCarry) {
    const summaryParts: string[] = [];
    for (const s of activeSeq) {
      const sname = s.name as string;
      if (sname === stepName) break;
      const sdata = manifest.steps?.[sname] as { status?: string } | undefined;
      if (sdata?.status === 'completed') {
        const summaryPath = join(runtimeDir, 'context', `${sname}.summary.md`);
        try {
    ...
    if (summaryParts.length > 0) summariesText = summaryParts.join('\n\n');
  }

  let specGuidanceText = '';
  {
    const specCheckVal = Boolean(
      'spec_check' in stepConfig ? stepConfig.spec_check : false,
    );
    const rawAuthoring = stepConfig.spec_authoring;
    const specAuthoringVal =
      rawAuthoring === 'persist' || rawAuthoring === 'validate' ? rawAuthoring : undefined;
    if (specCheckVal || specAuthoringVal) {
      try {
        const projectRootSpec = dirname(paths.agentDir);
        const specScope = scopeResolveActiveScope(
          (params.scope as string | undefined) ?? null,
          projectRootSpec,
        )[0];
        const domains = composeDomainsEcho(specScope, projectRootSpec);
        const scopeHint = scopeGetScopeHint(specScope, projectRootSpec);
        specGuidanceText = composeSpecGuidance(
          { specCheck: specCheckVal, specAuthoring: specAuthoringVal },
          specScope,
          domains,
          scopeHint,
        );
  ...
  if (useSubagent) {
    ...
    const deniedShell = stepDeniedCapabilities(
      stepConfig as unknown as Record<string, unknown>,
    ).includes('shell');
    if (deniedShell) {
  ...
  let activeTools: ReadonlyArray<ActiveToolDescriptor> = [];
  const wfTools = (workflow.tools ?? []) as unknown as ReadonlyArray<
    ActiveToolDescriptor & { name?: string }
  >;
  if (wfTools.length > 0) {
    const stepToolsFilter = stepConfig.tools as readonly string[] | undefined;
  ...
  const isOcPerStep =
    ...
    (() => {
      try {
        return statSync(
          join(perStepProjectRoot, '.opencode', 'agents', `${AGENT_PREFIX}${workflowName}-${stepName}.md`),
        ).isFile();
      } catch (e) {
        if (!(e instanceof Error) || !('code' in e)) throw e;
        return false;
      ...
      try {
        return statSync(
          join(
            ...
            '.github',
            'agents',
            `${AGENT_PREFIX}${workflowName}-${stepName}.agent.md`,
          ),
        ).isFile();
      } catch (e) {
        if (!(e instanceof Error) || !('code' in e)) throw e;
  ...
  const isGeminiPerStep =
    useSubagent &&
    getEngineHost() === 'gemini' &&
    (() => {
      try {
        return statSync(
          join(perStepProjectRoot, '.gemini', 'agents', `${AGENT_PREFIX}${workflowName}-${stepName}.md`),
        ).isFile();
      } catch (e) {
        if (!(e instanceof Error) || !('code' in e)) throw e;
        return false;
  ...
  const profileSubagentMemo = new Map<string, string | null>();
  const profileSubagentTypeFor = (profileId: string): string | null => {
...
function composeStepBeginPayload(ctx: StepBeginComposeContext): StepBeginOutput | StepBeginError {
  const {
    workflowName,
    stepName,
    workflow,
  ...
  const params = (manifest.params ?? {}) as Record<string, unknown>;
  {
    const it = iterationForStep(workflow, manifest as unknown as Record<string, unknown>, stepName);
    if (it !== null) {
      params.iteration = it;
      if (!manifest.params) manifest.params = params;
      else (manifest.params as Record<string, unknown>).iteration = it;
    }
  }
  ...
  if ((stepConfig.type as string | undefined) === 'planning') {
    ...
    const stepData = (manifest.steps[stepName] ?? {}) as Record<string, unknown>;
    const prevPlanning = (stepData.planning as Record<string, unknown> | undefined) ?? null;
    const attempts =
      prevPlanning && typeof prevPlanning.attempts === 'number' ? prevPlanning.attempts : 0;
    stepData.status = 'in_progress';
    stepData.started_at = now;
    if (!stepData.first_started_at) stepData.first_started_at = now;
    {
      const priorLoopState = stepData.loop_state as Record<string, unknown> | undefined;
      if (priorLoopState) delete priorLoopState.resolved_for_pass;
    }
    stepData.planning = {
      attempts,
      phase: 'planning',
      child_run_id: (prevPlanning?.child_run_id as string | null | undefined) ?? null,
      child_workflow_path:
        (prevPlanning?.child_workflow_path as string | null | undefined) ?? null,
    };
    ...
    const runIdForTracePlan = (manifest.run_id as string | undefined) ?? '';
    ...
    return {
      ...
      model: (stepConfig.model as string | undefined) ?? null,
    } as unknown as StepBeginOutput;
  }

  if ('delegate_to' in stepConfig) {
    const delegateParams = (stepConfig.params ?? {}) as Record<string, unknown>;
    ...
    try {
      ...
      if (existsSync(delegGateResultPath)) unlinkSync(delegGateResultPath);
    } catch {
    }

    const runIdForTraceDeleg = (manifest.run_id as string | undefined) ?? '';
    ...
    return {
      ...
      goal: (stepConfig.goal as string | undefined) ?? '',
      param_bindings: (stepConfig.param_bindings ?? {}) as Record<string, unknown>,
      model: (stepConfig.model as string | undefined) ?? null,
      engine_instructions:
        `Delegation protocol: (1) call workflow_init for '${delegateTarget}' with the ` +
  ...
  if ('error' in _mat) return _mat as unknown as StepBeginError;
  const {
    paramsText,
    goalResolved,
    dedupedInputs,
  ...
  try {
    ...
    if (existsSync(perStepGate)) unlinkSync(perStepGate);
  } catch {
  }
  const gateResultPath = join(runtimeDir, 'gate-result.json');
  try {
    if (existsSync(gateResultPath)) unlinkSync(gateResultPath);
  } catch {
  }
  ...
  const now = nowIsoLocal();
  const stepDataInit = (manifest.steps[stepName] ?? {}) as Record<string, unknown>;
  stepDataInit.status = 'in_progress';
  if (!('first_started_at' in stepDataInit)) stepDataInit.first_started_at = now;
  stepDataInit.started_at = now;
  {
    const priorLoopState = stepDataInit.loop_state as Record<string, unknown> | undefined;
    if (priorLoopState) delete priorLoopState.resolved_for_pass;
  }
  ...
  const parallelKeyRaw = stepConfig.parallel_key as string | undefined;

  if (isParallel && parallelKeyRaw) {
    let branchItems: unknown[];
    try {
      branchItems = resolveParallelBranches(
        parallelKeyRaw,
        runtimeDir,
        workflow as unknown as { steps?: ReadonlyArray<Record<string, unknown>> },
        params,
      );
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
    ...
    const branchProfilesCfg = stepBranchProfiles(stepConfig as unknown as Record<string, unknown>);
    const profileByBranch: Array<{ id: string; profile: BranchProfile } | null> = [];
    if (branchProfilesCfg) {
      for (let bi = 0; bi < branchItems.length; bi += 1) {
        ...
        if (pid === null || profile === undefined) {
          return {
            error:
              `Step '${stepName}' declares branch_profiles, but parallel_key item ${bi} ` +
              (pid === null
      ...
      for (let bi = 0; bi < branchItems.length; bi += 1) profileByBranch.push(null);
    }

    const branches: BranchPromptDescriptor[] = [];
    const branchesForSnapshot: SnapshotBranchInput[] = [];
    const rawOutputs = (stepConfig.outputs ?? []) as ReadonlyArray<unknown>;
    const allBranchResults: BranchPathResult[] = [];
    const resolvedOutputsByBranch: Array<
      Array<{ declared: string; working: string; semantic: string; scaffolded: boolean; struct?: string }>
    > = [];
    for (let bi = 0; bi < branchItems.length; bi += 1) {
      ...
      const branchOutputsParts: string[] = [];
      const branchSchemaSections: string[] = [];
      const branchOutputSchemas: Array<Record<string, unknown>> = [];
      const branchOutputsForSnapshot: Array<Record<string, unknown>> = [];
      const branchResolvedOutputs: Array<{
        declared: string;
        working: string;
        semantic: string;
        scaffolded: boolean;
      ...
      for (const outRaw of rawOutputs) {
        let opath: string;
        let ostruct: string | undefined;
        let writeProof: string | undefined;
        if (outRaw !== null && typeof outRaw === 'object' && !Array.isArray(outRaw)) {
          const out = outRaw as Record<string, unknown>;
          opath = (out.path as string | undefined) ?? '';
          ostruct = out.struct as string | undefined;
          writeProof = out.write_proof as string | undefined;
        } else {
          opath = String(outRaw);
        }
        ...
        let resolvedBranch: BranchPathResult;
        try {
          ...
          if (e instanceof BranchPathError || e instanceof OutputPathError) {
            return {
              error:
                `Parallel output path error in step '${stepName}' (output '${opath}'): ${e.message}` +
                AUTHORING_ERROR_DIRECTIVE,
              action: 'STOP_WORKFLOW',
            };
          }
        ...
        let branchOpathForAgent: string;
        ...
        if (ostruct) {
          ...
          const fmt = FORMAT_MAP[ext] ?? 'JSON';
          branchOutputsParts.push(
            `- \`${branchOpathForAgent}\` — **${fmt}** file, struct: \`${ostruct}\``,
          );
          try {
            const sc = readFileSync(schemaPath, 'utf-8').trim();
            branchSchemaSections.push(
              `**Struct \`${ostruct}\`** (output: \`${branchOpathForAgent}\`):\n\`\`\`yaml\n${sc}\n\`\`\`\nWrite as **${fmt}** matching this schema exactly.`,
            );
          } catch (e) {
            if ((e as NodeJS.ErrnoException)?.code !== 'ENOENT') throw e;
            branchSchemaSections.push(
              `**Struct \`${ostruct}\`**: schema not found at \`${schemaPath}\``,
            );
          }
        ...
        const snapEntry: Record<string, unknown> = { path: branchOpathResolved };
        if (ostruct) snapEntry.struct = ostruct;
        if (writeProof !== undefined) snapEntry.write_proof = writeProof;
      ...
      if (branchProfile !== null) {
        ...
        const branchActiveTools = activeTools.filter(
          (t) => typeof t.name === 'string' && subset.has(t.name),
        );
        branchToolDocs =
      ...
      branches.push({
        ...
        inputs: dedupedInputs as unknown as readonly BranchInputDescriptor[],
      ...
      branchesForSnapshot.push({
        branch_index: bi,
        branch_dir: branchDir,
        outputs: branchOutputsForSnapshot as unknown as Parameters<typeof createSnapshot>[0],
      });
    }
    ...
    try {
      ...
      if (e instanceof BranchPathError) {
        return {
          error:
            `Parallel output collision in step '${stepName}': ${e.message}` +
            AUTHORING_ERROR_DIRECTIVE,
          action: 'STOP_WORKFLOW',
        };
      }
    ...
    const slimBranches: Array<Record<string, unknown>> = [];
    ...
    const parallelBranches: Record<string, unknown> = {};
    ...
    const sd = (manifest.steps[stepName] ?? {}) as Record<string, unknown>;
    ...
    const sd2 = (manifest.steps[stepName] ?? {}) as Record<string, unknown>;
    sd2.started_at = now2;
    manifest.steps[stepName] = sd2;
    manifest.updated_at = now2;
    safeWriteJson(manifestPath, manifest);

    const runIdForTrace2 = (manifest.run_id as string | undefined) ?? '';
    if (runIdForTrace2) {
    ...
    if (
      result.subagent_warning === undefined &&
      (isCcPerStep || isOcPerStep || isCopilotPerStep || isGeminiPerStep)
    ) {
      const missingProfiles = [
        ...new Set(
          profileByBranch
            .filter((p): p is { id: string; profile: BranchProfile } => p !== null)
            .map((p) => p.id),
        ),
    ...
    {
      const _ei = composeStepBeginEngineInstructions(
        useSubagent,
        existingStatus === 'in_progress',
        stepName,
        effectiveModel,
      );
      if (_ei) result.engine_instructions = _ei;
    }
    return result as unknown as StepBeginOutput;
  }

  const runIdForTrace = (manifest.run_id as string | undefined) ?? '';
  if (runIdForTrace) {
  ...
  if (useSubagent) {
    ...
    const slim: Record<string, unknown> = {
      ...
      model: effectiveModel,
      gate: {
        semantic: stepGateFlag(
          (workflow.gate ?? {}) as Record<string, unknown>,
          (stepConfig.gate ?? {}) as Record<string, unknown>,
          'semantic',
        ),
        human: humanGateForBeginPayload(
          (workflow.gate ?? {}) as Record<string, unknown>,
          (stepConfig.gate ?? {}) as Record<string, unknown>,
        ),
      },
    ...
    }
    {
      ...
      if (_ei) slim.engine_instructions = _ei;
    }
    return slim as unknown as StepBeginOutput;
  }
...
function composeNextBeginForAdvance(ctx: {
  ...
  readonly paths: EnginePaths;
}): StepBeginOutput | null {
  try {
    const manifest = JSON.parse(
      readFileSync(ctx.manifestPath, 'utf-8'),
    ...
    const modelOverride: ModelMode | undefined = isModelMode(manifest.model_override)
      ? manifest.model_override
      : undefined;
    const effectiveModel: string | null =
      modelOverride ?? ((stepConfig.model as string | undefined) ?? null);
    const existingStatus = (manifest.steps[ctx.nextStep] as { status?: string } | undefined)
      ?.status;
    ...
    if ((payload as { error?: unknown }).error !== undefined) return null;
    const stepData = (manifest.steps[ctx.nextStep] ?? {}) as Record<string, unknown>;
    stepData.composite_begun = true;
    manifest.steps[ctx.nextStep] = stepData;
    safeWriteJson(ctx.manifestPath, manifest);
    return payload as StepBeginOutput;
  } catch {
    return null;
  }
}
...
function composeRetryBegin(ctx: {
  ...
  try {
    ...
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf-8')) as MutableRunManifest;
    if (!manifest.steps) manifest.steps = {};
    ...
    if ('error' in mat) return null;
    if (!mat.useSubagent) return null;
      ...
      composeBranchPrompt(
        {
          ...
          output_schemas: mat.outputSchemas as readonly Record<string, unknown>[],
          tool_docs: mat.toolDocsBlock,
          params_text: mat.paramsText,
          inputs: mat.dedupedInputs as unknown as readonly BranchInputDescriptor[],
    ...
    return {
      ...
      gate: {
        semantic: stepGateFlag(
          (ctx.workflow.gate ?? {}) as Record<string, unknown>,
          (stepConfig.gate ?? {}) as Record<string, unknown>,
          'semantic',
        ),
        human: humanGateForBeginPayload(
...
export function toolStepBegin(
  args: StepBeginInput,
  paths: EnginePaths = defaultPaths(),
  overrides?: EngineRunOverrides,
): StepBeginOutput | StepBeginError {
  const workflowName =
    (args.name as unknown as string) ||
    ((args as unknown as Record<string, unknown>).workflow_name as string);
  const stepName =
    (args.step as unknown as string) ||
    ((args as unknown as Record<string, unknown>).step_name as string);

  if (typeof workflowName !== 'string' || !workflowName) {
    return {
      error:
        "step_begin: missing required parameter 'name' (workflow name). " +
  ...
  if (typeof stepName !== 'string' || !stepName) {
    return {
      error:
        "step_begin: missing required parameter 'step' (step name). " +
  ...
  if (_ownBegin) return _ownBegin as unknown as StepBeginError;

  const stepConfig: Record<string, unknown> | null = findStepConfig(workflow, stepName);
  const activeSeq = resolveActiveSequence(workflow, manifest);
  const activeNames = activeSeq.map((s) => (s.name as string | undefined) ?? '');
  ...
  if (_laneCursors.length > 0) {
    ...
    if (!hit) {
      ...
      return {
        error:
          `Cannot begin step '${stepName}': parallel lanes are ACTIVE and it is not any ` +
    ...
    if (hitEntry && hitEntry.status === 'failed') hitEntry.status = 'running';
  }
  const cursorStep = (manifest.current_step as string | undefined) ?? '';
  if (_laneCursors.length === 0 && cursorStep && cursorStep !== stepName) {
    ...
    if (cursorIdx < 0) {
      // eslint-disable-next-line no-console
      console.error(
        `[workflow-engine] Warning: manifest cursor '${cursorStep}' not found in the ` +
      ...
      if (_bug077Gr !== null) {
        const _check = checkStepFailureBlocked(
          _bug077Gr,
          _bug077PrevConfig,
          workflow as unknown as Record<string, unknown>,
        ...
        if (_check.blocked) {
          return {
            error:
              `Cannot begin step '${stepName}': previous step '${cursorStep}' ` +
      ...
      const _cursorData = manifest.steps?.[cursorStep] as
        | { status?: string; composite_begun?: boolean }
        | undefined;
      const cursorStatus = (_cursorData?.status as string | undefined) ?? 'pending';
      const directive =
        cursorStatus === 'pending' || _cursorData?.composite_begun === true
      ...
      return {
        error:
          `Cannot begin step '${stepName}': the workflow cursor is at step ` +
      ...
      return {
        error:
          `Cannot begin step '${stepName}': it is not the current step and not part of the ` +
      ...
      return {
        error:
          `Cannot begin step '${stepName}': it is BEFORE the workflow cursor ` +
  ...
  const existingStatus = (manifest.steps?.[stepName] as { status?: string })?.status;
  if (existingStatus === 'completed') {
    return {
      error:
        `Cannot begin step '${stepName}': it is already completed. Re-running ` +
  ...
  {
    const _sd = manifest.steps[stepName] as Record<string, unknown> | undefined;
...
function recordLoopTraceEvent(
  runtimeDir: string,
  _runId: string,
  stepOrder: ReadonlyArray<string>,
  toIdx: number,
  fromIdx: number,
  event: Record<string, unknown> & { kind: 'loop_back' | 'loop_exit'; phase: number },
): void {
  const tracePath = join(runtimeDir, 'trace.json');
  withTraceLock(tracePath, () => {
    try {
      ...
      for (let j = Math.max(0, toIdx); j <= fromIdx; j += 1) {
        const n = stepOrder[j];
        if (n) rangeNames.add(n);
      }
      for (const s of trace.steps ?? []) {
        if (!rangeNames.has(s.name as string) || s.loop_archived === true) continue;
        s.loop_phase = event.phase;
        if (event.kind === 'loop_back') s.loop_archived = true;
      }
      if (!Array.isArray(trace.loop_events)) trace.loop_events = [];
...
function recordRouteTraceEvent(
  runtimeDir: string,
  _runId: string,
  event: Record<string, unknown> & { kind: 'route_enter' | 'route_exit'; route_id: string; owner_step: string },
): void {
  const tracePath = join(runtimeDir, 'trace.json');
  withTraceLock(tracePath, () => {
    try {
      const trace = JSON.parse(readFileSync(tracePath, 'utf-8')) as Record<string, unknown> & {
        route_events?: Array<Record<string, unknown>>;
      };
      if (!Array.isArray(trace.route_events)) trace.route_events = [];
...
function recordLaneTraceEvent(
  runtimeDir: string,
  _runId: string,
  event: Record<string, unknown> & { kind: 'lanes_enter' | 'lanes_join'; fork_step: string },
): void {
  const tracePath = join(runtimeDir, 'trace.json');
  withTraceLock(tracePath, () => {
    try {
      const trace = JSON.parse(readFileSync(tracePath, 'utf-8')) as Record<string, unknown> & {
        lane_events?: Array<Record<string, unknown>>;
      };
      if (!Array.isArray(trace.lane_events)) trace.lane_events = [];
...
function flushTraceSnapshot(
  ...
  try {
    let trace: Record<string, unknown> & { steps: Array<Record<string, unknown>> };
    try {
      trace = JSON.parse(readFileSync(tracePath, 'utf-8')) as typeof trace;
    } catch (readErr) {
      ...
      trace = {
        trace_version: 1,
        workflow: (manifest.workflow as string | undefined) ?? null,
        run_id: _runId,
        steps: [],
    ...
    if (!stepEntry) {
      const wfSteps = (workflow.steps ?? []) as ReadonlyArray<Record<string, unknown>>;
      let stepIndex = -1;
      for (let i = 0; i < wfSteps.length; i++) {
        if ((wfSteps[i] as { name?: string }).name === stepName) {
          stepIndex = i;
          break;
        }
      }
      let stepConfig: Record<string, unknown> = stepIndex >= 0 ? (wfSteps[stepIndex] ?? {}) : {};
      if (stepIndex < 0) {
        for (const s of collectAllSteps(workflow as Workflow)) {
          if (s.name === stepName) {
            stepConfig = s;
            break;
          }
        }
        const mstepAll = ((manifest.steps as Record<string, Record<string, unknown>> | undefined) ??
          {})[stepName];
        const mstepRoute = mstepAll?.route as { owner_step?: string } | undefined;
        const mstepLane = mstepAll?.lane as { fork_step?: string; lane_no?: number; pos?: number } | undefined;
        if (mstepRoute?.owner_step !== undefined || mstepLane?.fork_step === undefined) {
          ...
          const siblings = trace.steps.filter(
            (s) => typeof s.index === 'number' && (s.index as number) > ownerIdx && (s.index as number) < ownerIdx + 1,
          ).length;
          stepIndex = ownerIdx + 0.001 * (siblings + 1);
        } else {
          const forkCard = trace.steps.find((s) => s.name === mstepLane.fork_step);
          const forkIdx = typeof forkCard?.index === 'number' ? (forkCard.index as number) : trace.steps.length;
      ...
      stepEntry = {
        ...
        config: {
          ...
          ...('spec_authoring' in stepConfig
            ? { spec_authoring: (stepConfig as { spec_authoring?: unknown }).spec_authoring }
            : {}),
          subagent: 'subagent' in stepConfig ? stepConfig.subagent : true,
          gate: (stepConfig as { gate?: unknown }).gate ?? null,
        },
        goal: (stepConfig as { goal?: string }).goal ?? null,
        invocations: [],
        retry_count: 0,
      };
      if ('delegate_to' in stepConfig) {
    ...
    const mstep = ((manifest.steps as Record<string, Record<string, unknown>> | undefined) ??
      {})[stepName] ?? {};
    if (mstep.status) stepEntry.status = mstep.status;
    if (stepEntry.awaiting_human && mstep.status && mstep.status !== 'in_progress') {
      delete stepEntry.awaiting_human;
    }
    if (mstep.started_at) stepEntry.started_at = mstep.started_at;
    if (mstep.completed_at) stepEntry.completed_at = mstep.completed_at;
    if (mstep.duration_ms != null) stepEntry.duration_ms = mstep.duration_ms;
    if (mstep.summary) stepEntry.summary = mstep.summary;
    if (mstep.tool_warnings) stepEntry.tool_warnings = mstep.tool_warnings;
    if (mstep.route) stepEntry.route = mstep.route;
    if (mstep.lane) stepEntry.lane = mstep.lane;
    if (mstep.planning) {
      stepEntry.planning = { ...(mstep.planning as Record<string, unknown>) };
      const planning = mstep.planning as Record<string, unknown>;
      const childWfPath = planning.child_workflow_path as string | undefined;
      const childRunId = planning.child_run_id as string | undefined;
    ...
    if (mstep.delegation) {
      stepEntry.delegation = { ...(mstep.delegation as Record<string, unknown>) };
      const delegChildRunId = (mstep.delegation as { child_run_id?: string | null }).child_run_id;
    ...
    for (const s of trace.steps) {
      const invs = (s.invocations ?? []) as ReadonlyArray<Record<string, unknown>>;
      for (const inv of invs) {
        totalMsgs += (inv.message_count as number | undefined) ?? 0;
        totalTools += (inv.tool_call_count as number | undefined) ?? 0;
        for (const fp of (inv.modified_files as string[] | undefined) ?? []) allFiles.add(fp);
      }
      totalStepDuration += (s.duration_ms as number | undefined) ?? 0;
    }
    ...
    if (manifest.params) trace.params = manifest.params;

    trace.steps.sort(
      (a, b) =>
        ((a.index as number | undefined) ?? 999) - ((b.index as number | undefined) ?? 999),
    );

    safeWriteJson(tracePath, trace);

    const bridgeParentRunId = manifest.parent_run_id as string | undefined;
    const bridgeParentStep = manifest.parent_step as string | undefined;
    if (
      typeof bridgeParentRunId === 'string' &&
      ...
      try {
        ...
        withTraceLock(parentTracePath, () => {
          const parentTrace = JSON.parse(readFileSync(parentTracePath, 'utf-8')) as {
            steps?: Array<Record<string, unknown>>;
          };
          const card = (parentTrace.steps ?? []).find(
            (s) => s.name === bridgeParentStep && s.loop_archived !== true,
          );
          if (card) {
    ...
    logEngine(
      `flushTraceSnapshot failed for step '${stepName}': ${e instanceof Error ? e.message : String(e)}`,
    );
  } finally {
    if (traceLockFd !== null) releaseFileLock(traceLockFd, `${tracePath}.lock`);
  }
}


function composeSpecClauseForSemanticGate(
  stepConfig: Record<string, unknown>,
  domains: readonly DomainSummary[] = [],
  scopeHint?: string | null,
): string {
  const specCheckVal = 'spec_check' in stepConfig ? stepConfig.spec_check : false;
  if (!specCheckVal) return '';
...
function resolveConsumptionDomains(paths: EnginePaths): DomainSummary[] {
  try {
...
function resolveConsumptionScopeHint(paths: EnginePaths): string | null {
  try {
    const root = dirname(paths.agentDir);
...
function resolvesStepHuman(wfGate: Record<string, unknown>, step: Record<string, unknown>): boolean {
  const g = (step.gate ?? {}) as Record<string, unknown>;
  return Object.prototype.hasOwnProperty.call(g, 'human')
    ? Boolean(g.human)
    : Boolean(wfGate.human);
}

export function workflowMayNeedExternalChannel(workflow: Workflow): boolean {
  const wfGate = (workflow.gate ?? {}) as Record<string, unknown>;
  for (const step of collectAllSteps(workflow)) {
    const gate = (step.gate ?? {}) as Record<string, unknown>;
    if (resolveHumanChannel(wfGate, gate) !== 'terminal' && resolvesStepHuman(wfGate, step)) return true;
    for (const key of ['loop_back', 'routes'] as const) {
      const when = ((step[key] as Record<string, unknown> | undefined)?.when ?? {}) as Record<
        string,
        unknown
      >;
      if (when.human === true && isExternalChannel(when.human_channel)) return true;
    }
  }
  return false;
}
...
function evaluateHumanGate(
  ...
  if (typeof cfg === 'boolean') return { required: cfg };
  ...
  const steps = (manifest?.steps ?? {}) as Record<string, Record<string, unknown> | undefined>;
  const recorded = steps[stepName]?.human_gate_verdict as { required?: unknown } | undefined;
  ...
  try {
    const parsed = JSON.parse((proc.stdout ?? '').trim()) as { human?: unknown };
    if (typeof parsed.human !== 'boolean') throw new Error('missing boolean "human" field');
  ...
  if (manifest) {
    const stepsObj = (manifest.steps ?? {}) as Record<string, Record<string, unknown>>;
...
function humanGateForBeginPayload(
  wfGate: Record<string, unknown>,
  stepGate: Record<string, unknown>,
): boolean | 'conditional' {
  const cfg = humanGateConfig(wfGate, stepGate);
...
function appendHumanGateInstructions(existing: unknown, channel: HumanChannel = 'terminal'): string {
  const block =
    channel === 'terminal'
...
function autoFailBudgetExhaustedRun(
  workflowName: string,
  stepName: string,
  manifestPath: string,
  details: string,
  finalizeOverrides: EngineRunOverrides,
  paths: EnginePaths,
): void {
  try {
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf-8')) as Record<string, unknown>;
  ...
  try {
    toolWorkflowFinalize({ name: workflowName } as WorkflowFinalizeInput, paths, finalizeOverrides);
  } catch {
  }
}

function runInlineGateValidation(
  workflowName: string,
  stepName: string,
  runtimeDir: string,
  manifestPath: string,
  resultPath: string,
  yamlPathOverride: string | null,
  paths: EnginePaths,
  finalizeOverrides: EngineRunOverrides = {},
  ...
  try {
    ...
    return {
      ...
      action: 'STOP_WORKFLOW',
    } as unknown as StepCollectResultOutput;
  }
  ...
  let manifest: Record<string, unknown> = {};
  let stepStartedAt: string | null = null;
  try {
    manifest = JSON.parse(readFileSync(manifestPath, 'utf-8')) as Record<string, unknown>;
    const steps = (manifest.steps ?? {}) as Record<string, Record<string, unknown>>;
    const stepData = steps[stepName] ?? {};
    stepStartedAt =
      (stepData.first_started_at as string | undefined) ??
      (stepData.started_at as string | undefined) ??
      null;
  } catch {
  }
  ...
  const outputs = (stepConfig.outputs ?? []) as ReadonlyArray<Output | string>;

  const parallelBranches = manifest.parallel_branches as Record<string, unknown> | undefined;
  let result: { passed: boolean; checks: number; failures: number; details: readonly string[] };
  let gateResultExtra: Record<string, unknown> = {};
  ...
  if (!structuralEnabled) {
    ...
    for (const biStr of sortedKeys) {
      ...
      const branchResolved = (
        parallelBranches[biStr] as
          | { resolved_outputs?: ReadonlyArray<{ declared: string; working: string }> }
          | undefined
      )?.resolved_outputs;
      const narrowed =
        branchResolved && branchResolved.length > 0
      ...
      const r = validateStepOutputs(narrowed, definitionDir, runtimeDir, {
        ...
        params: (manifest.params ?? null) as Record<string, unknown> | null,
      });
      branchResults.push({
        branch_index: bi,
        passed: r.passed,
      ...
      for (const d of r.details) allDetails.push(`[branch ${bi}] ${d}`);
    }
    const failedBranches = branchResults
      .filter((br) => !br.passed)
      .map((br) => br.branch_index as number);
    let aggPassed: boolean;
    if (failedBranches.length === 0) aggPassed = true;
    else if (failedBranches.length === branchResults.length) aggPassed = false;
    else aggPassed = true;
    result = { passed: aggPassed, checks: totalChecks, failures: totalFailures, details: allDetails };
    gateResultExtra = { branch_results: branchResults };
    if (failedBranches.length > 0) gateResultExtra.failed_branches = failedBranches;
  } else {
  ...
  const runToken = (manifest.run_token as string | undefined) ?? '';
  const validatedAt = nowIsoLocal();
  const branchResultsForLedger = gateResultExtra.branch_results as
    | Array<{ branch_index: number; passed: boolean; checks: number; failures: number; details: string[] }>
    | undefined;
  try {
    mkdirSync(dirname(resultPath), { recursive: true });
    lockedJsonReadModifyWriteSync<GateLedgerV2>(resultPath, (prev) => {
      ...
      if (branchResultsForLedger && branchResultsForLedger.length > 0) {
        ...
        const prevLedger = prev as Partial<GateLedgerV2> | null;
  ...
  const wfGate = (workflow.gate ?? {}) as Record<string, unknown>;
  const stepGateLookup = (stepConfig.gate ?? {}) as Record<string, unknown>;
  ...
  if (result.passed) {
    ...
    if (semantic) {
      const isPlanning = stepConfig && (stepConfig.type as string | undefined) === 'planning';
      ...
      const specClause = composeSpecClauseForSemanticGate(stepConfig as Record<string, unknown>, resolveConsumptionDomains(paths), resolveConsumptionScopeHint(paths));
      inlineResult.engine_instructions = baseInstr + specClause;
    }
    if (human) {
      inlineResult.engine_instructions = appendHumanGateInstructions(
        inlineResult.engine_instructions,
        resolveHumanChannel(wfGate, stepGateLookup),
      );
    }
    return inlineResult as unknown as StepCollectResultOutput;
  }
  ...
  try {
    ...
    const steps = (manifest.steps ?? {}) as Record<string, Record<string, unknown>>;
    const stepData = steps[stepName] ?? {};
    stepRetryCount = ((stepData.step_retry_count as number | undefined) ?? 0) + 1;
  ...
  if (stepRetryCount > maxStepRetries) {
    autoFailBudgetExhaustedRun(
      workflowName,
      stepName,
      manifestPath,
      `Step '${stepName}' exhausted retry budget (${stepRetryCount}/${maxStepRetries}).`,
      finalizeOverrides,
      paths,
    );
    return {
  ...
  return {
    ...
    ...(getEngineHost() === 'opencode'
      ? {
          engine_instructions:
            `RETRY (${stepRetryCount}/${maxStepRetries}): re-invoke the task tool with ` +
      ...
      : getEngineHost() === 'gemini'
        ? {
            engine_instructions:
              `RETRY (${stepRetryCount}/${maxStepRetries}): re-invoke the invoke_agent tool ` +
        ...
        : retryBegin !== null
          ? {
              engine_instructions:
                `RETRY (${stepRetryCount}/${maxStepRetries}): spawn a FRESH subagent from ` +
...
function opencodeSubagentTypeFor(workflowName: string, stepName: string, projectRoot: string): string {
  try {
    if (
      statSync(join(projectRoot, '.opencode', 'agents', `riglane-${workflowName}-${stepName}.md`)).isFile()
    ) {
...
function geminiSubagentTypeFor(workflowName: string, stepName: string, projectRoot: string): string {
  try {
    if (
      statSync(join(projectRoot, '.gemini', 'agents', `riglane-${workflowName}-${stepName}.md`)).isFile()
    ) {
      return `riglane-${workflowName}-${stepName}`;
    }
  } catch (e) {
...
export function toolStepCollectResult(
  args: StepCollectResultInput,
  paths: EnginePaths = defaultPaths(),
  overrides?: EngineRunOverrides,
): StepCollectResultOutput {
  const workflowName =
    (args.name as unknown as string) ||
    ((args as unknown as Record<string, unknown>).workflow_name as string);
  const stepName: string = ((args.step as unknown as string | undefined) ||
    ((args as unknown as Record<string, unknown>).step_name as string | undefined) ||
    '') as string;
  ...
  const _perStepResultPath = stepName
    ...
    : null;
  const resultPath =
    _perStepResultPath !== null && existsSync(_perStepResultPath)
      ? _perStepResultPath
      : _legacyResultPath;
  const inlineWritePath = _perStepResultPath ?? _legacyResultPath;
  const manifestPath = join(runtimeDir, 'manifest.json');

  let ownerManifest: Record<string, unknown> | null = null;
  try {
    ownerManifest = JSON.parse(readFileSync(manifestPath, 'utf-8')) as Record<string, unknown>;
  } catch {
    ownerManifest = null;
  }
  if (ownerManifest !== null) {
    const _ownEntry = checkRunOwnership(ownerManifest, 'step_collect_result');
    if (_ownEntry) return _ownEntry as unknown as StepCollectResultOutput;
  }

  if (stepName) {
    let wfCheck: Workflow;
    if (yamlPathOverride !== null) {
      try {
        wfCheck = loadYaml<Workflow>(yamlPathOverride);
      } catch {
        wfCheck = { steps: [] } as unknown as Workflow;
      }
    } else {
      ({ workflow: wfCheck } = resolveWorkflow(workflowName, paths));
    }
    const sObj = findStepConfig(wfCheck, stepName) as
      | { name?: string; subagent?: boolean }
      | null;
  ...
  if (!resultExists) {
    return runInlineGateValidation(
      workflowName,
      stepName,
      runtimeDir,
      manifestPath,
  ...
  const gateResult = JSON.parse(readFileSync(resultPath, 'utf-8')) as Record<string, unknown>;
  const manifest: Record<string, unknown> =
    ownerManifest ?? (JSON.parse(readFileSync(manifestPath, 'utf-8')) as Record<string, unknown>);
  ...
  const manifestToken = (manifest.run_token as string | undefined) ?? '';
  const resultToken = (gateResult.run_token as string | undefined) ?? '';
  ...
  const resultStep = (gateResult.step as string | undefined) ?? '';
  ...
  let wf2: Workflow;
  let wf2DefnDir: string;
  if (yamlPathOverride !== null) {
    wf2 = loadYaml<Workflow>(yamlPathOverride);
    wf2DefnDir = dirname(yamlPathOverride);
  } else {
    ({ workflow: wf2, definitionDir: wf2DefnDir } = resolveWorkflow(workflowName, paths));
  }
  const wfGateLookup: Record<string, unknown> = (wf2.gate ?? {}) as Record<string, unknown>;
  const _collectStepConfig: Record<string, unknown> | null = findStepConfig(
    wf2,
    stepName || resultStep,
  );
  const stepGateLookup: Record<string, unknown> =
    (_collectStepConfig?.gate as Record<string, unknown> | undefined) ?? {};

  const passed = Boolean(gateResult.passed);

  if (passed) {
    const semantic = stepGateFlag(wfGateLookup, stepGateLookup, 'semantic');
    const humanEval = evaluateHumanGate(
      wfGateLookup,
      stepGateLookup,
      stepName || resultStep,
      runtimeDir,
    ...
    const result: Record<string, unknown> = {
      ...
      step: resultStep,
      action: 'PROCEED',
      checks: (gateResult.checks as number | undefined) ?? 0,
      details: (gateResult.details as readonly string[] | undefined) ?? [],
      needs_semantic_gate: semantic,
      needs_human_gate: human,
    };
    if ('branches' in gateResult) result.branches = gateResult.branches;
    if ('branch_results' in gateResult) result.branch_results = gateResult.branch_results;
    if ('failed_branches' in gateResult) result.failed_branches = gateResult.failed_branches;

    if (semantic) {
      const isPlanning = _collectStepConfig?.type === 'planning';
      const stepConfigForSpec: Record<string, unknown> = _collectStepConfig ?? {};
      const specClause = composeSpecClauseForSemanticGate(stepConfigForSpec, resolveConsumptionDomains(paths), resolveConsumptionScopeHint(paths));
      if (isPlanning) {
    ...
    if (human) {
      result.engine_instructions = appendHumanGateInstructions(
        result.engine_instructions,
        resolveHumanChannel(wfGateLookup, stepGateLookup),
      );
    }

    return result as unknown as StepCollectResultOutput;
  }
  ...
  const failedBranchesForRetry = (gateResult.failed_branches as unknown[] | undefined) ?? [];
  ...
  if (hostToolCaps(getEngineHost()).gateRetryModel === 'fresh-spawn-gate-budget') {
    ...
    const gateLoopCount = (gateResult.loop_count as number | undefined) ?? 0;
    if (gateLoopCount < maxGateRetries) {
      return {
        ...
        action: 'RETRY_STEP',
        checks: (gateResult.checks as number | undefined) ?? 0,
        failures: (gateResult.failures as number | undefined) ?? 0,
        details: (gateResult.details as readonly string[] | undefined) ?? [],
          ...
          getEngineHost() === 'gemini'
            ? parallelFail
              ...
              : `GATE RETRY (${gateLoopCount + 1}/${maxGateRetries}): re-invoke the invoke_agent ` +
                `tool with agent_name '${geminiSubagentTypeFor(
                  workflowName,
                  stepName || resultStep,
                  dirname(paths.agentDir),
                )}' and a NEW prompt carrying the gate feedback (the block reason you received, ` +
            ...
            : parallelFail
              ...
              : `IN-SESSION GATE RETRY (${gateLoopCount + 1}/${maxGateRetries}): re-invoke the ` +
                `task tool with subagent_type '${opencodeSubagentTypeFor(
                  workflowName,
                  stepName || resultStep,
                  dirname(paths.agentDir),
                )}' AND the SAME task_id from the previous task result — BOTH parameters are ` +
  ...
  if (stepRetryCount > maxStepRetries) {
    autoFailBudgetExhaustedRun(
      ...
      stepName || resultStep,
      manifestPath,
      `Step '${stepName || resultStep}' exhausted retry budget ` +
        `(${stepRetryCount}/${maxStepRetries}).`,
      overrides ?? {},
      paths,
    );
    return {
      ...
      failures: (gateResult.failures as number | undefined) ?? 0,
      details: (gateResult.details as readonly string[] | undefined) ?? [],
      loop_count: (gateResult.loop_count as number | undefined) ?? 0,
      max_step_retries: maxStepRetries,
      step_retry_count: stepRetryCount,
      run_failed: true,
...
import type {
  StepCompleteInput,
  StepCompleteOutput,
  WorkflowFinalizeInput,
  WorkflowFinalizeOutput,
} from '../types/mcp.js';
import { loadSnapshot, validateOutputs } from './output-validator.js';

function moveTreeContents(src: string, dst: string): void {
  let entries: Array<{ name: string; isDirectory: () => boolean }>;
  try {
    entries = readdirSync(src, { withFileTypes: true }) as unknown as Array<{
      name: string;
      isDirectory: () => boolean;
    }>;
  } catch {
...
function checkStepFailureBlocked(
  gateResult: Record<string, unknown>,
  stepConfig: Record<string, unknown> | null,
  workflow: Record<string, unknown>,
  manifest?: Record<string, unknown> | null,
): { blocked: boolean; reason: string; failedBranches: unknown[] } {
  const isParallel = Boolean(stepConfig?.parallel);
  const wfGate = ((workflow.gate as Record<string, unknown> | undefined) ?? {}) as Record<
    string,
    unknown
  >;
  const stepGate = ((stepConfig?.gate as Record<string, unknown> | undefined) ?? {}) as Record<
    string,
    unknown
  >;
  ...
  const allowPartialRaw: unknown = stepHas
  ...
  if (isParallel) {
    const branchesMap = (gateResult.branches ?? null) as Record<
      string,
      { passed?: boolean } | undefined
    > | null;
    let reportedKeys: Set<string>;
    let failedBranches: unknown[];
    if (branchesMap && typeof branchesMap === 'object') {
      ...
      const branchResults = (gateResult.branch_results ?? []) as ReadonlyArray<
        Record<string, unknown>
      >;
      reportedKeys = new Set<string>();
      for (const r of branchResults) {
        const idx = r.branch_index;
        if (idx !== undefined && idx !== null) reportedKeys.add(String(idx));
      }
      failedBranches = [...((gateResult.failed_branches ?? []) as ReadonlyArray<unknown>)];
    }
...
function readGateResultForStep(
  runtimeDir: string,
  stepName: string,
): Record<string, unknown> | null {
  for (const gateResultPath of [
    ...
    if (!exists) continue;
    try {
      const gr = JSON.parse(readFileSync(gateResultPath, 'utf-8')) as Record<string, unknown>;
      if (gr.step !== stepName) continue;
      return gr;
    } catch {
      continue;
    }
...
function deciderEnv(runDir: string): NodeJS.ProcessEnv {
  return {
    ...process.env,
    [ENV_RUN_DIR]: runDir,
...
interface LoopBackEvalResult {
  readonly kind: 'loop' | 'proceed' | 'awaiting' | 'error';
  readonly decidedBy?: 'script' | 'orchestrator' | 'budget_exhausted';
  readonly rationale?: string;
  readonly scriptSaysLoop?: boolean;
  readonly message?: string;
}

function evaluateLoopBack(
  loopBack: {
    readonly to: string;
    readonly max_iterations: number;
    readonly when: { readonly script?: string; readonly semantic?: string; readonly human?: boolean };
  },
  loopState: { iterations?: number; pending?: { script_says_loop?: boolean } | null },
  args: { readonly loop_decision?: string; readonly loop_rationale?: string },
  runDir: string,
): LoopBackEvalResult {
  ...
  if (typeof loopBack.when.script === 'string' && loopBack.when.script.length > 0) {
    ...
    if (typeof cached === 'boolean') {
      ...
      try {
        const parsed = JSON.parse((proc.stdout ?? '').trim()) as { loop?: unknown };
        if (typeof parsed.loop !== 'boolean') throw new Error('missing boolean "loop" field');
  ...
  const needsOrchestrator =
    (typeof loopBack.when.semantic === 'string' && loopBack.when.semantic.length > 0) ||
...
function composeLoopDecisionInstructions(
  stepName: string,
  loopBack: {
    readonly to: string;
    readonly when: {
      ...
      readonly human_channel?: string;
    };
  },
  iteration: number,
  scriptSaysLoop: boolean | undefined,
): string {
  const parts: string[] = [];
  parts.push(
    `LOOP DECISION REQUIRED for step '${stepName}' (loop_back → '${loopBack.to}', ` +
  ...
  if (typeof loopBack.when.semantic === 'string' && loopBack.when.semantic.length > 0) {
    parts.push(
      `EVALUATE this condition against the step's actual outputs (read the files): ` +
  ...
  if (loopBack.when.human === true) {
    ...
    if (ch === 'external' || ch === 'both') {
      parts.push(
        `ASK THE USER through the run inbox: first call inbox(op:'rules', name, step: ` +
      ...
      parts.push(
        `ASK THE USER: should the workflow loop back to step '${loopBack.to}' for another ` +
          `iteration? Relay their answer — do not decide for them.`,
      );
    }
  }
  parts.push(
    `All deciders must agree to loop; any 'proceed' wins. Then call step_complete AGAIN ` +
...
interface RouteEvalResult {
  readonly kind: 'route' | 'proceed' | 'awaiting' | 'error';
  readonly routeId?: string;
  readonly decidedBy?: 'script' | 'orchestrator';
  readonly rationale?: string;
  readonly note?: string;
  readonly scriptRoute?: string;
  readonly message?: string;
}
...
function evaluateRoutes(
  routes: {
    readonly when: { readonly script?: string; readonly semantic?: string; readonly human?: boolean };
    readonly define: ReadonlyArray<{ readonly id: string }>;
  },
  routeState: { pending?: { script_route?: string } | null },
  args: { readonly route_decision?: string; readonly route_rationale?: string },
  runDir: string,
): RouteEvalResult {
  const validIds = new Set(routes.define.map((r) => r.id));
  const isValidChoice = (v: string): boolean => v === 'proceed' || validIds.has(v);
  const idList = [...validIds].map((i) => `'${i}'`).join(', ');

  let scriptRoute: string | undefined;
  if (typeof routes.when.script === 'string' && routes.when.script.length > 0) {
    ...
    if (typeof cached === 'string') {
      ...
      try {
        const parsed = JSON.parse((proc.stdout ?? '').trim()) as { route?: unknown };
        if (typeof parsed.route !== 'string') throw new Error('missing string "route" field');
  ...
  if (!hasHuman && scriptRoute !== undefined && choice !== scriptRoute) {
    return {
      ...
      note:
        `Routing deciders disagreed for this step — the script chose '${scriptRoute}' but the ` +
...
function composeRouteDecisionInstructions(
  stepName: string,
  routes: {
    readonly when: {
      readonly script?: string;
  ...
  const parts: string[] = [];
  parts.push(
    `ROUTE DECISION REQUIRED for step '${stepName}'. Choose one of: ${ids}, or 'proceed' ` +
  ...
  if (hasHuman) {
    ...
    if (ch === 'external' || ch === 'both') {
      ...
      parts.push(
        `ASK THE USER which route to take (${ids} or 'proceed'). ` +
          (hasScript || hasSemantic
  ...
  if (!hasHuman && hasScript && hasSemantic) {
    parts.push(
      `Without a human decider, the script and your semantic evaluation must AGREE on the same ` +
        `value; if they differ the engine takes NO route and continues.`,
    );
  }
  parts.push(
    `Then call step_complete AGAIN for step '${stepName}' with the same summary plus ` +
...
function unattributedLedgerToolNames(runtimeDir: string): Set<string> {
  ...
  for (const line of raw.split('\n')) {
    ...
    try {
      ...
      if (rec.step !== null && rec.step !== undefined) continue;
      const full = (rec.tool as string | undefined) || '';
      const short =
        (rec.tool_short as string | undefined) || (full ? (full.split('__').pop() ?? '') : '');
      if (full) names.add(full);
      if (short) names.add(short);
      const wfTool = rec.wf_tool as string | undefined;
      if (wfTool) names.add(wfTool);
      const server = rec.server as string | undefined;
      if (server && rec.kind === 'mcp') names.add(`mcp-server:${server}`);
    } catch {
    }
  }
  return names;
...
function consumeToolEvents(
  ...
  let raw: string;
  try {
    if (!statSync(ledgerPath).isFile()) return [];
  ...
  const wfNameForAttribution = (manifest.workflow as string | undefined) ?? '';
  const manifestStepsForAttribution = (manifest.steps ?? {}) as Record<string, unknown>;
  const attributeStep = (rec: Record<string, unknown>): void => {
    ...
    if (at && wfNameForAttribution && at.startsWith(`${wfNameForAttribution}-`)) {
      ...
      if (dd > 0) cand = cand.slice(0, dd);
      if (cand && cand in manifestStepsForAttribution) {
        rec.step = cand;
        return;
      }
    }
    if (stepName !== null) rec.step = stepName;
  };
  const fresh: Array<Record<string, unknown>> = [];
  for (let i = offset; i < lines.length; i++) {
    try {
      const rec = JSON.parse(lines[i] as string) as Record<string, unknown>;
      attributeStep(rec);
      fresh.push(rec);
    } catch {
    }
  ...
  try {
    ...
    const claimed = claimSpooledEvents(
      agentDir,
      (manifest.run_id as string | undefined) ?? '',
      (manifest.run_token as string | undefined) ?? null,
    );
...
function appendToolCallsToTrace(
  runtimeDir: string,
  fresh: ReadonlyArray<Record<string, unknown>>,
): void {
  ...
  withTraceLock(tracePath, () => {
    try {
      const trace = JSON.parse(readFileSync(tracePath, 'utf-8')) as Record<string, unknown>;
      const existing = Array.isArray(trace.tool_calls) ? (trace.tool_calls as unknown[]) : [];
      ...
      logEngine(
        `appendToolCallsToTrace skipped ${fresh.length} event(s): ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  });
}

interface HostToolCaps {
  readonly scriptReliable: boolean;
  readonly mcpReliable: boolean;
  readonly mcpPrefixed: boolean;
  readonly gateRetryModel: 'host-bounce' | 'fresh-spawn-gate-budget';
}
const HOST_TOOL_CAPS: Record<Host, HostToolCaps> = {
...
const CC_FALLBACK_CAPS: HostToolCaps = { scriptReliable: true, mcpReliable: true, mcpPrefixed: true, gateRetryModel: 'host-bounce' };
function hostToolCaps(host: Host | null): HostToolCaps {
  return host === null ? CC_FALLBACK_CAPS : HOST_TOOL_CAPS[host];
}

function reclassifyScriptTools(
  events: Array<Record<string, unknown>>,
  workflow: Record<string, unknown>,
  workflowName: string,
): void {
  if (events.length === 0) return;
  const wfTools = (workflow.tools ?? []) as Array<Record<string, unknown>>;
  const wfNorm = normalizeForMcp((workflow.name as string | undefined) ?? workflowName);
  const registered = new Map<string, string>();
  for (const t of wfTools) {
    if (((t.type as string | undefined) ?? 'script') !== 'script') continue;
    const name = t.name as string | undefined;
    if (name) registered.set(`${wfNorm}__${normalizeForMcp(name)}`, name);
  }
  if (registered.size === 0) return;
  for (const e of events) {
    if (e.kind === 'script') continue;
    const clean = registered.get((e.tool as string | undefined) ?? '');
    if (clean) {
      e.kind = 'script';
      e.tool_short = clean;
      if (e.server === undefined || e.server === null) e.server = 'workflow_tools';
    }
  }
}

function reclassifyMcpTools(
  events: Array<Record<string, unknown>>,
  workflow: Record<string, unknown>,
): void {
  if (events.length === 0) return;
  ...
  for (const t of wfTools) {
    if ((t.type as string | undefined) !== 'mcp') continue;
    const name = t.name as string | undefined;
    const et = t.expected_tools;
    if (!name || !Array.isArray(et)) continue;
  ...
  for (const e of events) {
    ...
    const server = expected.get(normalizeForMcp((e.tool as string | undefined) ?? ''));
    if (server) {
      e.kind = 'mcp';
      e.server = server;
    }
...
function scriptCommandNeedle(command: string): string {
...
function shellEventCommand(e: Record<string, unknown>): string {
  const args = e.args;
...
function reclassifyBashRunScriptTools(
  events: Array<Record<string, unknown>>,
  workflow: Record<string, unknown>,
): void {
  if (events.length === 0) return;
  ...
  for (const t of wfTools) {
    if (((t.type as string | undefined) ?? 'script') !== 'script') continue;
    const name = t.name as string | undefined;
    const command = t.command as string | undefined;
    if (!name || !command) continue;
    const needle = scriptCommandNeedle(command);
    if (needle) needles.push({ needle, name });
  }
  if (needles.length === 0) return;
  for (const e of events) {
    if (e.kind !== 'shell' || e.wf_tool) continue;
...
function readValuePreview(absPath: string): ResolvedFilePreview {
  const display = relpathPosix(absPath);
  let size: number;
  try {
    const st = statSync(absPath);
    if (!st.isFile()) return { path: display, exists: false };
    size = st.size;
  } catch {
  ...
  if (slice.includes(0)) return { path: display, exists: true, size, binary: true, value_preview: null };
  let text = slice.toString('utf-8');
  let truncated = bytesRead > MAX_IO_PREVIEW_BYTES || size > bytesRead;
  if (truncated) text = slice.subarray(0, MAX_IO_PREVIEW_BYTES).toString('utf-8');
...
function resolveInputAbsPaths(
  ...
  if (isDottedRoot(resolved) || pathIsAbsolute(resolved)) {
    if (hasGlob) return [...globSync(resolved, { windowsPathsNoEscape: true })].sort();
...
function enrichStepIO(
  runtimeDir: string,
  stepName: string,
  stepConfig: Record<string, unknown>,
  params: Record<string, unknown>,
  defnDir: string,
  bindingsApplied: Record<string, unknown>,
  paramBindings: Record<string, string> | undefined,
): void {
  const normStr = (v: unknown): string => (typeof v === 'string' ? v : '');
  const inputsOut = ((stepConfig.inputs ?? []) as ReadonlyArray<unknown>).map((raw) => {
    const isObj = raw !== null && typeof raw === 'object' && !Array.isArray(raw);
    const inp = (isObj ? raw : {}) as Record<string, unknown>;
    const rawPath = isObj ? normStr(inp.path) : String(raw);
    const entry: Record<string, unknown> = { path: rawPath, inject: normStr(inp.inject) || 'reference' };
    if (inp.struct) entry.struct = inp.struct;
    entry.resolved = resolveInputAbsPaths(rawPath, params, runtimeDir, defnDir).map(readValuePreview);
    return entry;
  });
  const outputsOut = ((stepConfig.outputs ?? []) as ReadonlyArray<unknown>).map((raw) => {
    const isObj = raw !== null && typeof raw === 'object' && !Array.isArray(raw);
    const out = (isObj ? raw : {}) as Record<string, unknown>;
    const rawPath = isObj ? normStr(out.path) : String(raw);
    const entry: Record<string, unknown> = { path: rawPath };
    if (out.struct) entry.struct = out.struct;
    if (out.write_proof) entry.write_proof = out.write_proof;
    if (out.optional) entry.optional = out.optional;
    if (out.from_delegated) entry.from_delegated = out.from_delegated;
  ...
  const bindings: Record<string, { expr: string; value: unknown }> = {};
  if (paramBindings) {
  ...
  withTraceLock(tracePath, () => {
    try {
      ...
      const entry = (trace.steps ?? []).find(
        (s) => s.name === stepName && s.loop_archived !== true,
      );
      if (!entry) return;
      entry.inputs = inputsOut;
      entry.outputs = outputsOut;
      if (Object.keys(bindings).length > 0) entry.param_bindings = bindings;
      safeWriteJson(tracePath, trace);
    } catch {
    }
  });
...
export function toolStepComplete(
  args: StepCompleteInput,
  paths: EnginePaths = defaultPaths(),
  overrides?: EngineRunOverrides,
): StepCompleteOutput {
  const workflowName =
  ...
  if (yamlPathOverride !== null) {
    try {
      ...
      } as unknown as StepCompleteOutput;
    }
    defnDir = dirname(yamlPathOverride);
  } else {
    ({ definitionDir: defnDir, workflow } = resolveWorkflow(workflowName, paths));
  ...
  if (_ownComplete) return _ownComplete as unknown as StepCompleteOutput;
  ...
  if (_bug077Gr !== null) {
    ...
    if (_check.blocked) {
      ...
      return {
        ...
        action: 'BLOCKED_PARTIAL_FAILURE',
        step: stepName,
        blocked_reason: _check.reason as
          | 'all_branches_failed'
          | 'missing_branches'
  ...
  {
    const wfGateEnf = (workflow.gate ?? {}) as Record<string, unknown>;
    const stepGateEnf = ((stepConfig?.gate as Record<string, unknown> | undefined) ?? {});
    const humanEnfEval = evaluateHumanGate(wfGateEnf, stepGateEnf, stepName, runtimeDir);
    if ('error' in humanEnfEval) {
      return { error: humanEnfEval.error, action: 'STOP_WORKFLOW' } as unknown as StepCompleteOutput;
    }
    ...
    if (humanEnf && channelEnf !== 'terminal') {
      const startedAtRaw = (manifest.steps?.[stepName] as { started_at?: string } | undefined)
        ?.started_at;
      ...
      if (passMsgs.length === 0 || unanswered.length > 0) {
        ...
        return {
          error:
            `Cannot complete step '${stepName}': its human gate uses the external channel ` +
            `('${channelEnf}') and ` +
            (passMsgs.length === 0
          ...
          step: stepName,
          ...(unanswered.length > 0
            ? { message_id: (unanswered[0] as { message_id: string }).message_id }
            : {}),
          engine_instructions:
            passMsgs.length === 0
      ...
      const rejected = passMsgs.find(
        (m) =>
          !Array.isArray((m as { items?: unknown }).items) &&
          (m.response as { type?: string } | null)?.type === 'reject',
      );
      if (rejected) {
        const feedback = String(
          (rejected.response as { text?: string } | null)?.text ?? '',
        ).slice(0, 300);
        return {
          error:
            `Cannot complete step '${stepName}': the user REJECTED message ` +
          ...
          engine_instructions:
            `The user refused. Re-run step '${stepName}' applying their feedback` +
            (feedback ? ` ("${feedback}")` : '') +
  ...
  const stepParamBindings = stepConfig?.param_bindings as Record<string, string> | undefined;
  const handoffEntries = (
    (stepConfig?.outputs ?? []) as ReadonlyArray<Record<string, unknown> | string>
  ).filter(
    (o): o is Record<string, unknown> =>
      typeof o === 'object' && o !== null && typeof o.from_delegated === 'string',
  );
  let delegatedBaseDir: string | null = null;
  let delegationWarning: string | null = null;
  let resolvedChildRunId: string | null = null;
  const handoffApplied: Array<{ path: string; from: string }> = [];
  if (isDelegationStep && stepConfig && (stepParamBindings || handoffEntries.length > 0)) {
    ...
    const explicitId = (args as { delegated_run_id?: string }).delegated_run_id;
    let linkedId: string | null = null;
    if (explicitId) {
      try {
        const em = JSON.parse(
          readFileSync(join(runDir(paths.agentDir, explicitId), 'manifest.json'), 'utf-8'),
        ) as Record<string, unknown>;
    ...
    if (!linkedId) {
      const stampedId = (
        (manifest.steps[stepName] as Record<string, unknown> | undefined)?.delegation as
          | { child_run_id?: string | null }
          | undefined
      )?.child_run_id;
      if (typeof stampedId === 'string' && stampedId.length > 0) linkedId = stampedId;
    }
    const resolvedChild = resolveDelegatedRunDir(
      paths,
      delegatedName,
      linkedId,
      (manifest.run_id as string | undefined) ?? null,
      stepName,
    );
    if (resolvedChild.source === 'unresolved') {
      const refusal =
        resolvedChild.candidates > 0
      ...
      if (resolvedChild.source === 'backlink') {
        logEngine(
          `delegation resolution: child for step '${stepName}' resolved via reverse linkage ` +
            `(backlink scan) → ${resolvedChildRunId}.`,
        );
      }
      try {
        const cm = JSON.parse(
          readFileSync(join(delegatedBaseDir, 'manifest.json'), 'utf-8'),
        ) as { status?: string; stopped?: { step?: string; reason?: string } };
        if (cm.status && cm.status !== 'completed') {
          const stoppedNote = cm.stopped
            ? ` (stopped at step '${cm.stopped.step ?? '?'}': ${cm.stopped.reason ?? 'unknown'})`
            : '';
          const statusNote =
            `Delegated child run '${resolvedChildRunId}' is '${cm.status}'${stoppedNote} — ` +
    ...
    if (delegatedBaseDir === null) {
      return {
        error:
          `Delegation resolution failed for step '${stepName}': ` +
    ...
    if (handoffEntries.length > 0) {
      if (delegatedBaseDir === null) {
        return {
          error:
            `Delegation artifact handoff failed for step '${stepName}': ` +
            (delegationWarning ?? 'delegated child run not resolved'),
          action: 'FIX_AND_RETRY',
          step: stepName,
          validation_errors: [delegationWarning ?? 'delegated child run not resolved'],
      ...
      try {
        ...
        childParams = (cm.params ?? {}) as Record<string, unknown>;
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        return {
          error:
            `Delegation artifact handoff failed for step '${stepName}': the delegated ` +
      ...
      for (const entry of handoffEntries) {
        ...
        if (matches.length === 0) {
          if (entry.optional) {
            logEngine(
              `delegation handoff: optional from_delegated '${srcResolved}' not found in child run ` +
                `'${resolvedChildRunId ?? delegatedName}' — skipped.`,
            );
            continue;
          }
          handoffErrors.push(
            `from_delegated '${srcResolved}' matched no file in the delegated child's run dir ` +
        ...
        if (matches.length > 1) {
          handoffErrors.push(
            `from_delegated '${srcResolved}' matched ${matches.length} files (need exactly 1): ` +
              matches.map((m) => toPosixSlashes(relative(delegatedBaseDir as string, m))).join(', ') +
              `. Narrow the pattern.`,
          );
          continue;
        }
        let destRel: string;
        try {
          destRel = resolveConcreteOutputPath(
            String(entry.path ?? ''),
            (manifest.params ?? {}) as Record<string, unknown>,
          );
        } catch (e) {
        ...
        const destAbs =
          isAbsolute(destRel) || isDottedRoot(destRel) ? destRel : join(runtimeDir, destRel);
        try {
          mkdirSync(dirname(destAbs), { recursive: true });
          copyFileSync(matches[0] as string, destAbs);
        } catch (e) {
          const msg = e instanceof Error ? e.message : String(e);
        ...
        handoffApplied.push({
          path: destRel,
          from: toPosixSlashes(relative(delegatedBaseDir, matches[0] as string)),
        });
      }
  ...
  if (
    ...
    if (gateResultExists) {
      try {
        const gr = JSON.parse(readFileSync(gateResultPath, 'utf-8')) as Record<string, unknown>;
        if (gr.step === stepName && gr.passed === true) gateAlreadyRan = true;
      } catch {
      }
    }
    ...
    if (!gateAlreadyRan) {
      ...
      const validation = validateOutputs(
        stepConfig.outputs as unknown as Parameters<typeof validateOutputs>[0],
        defnDir,
        runtimeDir,
        { snapshot: snap, stepStartedAt: startedAt, params: manifest.params as Record<string, unknown> | null },
      );
      ...
      const isParallelStep =
        Boolean(stepConfig.parallel) || Boolean(manifest.parallel_branches);
      const optionalOutputs = (stepConfig.outputs as Array<Record<string, unknown> | string>).filter(
        (o): o is Record<string, unknown> =>
          typeof o === 'object' && o !== null && Boolean((o as { optional?: unknown }).optional),
      );
      if (!isParallelStep && optionalOutputs.length > 0) {
        ...
        const startedAt =
          ...
          null;
        const validation = validateOutputs(
          optionalOutputs as unknown as Parameters<typeof validateOutputs>[0],
          defnDir,
          runtimeDir,
          { snapshot: snap, stepStartedAt: startedAt, params: manifest.params as Record<string, unknown> | null },
        );
        if (!validation.passed) {
          return {
            error:
              `Late-flushed optional output failed validation for step '${stepName}' ` +
  ...
  let branchMergeInfo: { branches: number; incomplete?: string[] } | null = null;
  if (manifest.parallel_branches && stepConfig?.parallel) {
    const branchCount = Object.keys(manifest.parallel_branches).length;
    const mergeSrcs = new Set<string>();
    const moveScaffold = (branchSrc: string, destDir: string): void => {
      let isDir = false;
      try {
    ...
    const mergePass = (): void => {
      for (const [branchKey, branchInfoRaw] of Object.entries(manifest.parallel_branches ?? {})) {
        const branchInfo = branchInfoRaw as {
          resolved_outputs?: ReadonlyArray<{ working?: string; scaffolded?: boolean }>;
        };
        const scaffold = `_branch_${branchKey}`;
        const resolved = branchInfo.resolved_outputs;
        if (resolved && resolved.length > 0) {
          for (const ro of resolved) {
            if (!ro.scaffolded || !ro.working) continue;
            const wsegs = ro.working.split('/');
            const idx = wsegs.indexOf(scaffold);
            if (idx < 0) continue;
            moveScaffold(
              join(runtimeDir, ...wsegs.slice(0, idx + 1)),
              idx > 0 ? join(runtimeDir, ...wsegs.slice(0, idx)) : runtimeDir,
            );
          }
        } else {
    ...
    branchMergeInfo = {
      ...
      ...(leftovers.length > 0
        ? {
            incomplete: leftovers.map((s) =>
              s.startsWith(runtimeDir) ? s.slice(runtimeDir.length + 1).replace(/\\/g, '/') : s,
            ),
          }
        : {}),
    };
    if (leftovers.length > 0) {
      // eslint-disable-next-line no-console
      console.error(
        `[workflow-engine] Warning: parallel branch merge INCOMPLETE for step ` +
  ...
  if (!(stepName in manifest.steps)) manifest.steps[stepName] = {};
  const stepRecord = manifest.steps[stepName] as Record<string, unknown>;
  stepRecord.status = 'completed';
  stepRecord.completed_at = now;
  stepRecord.summary = summary;
  manifest.updated_at = now;

  const stepStarted = stepRecord.started_at as string | undefined;
  if (stepStarted) {
    try {
  ...
  reclassifyScriptTools(freshToolEvents, workflow as unknown as Record<string, unknown>, workflowName);
  reclassifyBashRunScriptTools(freshToolEvents, workflow as unknown as Record<string, unknown>);
  reclassifyMcpTools(freshToolEvents, workflow as unknown as Record<string, unknown>);
  {
    const msteps = (manifest.steps ?? {}) as Record<string, unknown>;
    for (const c of freshToolEvents) {
      const owner = typeof c.step === 'string' ? c.step : '';
      if (!owner || owner === stepName || !(owner in msteps)) continue;
      const rec = (msteps[owner] ?? {}) as Record<string, unknown>;
      const credited = new Set<string>(
        Array.isArray(rec.tools_called) ? (rec.tools_called as string[]) : [],
      );
      const full = (c.tool as string | undefined) || '';
      const short =
        (c.tool_short as string | undefined) || (full ? (full.split('__').pop() ?? '') : '');
      if (full) credited.add(full);
      if (short) credited.add(short);
      const wfTool = c.wf_tool as string | undefined;
      if (wfTool) credited.add(wfTool);
      rec.tools_called = [...credited];
      if (c.kind === 'mcp') {
        const servers = new Set<string>(
          Array.isArray(rec.mcp_servers_called) ? (rec.mcp_servers_called as string[]) : [],
        );
        const server =
          (typeof c.server === 'string' && c.server) ||
  ...
  const stepToolWarnings: Array<Record<string, unknown>> = [];
  let toolVerificationRan = false;
  const isSubagentStep = stepConfig?.subagent !== false;
  if (stepConfig && isSubagentStep) {
    const wfTools = (workflow.tools ?? []) as unknown as Array<Record<string, unknown>>;
    const stepToolsFilter = stepConfig.tools as readonly string[] | undefined;
    if (wfTools.length > 0 && stepToolsFilter && stepToolsFilter.length > 0) {
      ...
      const expectedTools = wfTools.filter(
        (t) => t.name !== undefined && stepToolsFilter.includes(t.name as string),
      );
      if (expectedTools.length > 0) {
        ...
        const calledTools = new Set<string>(
          Array.isArray(stepRecord.tools_called) ? (stepRecord.tools_called as string[]) : [],
        );
        for (const c of stepCalls) {
          const full = (c.tool as string | undefined) || '';
          const short =
            (c.tool_short as string | undefined) || (full ? (full.split('__').pop() ?? '') : '');
          if (full) calledTools.add(full);
          if (short) calledTools.add(short);
          const wfTool = c.wf_tool as string | undefined;
          if (wfTool) calledTools.add(wfTool);
        }
        stepRecord.tools_called = [...calledTools];
        const mcpSeen = new Set<string>(
          Array.isArray(stepRecord.mcp_servers_called)
            ? (stepRecord.mcp_servers_called as string[])
            : [],
        );
        ...
        for (const tool of expectedTools) {
          ...
          const ttype = (tool.type as string | undefined) ?? 'script';
          if (ttype === 'script') {
            const wfNameNorm = normalizeForMcp(
              (workflow.name as unknown as string) ?? workflowName,
            );
            ...
            const mcpCalled =
              ...
              stepCalls.some(
                (c) =>
                  c.kind === 'mcp' &&
                  (((c.tool as string | undefined) ?? '').startsWith(mcpPrefix) ||
                    c.server === tname),
              );
            if (mcpCalled) mcpSeen.add(tname);
            const requiredVal = 'required' in tool ? tool.required : true;
            const hasExpected =
              Array.isArray(tool.expected_tools) && tool.expected_tools.length > 0;
            const canDetect = caps.mcpPrefixed || hasExpected;
            const mcpEvidence =
              unattributed.has(`mcp-server:${tname}`) ||
  ...
  const bindingsApplied: Record<string, unknown> = {};
  const paramBindings = stepParamBindings;
  if (isDelegationStep) _pendingDelegation = null;
  if (paramBindings) {
    for (const [paramName, bindingExpr] of Object.entries(paramBindings)) {
      try {
        ...
        const data = JSON.parse(readFileSync(absPath, 'utf-8')) as unknown;
        const value = readJsonField(data, fieldPath);
        if (!manifest.params) manifest.params = {};
  ...
  if (stepConfig) {
    enrichStepIO(
      ...
      defnDir,
      bindingsApplied,
      stepConfig.param_bindings as Record<string, string> | undefined,
    );
  }
  ...
  const stepOrder = (laneCtxOfStep
    ...
    : resolveActiveSequence(workflow, manifest)
  ).map((s) => s.name as string);
  const loopBackCfg = stepConfig?.loop_back as
    | {
        readonly to: string;
        readonly max_iterations: number;
        readonly when: {
  ...
  if (loopBackCfg && typeof loopBackCfg === 'object') {
    const stepData = (manifest.steps[stepName] ?? {}) as Record<string, unknown>;
    const loopState = (stepData.loop_state ?? { iterations: 0 }) as {
      iterations?: number;
      pending?: { script_says_loop?: boolean; asked_at?: string } | null;
      resolved_for_pass?: number;
      [key: string]: unknown;
    };
    const iterations = typeof loopState.iterations === 'number' ? loopState.iterations : 0;
    const loopResolvedThisPass =
      loopState.resolved_for_pass === iterations && !loopState.pending;
    const evalResult: LoopBackEvalResult = loopResolvedThisPass
      ? { kind: 'proceed' }
      : evaluateLoopBack(
          loopBackCfg,
          loopState,
          args as { loop_decision?: string; loop_rationale?: string },
          runtimeDir,
        );
    ...
    if (evalResult.kind === 'error') {
      return {
        error:
          `loop_back evaluation failed for step '${stepName}': ${evalResult.message}` +
    ...
    if (evalResult.kind === 'awaiting') {
      ...
      manifest.steps[stepName] = stepData;
      safeWriteJson(manifestPath, manifest);
      const runIdAwait = (manifest.run_id as string | undefined) ?? '';
      if (runIdAwait) flushTraceSnapshot(runtimeDir, runIdAwait, manifest, stepName, workflow);
      ...
      return {
        ...
        engine_instructions: composeLoopDecisionInstructions(
          stepName,
          loopBackCfg,
          iterations,
          evalResult.scriptSaysLoop ??
            (loopState.pending as { script_says_loop?: boolean }).script_says_loop,
        ),
      } as unknown as StepCompleteOutput;
    }
    ...
    if (!loopResolvedThisPass) {
      ...
      if (evalResult.rationale !== undefined) loopState.last_rationale = evalResult.rationale;
      if (evalResult.kind === 'proceed') loopState.resolved_for_pass = iterations;
    }
    ...
    if (evalResult.kind === 'loop') {
      ...
      const runIdLoop = (manifest.run_id as string | undefined) ?? '';
      if (runIdLoop) {
        flushTraceSnapshot(runtimeDir, runIdLoop, manifest, stepName, workflow);
        recordLoopTraceEvent(
          runtimeDir,
          runIdLoop,
          stepOrder,
          toIdx,
          ...
          {
            kind: 'loop_back',
            phase: loopState.iterations as number,
            next_phase: (loopState.iterations as number) + 1,
            to: loopBackCfg.to,
            from: stepName,
      ...
      for (let j = toIdx; j <= currIdx && j >= 0; j += 1) {
        ...
        const sData = (manifest.steps[sName] ?? {}) as Record<string, unknown>;
        sData.status = 'pending';
        delete sData.first_started_at;
        if (j !== currIdx && sData.loop_state !== undefined) delete sData.loop_state;
      ...
      return {
        ...
        params: manifest.params ?? {},
        engine_instructions:
          `LOOP_BACK — the engine has reset steps [${loopBackCfg.to} .. ${stepName}] to pending and ` +
    ...
    const runIdExit = (manifest.run_id as string | undefined) ?? '';
    if (!loopResolvedThisPass && exitIterations > 0 && runIdExit) {
      recordLoopTraceEvent(
        runtimeDir,
        runIdExit,
        stepOrder,
        stepOrder.indexOf(loopBackCfg.to),
        stepOrder.indexOf(stepName),
        {
          kind: 'loop_exit',
  ...
  } else if (typeof (args as { loop_decision?: string }).loop_decision === 'string') {
    logEngine(
      `Warning: step_complete received loop_decision for step '${stepName}' which has ` +
        `no loop_back block — ignored`,
    );
  }

  let routeProceedNote: string | null = null;
  const routesCfg = stepConfig?.routes as
    | {
        readonly when: { readonly script?: string; readonly semantic?: string; readonly human?: boolean };
        readonly define: ReadonlyArray<{ readonly id: string }>;
      }
  ...
  if (routesCfg && typeof routesCfg === 'object' && Array.isArray(routesCfg.define)) {
    const stepData = (manifest.steps[stepName] ?? {}) as Record<string, unknown>;
    const routeState = (stepData.route_state ?? {}) as {
      pending?: { script_route?: string; asked_at?: string } | null;
      [key: string]: unknown;
    };
    const evalR = evaluateRoutes(
      routesCfg,
      routeState,
      args as { route_decision?: string; route_rationale?: string },
      runtimeDir,
    );
    ...
    if (evalR.kind === 'error') {
      return {
        error:
          `routes evaluation failed for step '${stepName}': ${evalR.message}` +
    ...
    if (evalR.kind === 'awaiting') {
      ...
      return {
        ...
        engine_instructions: composeRouteDecisionInstructions(
          stepName,
          routesCfg,
          evalR.scriptRoute ?? routeState.pending?.script_route,
        ),
      } as unknown as StepCompleteOutput;
    }

    routeState.pending = null;
    routeState.last_decision = evalR.kind === 'route' ? (evalR.routeId as string) : 'proceed';
    routeState.last_decided_by = evalR.decidedBy;
    if (evalR.rationale !== undefined) routeState.last_rationale = evalR.rationale;
    stepData.route_state = routeState;
    manifest.steps[stepName] = stepData;

    if (evalR.kind === 'route') {
      const routeId = evalR.routeId as string;
      const routeDef = collectAllRoutes(workflow).find((r) => r.id === routeId);
      const routeSteps = (routeDef?.steps ?? []) as ReadonlyArray<Record<string, unknown>>;
      const returnTo = laneCtxOfStep
        ? staticNextOf(workflow, stepName)
        : computeProceedTarget(workflow, manifest, stepName);
      if (!laneCtxOfStep) {
        const stack = [
          ...((manifest.route_stack as Array<{ route_id: string; owner_step: string; return_to: string | null }> | undefined) ?? []),
        ];
        const activeNames = resolveActiveSequence(workflow, manifest).map((s) => s.name as string);
        const isLastInSeq = activeNames.indexOf(stepName) === activeNames.length - 1;
        if (stack.length > 0 && isLastInSeq) stack.pop();
      ...
      const ownerLaneStamp = laneCtxOfStep
        ? ((manifest.steps[stepName] as Record<string, unknown> | undefined)?.lane as
            | Record<string, unknown>
            | undefined) ?? {
      ...
      for (const rs of routeSteps) {
        const rsName = rs.name as string;
        const existing = (manifest.steps[rsName] ?? {}) as Record<string, unknown>;
      ...
      const firstStep = (routeSteps[0]?.name as string | undefined) ?? null;
      if (firstStep) setCursorFor(workflow, manifest, firstStep);
      safeWriteJson(manifestPath, manifest);
      const runIdR = (manifest.run_id as string | undefined) ?? '';
      if (runIdR) {
        ...
        recordRouteTraceEvent(runtimeDir, runIdR, {
          ...
          route_id: routeId,
          owner_step: stepName,
          steps: routeSteps.map((s) => s.name as string),
          decided_by: evalR.decidedBy ?? 'orchestrator',
        });
      }
      appendToolCallsToTrace(runtimeDir, freshToolEvents);
      const routeNextBegin =
        firstStep !== null
      ...
      return {
        ...
        params: manifest.params ?? {},
        engine_instructions:
          `ENTER_ROUTE — the engine selected route '${routeId}' and injected its steps into the ` +
    ...
    if (evalR.note) routeProceedNote = evalR.note;
  } else if (typeof (args as { route_decision?: string }).route_decision === 'string') {
    logEngine(
      `Warning: step_complete received route_decision for step '${stepName}' which has ` +
        `no routes block — ignored`,
    );
  }

  const lanesCfg = stepConfig?.lanes as
    | {
        readonly define: ReadonlyArray<{ readonly id: string; readonly steps: ReadonlyArray<Record<string, unknown>> }>;
        readonly join?: { readonly require?: string };
      }
    | undefined;
  if (lanesCfg && Array.isArray(lanesCfg.define) && lanesCfg.define.length > 0) {
    const stepDataFork = (manifest.steps[stepName] ?? {}) as Record<string, unknown>;
    const laneEntries: Record<string, LaneEntry> = {};
    const laneMap: Record<string, string> = {};
    lanesCfg.define.forEach((lane, laneNo) => {
      const laneId = lane.id;
      (lane.steps as ReadonlyArray<Record<string, unknown>>).forEach((rs, pos) => {
        const rsName = rs.name as string;
        const existing = (manifest.steps[rsName] ?? {}) as Record<string, unknown>;
        delete existing.first_started_at;
        delete existing.loop_state;
      ...
      const first = (lane.steps[0]?.name as string | undefined) ?? '';
    ...
    safeWriteJson(manifestPath, manifest);
    const runIdLanes = (manifest.run_id as string | undefined) ?? '';
    if (runIdLanes) {
      ...
      recordLaneTraceEvent(runtimeDir, runIdLanes, {
        ...
        lanes: lanesCfg.define.map((l) => ({
          lane_id: l.id,
          steps: (l.steps ?? []).map((st: Record<string, unknown>) => st.name as string),
        })),
      });
    }
    appendToolCallsToTrace(runtimeDir, freshToolEvents);
    const lanesBegin: Record<string, StepBeginOutput> = {};
    ...
    return {
      ...
      params: manifest.params ?? {},
      engine_instructions:
        `ENTER_LANES — the engine registered ${lanesCfg.define.length} parallel lanes: ` +
  ...
  let routeExit: { from: string; to: string; next_step: string | null } | null = null;
  let lanesExitInfo: Record<string, unknown> | null = null;
  if (laneCtxOfStep) {
    let cur = stepName;
    ...
    while (true) {
      ...
      if (anyRunning) {
        safeWriteJson(manifestPath, manifest);
        const runIdWait = (manifest.run_id as string | undefined) ?? '';
        if (runIdWait) flushTraceSnapshot(runtimeDir, runIdWait, manifest, stepName, workflow);
        appendToolCallsToTrace(runtimeDir, freshToolEvents);
        const running = Object.entries(ls.lanes)
          .filter(([, e]) => e.status === 'running' && e.cursor)
          .map(([id, e]) => ({ lane_id: id, next_step: e.cursor as string }));
        return {
          step: stepName,
          status: 'completed',
          action: 'LANE_WAIT',
          ...
          params: manifest.params ?? {},
          engine_instructions:
            `LANE_WAIT — lane '${cont.laneId}' of fork '${forkStep}' is complete, but ` +
      ...
      const joinCfg = ((forkCfg?.lanes as { join?: { require?: string } } | undefined)?.join ?? {});
      const requirePolicy = joinCfg.require === 'any' ? 'any' : 'all';
      const verdicts: Record<string, string> = {};
      for (const [id, e] of Object.entries(ls.lanes)) verdicts[id] = e.status;
      ...
      if (completedLanes.length === 0 || (requirePolicy === 'all' && failedLanes.length > 0)) {
        safeWriteJson(manifestPath, manifest);
        const runIdBlk = (manifest.run_id as string | undefined) ?? '';
        if (runIdBlk) flushTraceSnapshot(runtimeDir, runIdBlk, manifest, stepName, workflow);
        appendToolCallsToTrace(runtimeDir, freshToolEvents);
        ...
        return {
          ...
          params: manifest.params ?? {},
          error:
            `Join barrier of fork '${forkStep}' is blocked: require '${requirePolicy}' and ` +
      ...
      const joinIndex: Record<string, unknown> = { require: requirePolicy, lanes: {} };
      for (const lane of ((forkCfg?.lanes as { define?: ReadonlyArray<Record<string, unknown>> } | undefined)?.define ?? [])) {
        const laneId = lane.id as string;
        const outs: string[] = [];
        for (const st of collectAllSteps({ steps: lane.steps as unknown[] })) {
          for (const o of (st.outputs as ReadonlyArray<Record<string, unknown> | string> | undefined) ?? []) {
            const rawPath = typeof o === 'string' ? o : typeof o?.path === 'string' ? (o.path as string) : '';
            if (!rawPath) continue;
            const resolved = rawPath.replace(/\{([a-z][a-z0-9_.]*)\}/g, (m, key: string) => {
              const v = (manifest.params as Record<string, unknown> | undefined)?.[key];
              return typeof v === 'string' || typeof v === 'number' ? String(v) : m;
            });
            outs.push(resolved);
          }
        }
        (joinIndex.lanes as Record<string, unknown>)[laneId] = {
      ...
      const runIdJoin = (manifest.run_id as string | undefined) ?? '';
      if (runIdJoin) {
    ...
    if (nextStep) setCursorFor(workflow, manifest, nextStep);
  } else {
  ...
  if (nextStep) manifest.current_step = nextStep;
  }
  ...
  if (Object.keys(bindingsApplied).length > 0) result.param_bindings_applied = bindingsApplied;
  if (stepToolWarnings.length > 0) result.tool_warnings = stepToolWarnings;
  if (delegationWarning) result.delegation_warning = delegationWarning;
  if (resolvedChildRunId) result.delegated_run_id = resolvedChildRunId;
  if (handoffApplied.length > 0) result.delegated_outputs = handoffApplied;
  const instrParts: string[] = [];
  if (branchMergeInfo) {
    result.branch_merge = branchMergeInfo;
    if (branchMergeInfo.incomplete && branchMergeInfo.incomplete.length > 0) {
      instrParts.push(
        `WARNING — parallel branch merge INCOMPLETE: after a retry, files still remain under: ` +
      ...
      instrParts.push(
        `Parallel branch isolation is RESOLVED (verified): the engine merged ` +
  ...
  if (routeProceedNote) instrParts.push(routeProceedNote);
  if (lanesExitInfo) {
    result.lanes_exit = lanesExitInfo;
    const forkName = String((lanesExitInfo as { fork_step?: string }).fork_step ?? '');
    const verdictLine = Object.entries(
      ((lanesExitInfo as { lanes?: Record<string, string> }).lanes ?? {}),
    )
      .map(([id, v]) => `'${id}': ${v}`)
      .join(', ');
    instrParts.push(
      `JOIN — every lane of fork '${forkName}' reached a terminal state (${verdictLine}); ` +
        `the barrier passed (require: ${String((lanesExitInfo as { require?: string }).require)}) ` +
  ...
  const nextBegin =
    nextStep !== null && nextStep !== undefined
      ? composeNextBeginForAdvance({
          workflowName,
          workflow,
          defnDir,
  ...
  if (routeExit) {
    ...
    instrParts.push(
      `Route '${routeExit.from}' is finished — the engine returned the cursor to ` +
  ...
  if (instrParts.length > 0) result.engine_instructions = instrParts.join('\n\n');
  if (nextBegin !== null) result.next_begin = nextBegin;
  return result as unknown as StepCompleteOutput;
}


export function toolWorkflowFinalize(
  args: WorkflowFinalizeInput,
  paths: EnginePaths = defaultPaths(),
  overrides?: EngineRunOverrides,
): WorkflowFinalizeOutput {
  const workflowName =
  ...
  if (_ownFinalize) return _ownFinalize as unknown as WorkflowFinalizeOutput;
  ...
  const finalStatus: 'completed' | 'failed' = allDone ? 'completed' : 'failed';

  manifest.status = finalStatus;
  manifest.completed_at = now;
  manifest.updated_at = now;

  const promotedSteps = new Set<string>();
  ...
  if (manifest.scope_managed === true) {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error(
        `[workflow-engine] Warning: workflow_finalize could not restore ` +
  ...
  const wfSteps = (workflow.steps ?? []) as unknown as ReadonlyArray<Record<string, unknown>>;
  const runId = (manifest.run_id as string | undefined) ?? '';
  const tracePath = join(runtimeDir, 'trace.json');
  ...
  if (traceExists) {
    ...
    try {
      ...
      const started = (trace.started_at as string | undefined) ?? now;
      ...
      for (const stepEntry of trace.steps ?? []) {
        const invs = (stepEntry.invocations as Array<Record<string, unknown>> | undefined) ?? [];
        for (const inv of invs) {
          totalMsgs += (inv.message_count as number | undefined) ?? 0;
          totalTools += (inv.tool_call_count as number | undefined) ?? 0;
          const mf = (inv.modified_files as readonly string[] | undefined) ?? [];
          for (const fp of mf) allFiles.add(fp);
        }
      }

      let totalStepDuration = 0;
      for (const sdata of Object.values(manifest.steps ?? {})) {
        const dur = (sdata as { duration_ms?: number }).duration_ms;
        if (typeof dur === 'number' && dur > 0) totalStepDuration += dur;
      }
        ...
        if (!traceStepNames.has(sname)) {
          ...
          if (mstatus === 'completed' || mstatus === 'skipped') {
            ...
            const syntheticEntry: Record<string, unknown> = {
              ...
              inputs: s.inputs,
              outputs: s.outputs,
              started_at: (mstep as { started_at?: string }).started_at,
              completed_at: (mstep as { completed_at?: string }).completed_at,
              duration_ms: (mstep as { duration_ms?: number }).duration_ms,
              invocations: [],
              retry_count: 0,
              summary: (mstep as { summary?: string }).summary,
              tool_warnings: (mstep as { tool_warnings?: unknown }).tool_warnings,
              note,
            };
            if (isDelegation) {
              syntheticEntry.delegate_to = s.delegate_to as string;
            }
      ...
      if (promotedSteps.size > 0) {
        for (const traceStep of trace.steps ?? []) {
          if (traceStep.loop_archived === true) continue;
          const sname = traceStep.name as string;
          if (!promotedSteps.has(sname)) continue;
          traceStep.status = 'failed';
          const mstep = (manifest.steps[sname] ?? {}) as Record<string, unknown>;
          if (mstep.completed_at) traceStep.completed_at = mstep.completed_at;
      ...
      for (const traceStep of trace.steps ?? []) {
        ...
        if (tw && !('tool_warnings' in traceStep)) traceStep.tool_warnings = tw;
        const pf = mstep.prompt_files as readonly string[] | undefined;
        if (pf) {
          const enriched = pf.map((fp, i) => {
            const entry: Record<string, unknown> = { path: fp, branch_index: i };
            try {
        ...
        const planning = mstep.planning as Record<string, unknown> | undefined;
        if (planning) {
          traceStep.planning = { ...planning };
          const childWfPath = planning.child_workflow_path as string | null | undefined;
          const childRunId = planning.child_run_id as string | null | undefined;
          if (
            typeof childWfPath === 'string' &&
        ...
        const delegateTo = traceStep.delegate_to as string | undefined;
        if (typeof delegateTo === 'string' && delegateTo.length > 0) {
          try {
            const stampedChildId = (
              (manifest.steps?.[traceStep.name as string] as Record<string, unknown> | undefined)
                ?.delegation as { child_run_id?: string | null } | undefined
            )?.child_run_id;
            const resolvedForEmbed = resolveDelegatedRunDir(
              paths,
              delegateTo,
              typeof stampedChildId === 'string' ? stampedChildId : null,
              (manifest.run_id as string | undefined) ?? null,
              (traceStep.name as string | undefined) ?? '',
            );
            if (resolvedForEmbed.dir === null) throw new Error('unresolved child (no embed)');
            ...
            const childManifest = JSON.parse(
              readFileSync(childManifestPath, 'utf-8'),
            ) as Record<string, unknown>;
            const childRunId = childManifest.run_id as string | undefined;
            if (typeof childRunId === 'string' && childRunId.length > 0) {
              traceStep.child_trace_path = join(delegatedRunDir, 'trace.json');
              try {
                traceStep._embedded_child_trace = JSON.parse(
                  readFileSync(join(delegatedRunDir, 'trace.json'), 'utf-8'),
                );
              } catch {
              }
            }
      ...
      const matching: McpCallLogEntry[] = [];
      const remaining: McpCallLogEntry[] = [];
      for (const c of mcpCallLog()) {
        const matchesWorkflow = c.workflow === workflowName;
        const isUtilityCall =
          (c.tool === 'workflow_resolve' || c.tool === 'list_agent_files') &&
          (c.workflow === workflowName || c.workflow == null);
        if (matchesWorkflow || isUtilityCall) matching.push(c);
      ...
      clearMcpCallLog();
      for (const c of remaining) appendMcpCall(c);
  ...
  for (const [sname, sdata] of Object.entries(manifest.steps ?? {})) {
    stepStatuses[sname] = (sdata as { status?: string }).status ?? 'unknown';
  }
  ...
  return {
    ...
    run_id: runId,
  } as unknown as WorkflowFinalizeOutput;
}
...
export function fnmatch(name: string, pattern: string): boolean {
  ...
  while (i < normalizedPattern.length) {
    const c = normalizedPattern[i] as string;
    if (c === '*') {
      re += '.*';
      i += 1;
    } else if (c === '?') {
...
export function toolListAgentFiles(
  args: Record<string, unknown>,
): { files: readonly string[]; count: number } | { files: readonly []; error: string } {
  const path = (args.path as string | undefined) ?? `${PRODUCT_DIR}/`;
  ...
  if (!isDir) {
    return { files: [] as readonly [], error: `Directory not found: ${path}` };
  }
  ...
  for (const full of collected) {
    ...
    if (pattern && !fnmatch(baseName, pattern)) continue;
...
function walkSkipSymlinkDirs(dir: string, out: string[]): void {
