...
export function isRiglaneRoot(dir: string): boolean {
  try {
...
export function resolveProjectRoot(
  ...
  if (bakedRoot !== undefined && bakedRoot !== null && bakedRoot !== '') {
    ...
    if (isRiglaneRoot(baked)) return baked;
  }
  ...
  if (segIdx > 0) return start.slice(0, segIdx);
  ...
  let dir = start;
  for (;;) {
    if (isRiglaneRoot(dir)) return dir;
...
export function parseRootArg(argv: readonly string[]): string | null {
  for (let i = 0; i < argv.length; i += 1) {
    const a = argv[i] ?? '';
    if (a === '--root' && i + 1 < argv.length) return argv[i + 1] ?? null;
