...
export function readActiveWorkflow(root = '.'): string | null {
...
export function writeActiveWorkflow(name: string, root = '.'): void {
  const path = join(root, ACTIVE_WORKFLOW_FILE);
  mkdirSync(join(root, USER_STATE_DIR), { recursive: true });
  writeFileSync(path, `${name}\n`, 'utf-8');
}

export function clearActiveWorkflow(root = '.'): boolean {
  const path = join(root, ACTIVE_WORKFLOW_FILE);
...
): [string | null, ActiveWorkflowSource] {
