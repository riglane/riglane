...
export type TrustVerdict =
  | { readonly ok: true }
  | { readonly ok: false; readonly reason: 'untrusted' | 'changed'; readonly message: string };
...
export function trustStorePath(agentDir: string): string {
  return join(agentDir, 'local', TRUST_FILENAME);
}

const EMPTY_STORE: TrustStore = { version: 1, workflows: {} };

export function readTrustStore(agentDir: string): TrustStore {
  try {
    const raw = JSON.parse(readFileSync(trustStorePath(agentDir), 'utf-8')) as Record<string, unknown>;
    if (raw === null || typeof raw !== 'object') return EMPTY_STORE;
    if (raw.version !== 1 || raw.workflows === null || typeof raw.workflows !== 'object') return EMPTY_STORE;
    return raw as unknown as TrustStore;
  } catch {
    return EMPTY_STORE;
  }
}

export function writeTrustEntry(agentDir: string, workflowId: string, treeSha256: string): TrustEntry {
  const store = readTrustStore(agentDir);
  const entry: TrustEntry = { tree_sha256: treeSha256, trusted_at: toIsoLocal(new Date()) };
...
export function removeTrustEntry(agentDir: string, workflowId: string): void {
  const store = readTrustStore(agentDir);
  if (!(workflowId in store.workflows)) return;
...
export function computeTreeDigest(workflowDir: string): string {
  ...
  const visit = (rel: string): void => {
    ...
    for (const e of readdirSync(abs, { withFileTypes: true })) {
      ...
      if (rel === '' && e.name === 'entry.lock.yaml') continue;
...
export function checkCommunityTrust(agentDir: string, workflowId: string, workflowDir: string): TrustVerdict {
  ...
  if (entry === undefined) {
    return {
      ...
      reason: 'untrusted',
      message:
        `Workflow '${workflowId}' is installed from the catalog (community/) and has NOT been ` +
  ...
  try {
    ...
    return {
      ...
      reason: 'changed',
      message:
        `Workflow '${workflowId}' (community/) cannot be verified against its trust record: ${detail}` +
  ...
  if (currentDigest !== entry.tree_sha256) {
    return {
      ...
      reason: 'changed',
      message:
        `Workflow '${workflowId}' (community/) has CHANGED since it was trusted on ` +
