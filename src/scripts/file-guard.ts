...
function resolveOpts(opts: FileGuardOptions = {}): ResolvedOptions {
...
export function isProtected(filePath: string, cwd: string): boolean {
...
export const PROTECTED_SPEC_FILE_RE: readonly RegExp[] = [
...
export function protectedTrustFileReason(filePath: string, cwd: string): string | null {
  const normalized = normalizePath(filePath, cwd);
  if (!PROTECTED_TRUST_FILE_RE.test(normalized)) return null;
  return (
    `BLOCKED: '${filePath}' is the community-workflow trust store — engine-owned. ` +
...
export function protectedSpecFileReason(filePath: string, cwd: string): string | null {
  const normalized = normalizePath(filePath, cwd);
  if (!PROTECTED_SPEC_FILE_RE.some((re) => re.test(normalized))) return null;
  return (
    `BLOCKED: '${filePath}' is an engine-owned spec index/registry file — never ` +
...
const RUN_STATE_FILES: ReadonlySet<string> = new Set(['manifest.json', 'trace.json', 'gate-result.json']);

export function protectedRunStateReason(filePath: string, cwd: string): string | null {
  ...
  if (!isSnapshot && !isRunStateFile && !isInbox) return null;
  return (
    `BLOCKED: '${filePath}' is engine-owned run state under .riglane/local/workflow_runs/ ` +
...
export type BoundaryVerdict =
  ...
  | { readonly kind: 'warn'; readonly message: string; readonly runId: string };

function nowIsoNaiveLocal(): string {
  const d = new Date();
...
function outputPatternToRegex(pathTemplate: string, params: Record<string, unknown>): RegExp {
  const SEG_WILD = '\u0001';
  const DEEP_WILD = '\u0002';
...
function collectStepsWithOutputs(
  workflow: Record<string, unknown>,
): Array<{ name: string; outputs: string[] }> {
  const out: Array<{ name: string; outputs: string[] }> = [];
  ...
  visit(workflow.steps as ReadonlyArray<Record<string, unknown>> | undefined);
  return out;
}

function targetCandidates(filePath: string, cwd: string, runDir: string): string[] {
  const norm = (s: string): string => posix.normalize(s.replace(/\\/g, '/')).replace(/^\.\//, '');
  ...
  for (const c of [...cands]) {
    ...
    if (stripped !== c) cands.add(stripped);
  }
  return [...cands];
}

export function checkOutputBoundary(
  filePath: string,
  projectRoot: string,
  sessionCwd: string,
  ...
  try {
    ...
    if (!existsSync(runsRoot)) return { kind: 'allow' };

    const readManifest = (id: string): Record<string, unknown> | null => {
      try {
        const mp = runManifestPath(agentDir, id);
        if (!existsSync(mp)) return null;
        return JSON.parse(readFileSync(mp, 'utf-8')) as Record<string, unknown>;
      } catch {
        return null;
      }
    };
    ...
    if (!manifest) {
      ...
      for (const d of readdirSync(runsRoot, { withFileTypes: true })) {
        ...
        if (m && m.status === 'in_progress') live.push([d.name, m]);
      }
      const only = live.length === 1 ? live[0] : undefined;
      if (!only) return { kind: 'allow' };
      runId = only[0];
      manifest = only[1];
    }

    const stepName = manifest.current_step as string | undefined;
    if (!stepName || !runId) return { kind: 'allow' };
    if (Array.isArray(manifest.active_lanes) && (manifest.active_lanes as unknown[]).length > 0) {
      return { kind: 'allow' };
    }
    const stepData = (manifest.steps as Record<string, Record<string, unknown>> | undefined)?.[
      stepName
    ];
    if (stepData && stepData.planning !== undefined) return { kind: 'allow' };

    const wfName = manifest.workflow as string | undefined;
    if (!wfName) return { kind: 'allow' };
    ...
    if (matches(own.outputs)) return { kind: 'allow' };

    for (const other of steps) {
      if (other.name === own.name) continue;
      if (matches(other.outputs)) {
        return {
          kind: 'block',
          reason:
            `BLOCKED: '${filePath}' is a declared output of step '${other.name}', not of the ` +
    ...
    const targetAbs = posix
      .normalize(
        canonicalCompareForm(
          isAbsolute(filePath) ? filePath : join(sessionCwd, filePath),
        ).replace(/\\/g, '/'),
      )
      .toLowerCase();
    ...
    if (inRunDir) {
      return {
        ...
        runId,
        message:
          `[file-guard] run=${runId} step=${stepName}: write outside declared outputs: ` +
...
function appendBoundaryWarning(projectRoot: string, message: string): void {
  try {
    const agentDir = join(projectRoot, PRODUCT_DIR);
...
export function parseApplyPatchTargets(command: string): string[] {
  const out: string[] = [];
  const fileRe = /^\*\*\*\s+(?:Add|Update|Delete) File:\s*(.+?)\s*$/gm;
  const moveRe = /^\*\*\*\s+Move to:\s*(.+?)\s*$/gm;
  let m: RegExpExecArray | null;
  while ((m = fileRe.exec(command)) !== null) if (m[1]) out.push(m[1]);
  while ((m = moveRe.exec(command)) !== null) if (m[1]) out.push(m[1]);
  return out;
}

export function extractWriteTargets(hookInput: Record<string, unknown>): string[] {
  const toolName =
    typeof hookInput.tool_name === 'string'
      ...
      : typeof hookInput.toolName === 'string'
        ? hookInput.toolName
        : '';
  let toolInput: unknown = hookInput.tool_input !== undefined ? hookInput.tool_input : hookInput.toolArgs;
...
export async function runFileGuard(opts: FileGuardOptions = {}): Promise<number> {
  ...
  try {
    hookInput = raw.trim() ? (JSON.parse(raw) as Record<string, unknown>) : {};
  } catch {
    return 0;
  }
  ...
  for (const filePath of targets) {
    if (isProtected(filePath, r.cwd)) {
      return emitPreToolUseDeny(
        r.host,
        `BLOCKED: Cannot modify '${filePath}' — engine infrastructure file. Workflow steps must only write to their designated output paths.`,
        r,
      );
    }
