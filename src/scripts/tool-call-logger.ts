...
const MUTATION_TOOLS = new Set(
  ['Write', 'Edit', 'MultiEdit', 'NotebookEdit', 'apply_patch', 'create_file', 'create', 'str_replace_editor', 'write_file', 'replace'].map((t) =>
    t.toLowerCase(),
  ),
);
...
export function parseHost(argv: readonly string[]): LoggerHost | null {
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--host' && i + 1 < argv.length) {
      const v = argv[i + 1] ?? '';
      return (HOSTS as readonly string[]).includes(v) ? (v as LoggerHost) : null;
    }
...
function parseName(tool: string): { short: string; server: string | null } {
  if (tool.startsWith('mcp__')) {
    const seg = tool.split('__');
    return { short: seg[seg.length - 1] ?? tool, server: seg.length >= 3 ? (seg[1] ?? null) : null };
  }
  for (const server of ['workflow_engine', 'workflow_tools'] as const) {
...
export function classifyKind(tool: string, server: string | null): ToolKind {
  const t = tool.toLowerCase();
  if (server === 'workflow_tools' || t.includes('workflow_tools')) return 'script';
  if (t.startsWith('mcp__') || t.startsWith('mcp_') || t.startsWith('mcp:')) return 'mcp';
  if (tool === 'Bash' || t === 'bash' || t === 'shell' || t === 'powershell') return 'shell';
  if (t === 'run_shell_command') return 'shell';
  if (MUTATION_TOOLS.has(t)) return 'builtin';
  if (READ_ONLY_TOOLS.has(t)) return 'read';
  return 'other';
}

function sanitizeArgs(input: unknown): unknown {
  ...
  if (s.length <= MAX_ARGS_LEN) return input;
...
export function normalizeEvent(
  ...
  host: LoggerHost | null,
  cwd: string,
  now: string,
): ToolEventRecord | null {
  let tool = asStr(obj.tool_name) ?? asStr(obj.toolName) ?? '';
  if (!tool) {
    if (asStr(obj.command) !== null) tool = 'Bash';
    else if (asStr(obj.file_path) !== null) tool = 'Edit';
  }
  if (!tool) return null;
  ...
  if (
    (server === 'workflow_engine' || ENGINE_TOOL_NAMES.has(short)) &&
  ...
  const copilotResult =
    obj.toolResult !== null && typeof obj.toolResult === 'object'
      ? ((obj.toolResult as Record<string, unknown>).textResultForLlm ?? obj.toolResult)
      : undefined;
  const result =
  ...
  try {
    ...
    if (!runToken) runRef = extractRunIdRef(join(cwd, PRODUCT_DIR), fullArgsText);
  } catch {
    runToken = null;
    runRef = null;
  }
  ...
  return {
    ...
    corr:
      asStr(obj.conversation_id) ?? asStr(obj.turn_id) ?? asStr(obj.agent_id) ?? asStr(obj.sessionId),
    tool,
    tool_short: short,
    server,
    kind,
...
function resolveEventRun(cwd: string, record: ToolEventRecord): { runId: string; via: string } | null {
  const agentDir = join(cwd, PRODUCT_DIR);

  if (record.run_token) {
    const byToken = resolveRunByToken(agentDir, record.run_token);
    if (byToken) {
      if (record.corr) appendSessionMapEntry(agentDir, record.corr, byToken);
      const childSes = extractChildSessionId(record.result_preview);
      if (childSes) appendSessionMapEntry(agentDir, childSes, byToken);
      return { runId: byToken, via: 'token' };
    }
  }

  if (record.run_ref) {
    if (record.corr) appendSessionMapEntry(agentDir, record.corr, record.run_ref);
    return { runId: record.run_ref, via: 'run-ref' };
  }

  if (record.corr) {
    const mapped = lookupSessionMap(agentDir, record.corr);
    if (mapped && existsSync(runDirFor(agentDir, mapped))) return { runId: mapped, via: 'map' };
  }
  ...
  if (live.length === 1) return { runId: (live[0] as { runId: string }).runId, via: 'unique' };
  return null;
}

export async function runToolCallLogger(opts: ToolCallLoggerOptions = {}): Promise<number> {
  const argv = opts.argv ?? process.argv.slice(2);
  const cwd =
    opts.cwd ?? resolveProjectRoot(opts.startDir, parseRootArg(argv));
  if (cwd === null) {
    process.stderr.write(
      '[tool-call-logger] no riglane project root found from the hook cwd — event dropped\n',
    );
    return 0;
  }
  ...
  if (!resolved) {
    ...
    appendSpool(
      agentDir,
      record,
      liveCount === 0 ? 'no live run' : `ambiguous: ${liveCount} live runs, no token/map/env signal`,
    );
    return 0;
  }
  ...
  if (isMixedAdapterDuplicate(ledgerPath, finalRecord)) return 0;
...
export function isMixedAdapterDuplicate(
  ledgerPath: string,
  record: Pick<ToolEventRecord, 'corr' | 'host' | 'args' | 'ts'>,
): boolean {
  const corr = record.corr;
  const host = record.host;
  const args = (record.args ?? undefined) as Record<string, unknown> | undefined;
  const file = (args && (args.file_path ?? args.filePath ?? args.path ?? args.command)) as
    | string
    | undefined;
  const ts = typeof record.ts === 'string' ? Date.parse(record.ts) : NaN;
  const copilotPairCandidate = host === 'claude-code' || host === 'copilot';
  if ((!corr && !copilotPairCandidate) || !host || !file || Number.isNaN(ts)) return false;
  try {
    ...
    for (let i = lines.length - 1; i >= 0; i -= 1) {
      ...
      try {
        prev = JSON.parse(lines[i] as string) as Record<string, unknown>;
      } catch {
        continue;
      }
      const pTs = typeof prev.ts === 'string' ? Date.parse(prev.ts) : NaN;
      if (!Number.isNaN(pTs) && ts - pTs > MIXED_ADAPTER_DEDUP_MS) break;
      const pArgs = prev.args as Record<string, unknown> | undefined;
      const pFile = (pArgs &&
        (pArgs.file_path ?? pArgs.filePath ?? pArgs.path ?? pArgs.command)) as
        | string
        | undefined;
      if (
        prev.corr === corr &&
