...
import {
  lockedJsonReadModifyWrite,
  setFileLockErrorLogger,
} from '../engine/file-lock.js';
...
import {
  ...
  stepAwaitsExternalHuman,
  upsertBranch,
  type GateLedgerV2,
} from '../engine/gate-ledger.js';
import {
  injectSpecAuthoringOutputs,
  loadSnapshot,
  branchOutputsFromResolved,
  narrowOutputsForBranch,
  normalizePerIterationOutputs,
  validateOutputs,
} from '../engine/output-validator.js';
...
import { type EnginePaths, defaultPaths } from '../engine/workflow-engine.js';
...
export function gateCheckPaths(cwd?: string): GateCheckPaths {
...
export function findWorkflowDir(paths: GateCheckPaths = gateCheckPaths()): string | null {
  const runsRoot = runsRootDir(paths.agentDir);
  const envRun = process.env[ENV_RUN_ID];
  if (envRun) {
    const envDir = join(runsRoot, envRun);
    try {
      const envManifest = JSON.parse(readFileSync(join(envDir, 'manifest.json'), 'utf-8')) as {
        status?: string;
      };
      if (envManifest.status === 'in_progress' || envManifest.status === 'paused') return envDir;
    } catch {
    }
  }
  ...
  const considerManifest = (dir: string): void => {
  ...
  for (const rid of runIds) {
    ...
    let steps: string[];
    try {
      if (!statSync(dynamicDir).isDirectory()) continue;
...
export function detectRunAttributionWarning(
  paths: GateCheckPaths,
  workflowName: string,
  env: NodeJS.ProcessEnv = process.env,
): string | null {
  if (env[ENV_RUN_ID]) return null;
  const runsRoot = runsRootDir(paths.agentDir);
  let entries: string[];
  try {
  ...
  for (const rid of entries) {
    if (rid === 'index.jsonl') continue;
    try {
      const m = JSON.parse(readFileSync(join(runsRoot, rid, 'manifest.json'), 'utf-8')) as {
        workflow?: string;
        status?: string;
      };
  ...
  return (
    `Ambiguous run attribution: ${active.length} active runs of workflow ` +
...
export function findDefinitionDir(
  workflowDir: string,
  paths: GateCheckPaths = gateCheckPaths(),
): string {
  let workflowName = workflowDir.split(/[\\/]/).pop() ?? '';
  try {
    const m = JSON.parse(readFileSync(join(workflowDir, 'manifest.json'), 'utf-8')) as {
      workflow?: string;
    };
    if (typeof m.workflow === 'string' && m.workflow.length > 0) workflowName = m.workflow;
  } catch {
  }
  ...
  for (const c of candidates) {
    try {
      if (statSync(join(c, 'workflow.yaml')).isFile()) return c;
    } catch {
    }
  }
  try {
    if (statSync(join(workflowDir, 'workflow.yaml')).isFile()) return workflowDir;
  } catch {
  }
  return workflowDir;
}
...
export function findCurrentStep(
  ...
  const current = manifest.current_step as string | undefined;
...
export function definitionIndex(
  ...
  if (workflowConfig) {
    const steps = (workflowConfig.steps ?? []) as ReadonlyArray<Record<string, unknown>>;
...
export function findStepOutputs(
  workflowYaml: Record<string, unknown>,
  stepName: string,
): ReadonlyArray<unknown> {
  const step = findStepConfig(workflowYaml, stepName);
  return (step.outputs ?? []) as ReadonlyArray<unknown>;
}

export function getMaxGateRetries(
  workflowConfig: Record<string, unknown>,
  stepName: string,
): number {
  const defaultLimit = 5;
  const wfGate = (workflowConfig.gate ?? {}) as Record<string, unknown>;
  const workflowLimit = (wfGate.max_gate_retries as number | undefined) ?? defaultLimit;
  const step = findStepConfig(workflowConfig, stepName);
  const stepGate = (step.gate ?? {}) as Record<string, unknown>;
  return (stepGate.max_gate_retries as number | undefined) ?? workflowLimit;
}


export type { Host };

export function isCopilotSessionTranscript(transcriptPath: string): boolean {
  const norm = transcriptPath.replace(/\\/g, '/').toLowerCase();
  return norm.includes('/.copilot/') && norm.includes('/session-state/');
}

export function detectHost(hookInput: Record<string, unknown>): Host {
  if ('cursor_version' in hookInput || 'loop_count' in hookInput) return 'cursor';
...
export function extractFromTranscript(transcriptPath: string | null): TranscriptExtractResult {
  ...
  try {
    ...
    for (const rawLine of lines) {
      ...
      const blocks = msg.content as ReadonlyArray<unknown> | undefined;
        ...
        for (const blockRaw of blocks) {
          ...
          if (block.type === 'tool_use') {
            ...
            if (inp && typeof inp === 'object' && !Array.isArray(inp)) {
              ...
              if ((name === 'Write' || name === 'Edit') && fp) modified.add(fp);
            }
          }
        }
      }
...
export function deriveTaskTextFromTranscript(transcriptPath: string | null): string | null {
...
export function outputAndExit(
  ...
  scopeWarning: string | null = null,
): never {
  let finalFollowup: string | null = followup;
  if (scopeWarning) {
  ...
  if (host === 'claude-code') {
    ...
    if (followup) {
      process.stdout.write(
        `${JSON.stringify({ decision: 'block', reason: finalFollowup ?? '' })}\n`,
      );
    } else {
      if (scopeWarning) process.stderr.write(`${scopeWarning}\n`);
      process.stdout.write('{}\n');
...
export function logGateError(message: string, paths: GateCheckPaths = gateCheckPaths()): void {
  try {
...
export function detectCrossScopeWrites(modifiedFiles: readonly string[]): string | null {
  ...
  for (const fp of modifiedFiles) {
    ...
    if (pathScope === GENERIC_SCOPE) continue;
    if (pathScope === activeScope) continue;
  ...
  if (engineOwned.length > 0) {
    parts.push(
      `ENGINE-OWNED FILE WARNING: detected ${engineOwned.length} manual edit(s) of engine-owned _index.json/_registry.json under .riglane/specs/. These are derived + maintained by the engine (spec_write / spec_link) and must NEVER be hand-edited; the file-guard PreToolUse hook blocks them on Claude Code and Cursor (a drifted _index.json also self-heals via 'riglane doctor --fix'). Revert and use spec_write / spec_link (or spec_write op:update set_domain_description:true to change a domain description):\n${engineOwned.map((f) => `  - ${f}`).join('\n')}`,
    );
  }
  if (offenders.length > 0) {
    const lines = offenders.map(([fp, ps]) => `  - ${fp} (scope: ${ps})`);
    parts.push(
      `SCOPE WARNING: detected ${offenders.length} spec write(s) outside the active scope '${activeScope}'. There is NO live pre-write block for cross-scope spec writes on any host — this is a post-facto warning only. Review whether this was intentional:\n${lines.join('\n')}\nIf accidental: /riglane-scope-set to the correct scope and re-run with proper --scope param. Consider reverting the listed file(s).`,
    );
  }
...
export function buildTraceModifier(
  manifest: Record<string, unknown>,
  stepName: string,
  hookInput: HookInputShape,
  gateResult: GateResultShape,
  workflowConfig: Record<string, unknown> | null,
  loopCount: number,
  followupMessage: string | null = null,
  attributionWarning: string | null = null,
): TraceModifier {
  return (trace) => {
    ...
    let stepEntry: Record<string, unknown> | null = null;
    ...
    if (stepEntry === null) {
      ...
      const rawInputs = (stepCfg.inputs ?? []) as ReadonlyArray<unknown>;
      const rawOutputs = (stepCfg.outputs ?? []) as ReadonlyArray<unknown>;
      const inputsList: Array<Record<string, unknown>> = [];
      ...
      const outputsList: Array<Record<string, unknown>> = [];
    ...
    const invocations = (stepEntry.invocations ?? []) as Array<Record<string, unknown>>;
    const existingCount = invocations.length;
    const taskText = (hookInput.task as string | undefined) ?? '';
    let retryType: string | null;
    if (existingCount === 0) {
      retryType = null;
    ...
    const transcriptPath = (hookInput.agent_transcript_path as string | undefined) ?? null;
    const tx = extractFromTranscript(transcriptPath);

    const durationMs = (hookInput.duration_ms as number | undefined) || tx.duration_ms || 0;
    const model = (hookInput.model as string | undefined) || tx.model;
    const messageCount = (hookInput.message_count as number | undefined) || tx.message_count;
    const toolCallCount = (hookInput.tool_call_count as number | undefined) || tx.tool_call_count;
    const modifiedFiles =
      (hookInput.modified_files as readonly string[] | undefined) || tx.modified_files;
    const subagentId =
      (hookInput.subagent_id as string | undefined) || (hookInput.agent_id as string | undefined);
    ...
    const invocation: Record<string, unknown> = {
      ...
      subagent_summary:
        (hookInput.summary as string | undefined) ||
    ...
    stepEntry.retry_count = invocations.filter((inv) => {
      const rt = inv.retry_type as string | null | undefined;
      return rt !== null && rt !== undefined && rt !== 'branch';
    }).length;

    if (stepEntry.started_at) {
      const startMs = Date.parse(stepEntry.started_at as string);
    ...
    if (gateResult.passed) {
      ...
      const stepGate = ((stepCfg?.gate ?? {}) as Record<string, unknown>) ?? {};
      const mSteps = (manifest.steps ?? {}) as Record<
        string,
        { human_gate_verdict?: { required?: unknown } } | undefined
      >;
...
function nowIsoNaive(d: Date = new Date()): string {
  const pad = (n: number, w = 2): string => String(n).padStart(w, '0');
  const ms = pad(d.getMilliseconds(), 3);
  return (
    `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}` +
...
export async function appendTraceEntry(
  ...
  try {
    let runId = (manifest.run_id as string | undefined) ?? '';
    if (!runId) {
      const wfName = (manifest.workflow as string | undefined) ?? 'unknown';
      const d = new Date();
    ...
    const wfGate = (workflowConfig?.gate ?? {}) as Record<string, unknown>;
    const defaultTrace: Record<string, unknown> = {
      trace_version: 1,
    ...
    const modifier = buildTraceModifier(
      manifest,
      stepName,
      hookInput,
      gateResult,
...
export function resolveHost(hookInput: Record<string, unknown>, override: Host | null): Host {
  return override ?? detectHost(hookInput);
}

export async function mainInner(options: MainInnerOptions = {}): Promise<void> {
  ...
  try {
    ...
    if (raw.trim().length === 0) {
      ...
      logGateError(
        `Empty stdin received | raw_length=${rawBytes.length} | raw_repr=${repr}`,
        paths,
      );
    }
    hookInput = raw.trim().length > 0 ? (JSON.parse(raw) as Record<string, unknown>) : {};
  } catch (e) {
    ...
    if (isSyntax) {
      logGateError(
        `Invalid hook input JSON: ${errMsg} | raw_length=${len} | raw_repr=${safeRepr}`,
        paths,
      );
      process.stdout.write(
        `${JSON.stringify({ followup_message: `Gate error: invalid hook input — ${errMsg}` })}\n`,
      );
      process.exit(0);
    } else {
      logGateError(`Stdin decode error: ${errMsg} | raw_repr=${safeRepr}`, paths);
      process.stdout.write(
        `${JSON.stringify({ followup_message: `Gate error: stdin decode failed — ${errMsg}` })}\n`,
      );
      process.exit(0);
    }
    throw new Error('unreachable');
  ...
  if (testExit) {
    ...
    if (host === 'claude-code') {
      ...
      process.stdout.write(
        testCode !== 0 ? `${JSON.stringify({ decision: 'block', reason: msg })}\n` : '{}\n',
      );
      process.exit(0);
    } else {
      if (testCode !== 0) {
      ...
      try {
        ...
        if (gateDebug === '1') {
          ...
          body = `keys: [${keys.map((k) => `'${k}'`).join(', ')}]\nmodel: ${
            hookInput.model === undefined ? 'None' : JSON.stringify(hookInput.model)
          }\nstatus: ${
            hookInput.status === undefined ? 'None' : JSON.stringify(hookInput.status)
          }\n`;
        } else {
  ...
  try {
    const subagentIdRaw = (hookInput.subagent_id as string | null | undefined) ?? '';
    const taskRaw = (hookInput.task as string | null | undefined) ?? '';
    const statusRaw = (hookInput.status as string | null | undefined) ?? '';
    const hookEvent = (hookInput.hook_event_name as string | null | undefined) ?? '';
    const cursorVer = (hookInput.cursor_version as string | null | undefined) ?? '';
    const loopRaw = hookInput.loop_count;
    const payloadLine =
      `${nowIsoNaive()} | PAYLOAD | host=${host ?? 'null'} | status=${statusRaw}` +
  ...
  if (host === 'claude-code') {
    const tp = [hookInput.transcript_path, hookInput.agent_transcript_path].find(
  ...
  if (host === 'gemini') {
    const existingTask = (hookInput.task as string | null | undefined) ?? '';
    if (existingTask.trim().length === 0) {
      const ti = hookInput.tool_input as Record<string, unknown> | undefined;
      const prompt = typeof ti?.prompt === 'string' ? ti.prompt : '';
      if (prompt.trim().length > 0) hookInput.task = prompt;
    }
  }

  const nativeTaskText = (hookInput.task as string | null | undefined) ?? '';
  if (nativeTaskText.trim().length === 0) {
    const derived = deriveTaskTextFromTranscript(
      (hookInput.agent_transcript_path as string | null | undefined) ?? null,
    );
    if (derived) {
      hookInput.task = derived;
      logGateError(
        `task text derived from transcript (len=${derived.length})`,
        paths,
      );
    }
  }
  ...
  if (host === 'cursor') {
    const status = (hookInput.status as string | undefined) ?? 'unknown';
    if (status !== 'completed') {
      logGateError(`Skipping non-completed subagent: status=${status}`, paths);
      process.stdout.write(`${JSON.stringify({})}\n`);
      process.exit(0);
    }
    const taskPrompt = (hookInput.task as string | null | undefined) ?? '';
  ...
  const hostLoopCount = host === 'cursor' ? ((hookInput.loop_count as number | undefined) ?? 0) : 0;
  let loopCount = 0;

  let workflowDir: string | null = null;
  {
    const earlyTaskText = (hookInput.task as string | null | undefined) ?? '';
    const earlyToken = extractRunToken(earlyTaskText);
    if (earlyToken) {
      const byToken = resolveRunByToken(paths.agentDir, earlyToken);
      if (byToken) workflowDir = runDirFor(paths.agentDir, byToken);
    }
  }
  if (!workflowDir) workflowDir = findWorkflowDir(paths);
  if (!workflowDir) {
  ...
  try {
    ...
    workflowConfig = loadYaml<Record<string, unknown>>(workflowPath);
  ...
  let stepName: string;
  {
    const taskTextForStep = ((hookInput as Record<string, unknown>).task as string | undefined) ?? '';
    const stepMatch = /<!--workflow:step:(.+?)-->/.exec(taskTextForStep);
    const markerStep = stepMatch?.[1] ?? '';
    const manifestSteps = (manifest.steps ?? {}) as Record<string, unknown>;
    if (markerStep && markerStep in manifestSteps) {
      ...
      stepName = cursorStep as string;
    }
  }

  {
    const taskText = (hookInput.task as string | null | undefined) ?? '';
    ...
    if (rtMatch && manifestToken && rtMatch[1] !== manifestToken) {
      logGateError(
        `run_token mismatch: payload '${(rtMatch[1] ?? '').slice(0, 8)}…' != manifest ` +
          `'${manifestToken.slice(0, 8)}…' — skipping foreign fire`,
        paths,
      );
      outputAndExit(host, null);
    }
    if (!rtMatch && manifestToken && taskText.trim().length > 0) {
      logGateError(
        `no run_token marker in task text (len=${taskText.length}) — the stopping ` +
  ...
  const stepStructural = resolveStructuralGate(
    workflowConfig.gate,
    findStepConfig(workflowConfig, stepName).gate,
  );
  ...
  let firingBranch: number | null = null;

  if (!stepStructural) {
    const disabledDetail = STRUCTURAL_GATE_DISABLED_DETAIL;
    const pbDisabled = manifest.parallel_branches as Record<string, unknown> | undefined;
    ...
    const outputs = findStepOutputs(workflowConfig, stepName) as ReadonlyArray<Output | string>;
    const stepSnapshot = loadSnapshot(workflowDir, stepName);
    const stepsRecord = (manifest.steps ?? {}) as Record<string, Record<string, unknown>>;
    const stepEntry = stepsRecord[stepName] ?? {};
    const stepStartedAt =
      (stepEntry.first_started_at as string | undefined) ??
      (stepEntry.started_at as string | undefined) ??
      null;

    const parallelBranches = manifest.parallel_branches as Record<string, unknown> | undefined;
    const paramsForVal = (manifest.params ?? null) as Record<string, unknown> | null;
    const hasStructOutputs = outputs.some(
      (o) => typeof o === 'object' && o !== null && (o as Output).struct !== undefined,
    );
    ...
    if (parallelBranches && Object.keys(parallelBranches).length > 0) {
      ...
      let priorBranches: Record<string, { passed?: boolean }> = {};
      try {
        ...
        const prev = JSON.parse(readFileSync(priorPath, 'utf-8')) as Partial<GateLedgerV2>;
        if (prev.schema === GATE_LEDGER_SCHEMA && prev.step === stepName && prev.branches) {
          priorBranches = prev.branches as Record<string, { passed?: boolean }>;
        }
      } catch {
        priorBranches = {};
      }
      ...
      for (const bi of toValidate) {
        ...
        const waitForFiles =
          firingBranch !== null ? true : priorBranches[String(bi)]?.passed !== true;
        ...
        if (!r.passed && hasStructOutputs) {
          bDetails.push(
            `EMPTY — 0 output files. Subagent may have written to wrong path. Expected: data/*/_branch_${bi}/**`,
          );
        }
        observations.push({
          branchIndex: bi,
  ...
  const merged = await lockedJsonReadModifyWrite<GateLedgerV2>(
    resultPath,
    (prev) => {
      const ledger = ledgerForStep(prev, stepName, runToken);
      if (isParallelFire) {
        for (const o of observations) {
          const bi = o.branchIndex as number;
          const existing = ledger.branches[String(bi)];
          let lc: number;
          if (host === 'cursor' && capturedFiring === bi) lc = hostLoopCount;
          else if (!o.passed) lc = existing ? existing.loop_count + 1 : 0;
          else lc = existing ? existing.loop_count : 0;
          upsertBranch(ledger, {
            branchIndex: bi,
            passed: o.passed,
            checks: o.checks,
...
`, 'utf-8');
  ...
  const gateResult = ledger as unknown as Record<string, unknown>;
  ...
  if (!passed && loopCount >= maxGate) {
    logGateError(
      `Gate retry limit reached for step '${stepName}': ` +
        `${loopCount}/${maxGate} (failures: ${failures}/${checks})`,
      paths,
    );
    followup = null;
  }

  const attributionWarning = detectRunAttributionWarning(
    paths,
    (manifest.workflow as string | undefined) ?? '',
    procEnv,
  );
  if (attributionWarning) logGateError(attributionWarning, paths);
  await appendTraceEntry(
    workflowDir,
    manifest,
    stepName,
    hookInput,
    gateResult as GateResultShape,
    workflowConfig,
    loopCount,
    followup,
    attributionWarning,
  ...
  try {
    ...
    try {
      ...
      const line =
        `${ts} | RESULT | host=${host} | step=${stepName} | passed=${passedStr}` +
  ...
  try {
    let mf = (hookInput.modified_files as string[] | null | undefined) ?? [];
    if (mf.length === 0) {
      try {
        const tp = (hookInput.agent_transcript_path as string | undefined) ?? null;
        if (tp) {
...
export async function gateCheckCli(options: MainInnerOptions = {}): Promise<void> {
  const paths = options.paths ?? gateCheckPaths();
