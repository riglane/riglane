...
export function loadScopeConfig(root = '.'): ScopeConfig {
  ...
  let scopesRaw: Array<Record<string, unknown>>;
  if (Array.isArray(obj.scopes)) {
    scopesRaw = obj.scopes as Array<Record<string, unknown>>;
  } else if (!obj.scopes) {
    scopesRaw = [];
  } else {
...
export function saveScopeConfig(config: ScopeConfig, root = '.'): void {
  const path = join(root, SCOPE_CONFIG_PATH);
...
export function configExists(root = '.'): boolean {
  const path = join(root, SCOPE_CONFIG_PATH);
  return existsSync(path) && statSync(path).isFile();
}


export function readUserActiveScope(root = '.'): string | null {
...
export function writeUserActiveScope(scopeId: string, root = '.'): void {
...
export function clearUserActiveScope(root = '.'): boolean {
  const path = join(root, ACTIVE_SCOPE_FILE);
  if (existsSync(path) && statSync(path).isFile()) {
    unlinkSync(path);
    return true;
...
export function resolveActiveScope(cliScope?: string | null, root = '.'): [string, ScopeSource] {
  if (cliScope) return [cliScope, 'cli'];

  const userScope = readUserActiveScope(root);
  if (userScope) return [userScope, 'user'];

  const config = loadScopeConfig(root);
  const def = config.default_active_scope;
  if (def) return [def, 'project-default'];

  return [GENERIC_SCOPE, 'fallback'];
}


export function validateScopeId(scopeId: unknown): asserts scopeId is string {
  if (typeof scopeId !== 'string' || !SCOPE_ID_PATTERN.test(scopeId)) {
    throw new InvalidScopeIdError(
      `Invalid scope id '${String(scopeId)}'. Must match ${SCOPE_ID_PATTERN.source} (lowercase letter start, then lowercase/digits/hyphens only).`,
    );
  }
}

export function getAvailableScopes(root = '.'): AvailableScope[] {
  ...
  const config = loadScopeConfig(root);
  const seen = new Set<string>([GENERIC_SCOPE]);
  for (const entry of config.scopes) {
    if (entry === null || typeof entry !== 'object' || Array.isArray(entry)) continue;
    const sid = (entry as Record<string, unknown>).id;
    ...
    const hintRaw = (entry as Record<string, unknown>).hint;
...
export function getScopeHint(scopeId: string, root = '.'): string | null {
  try {
...
export function scopeExists(scopeId: string, root = '.'): boolean {
  return getAvailableScopes(root).some((s) => s.id === scopeId);
}

export function validateScopeExists(scopeId: string, root = '.'): void {
...
export function scopeDir(scopeId: string, root = '.'): string {
  validateScopeId(scopeId);
  return join(root, SPEC_ROOT, scopeId);
}

export function specDirFor(scopeId: string, domain: string, root = '.'): string {
  validateScopeId(scopeId);
...
export function ensureScopeDir(scopeId: string, root = '.'): string {
  validateScopeId(scopeId);
...
export function scopeFromPath(path: string, root = '.'): string | null {
  let p: string;
  let specRootAbs: string;
  try {
    p = resolve(path);
  ...
  const candidate = parts[0] as string;
  if (candidate.startsWith('_')) return null;
  return SCOPE_ID_PATTERN.test(candidate) ? candidate : null;
}


export function* iterSpecFiles(
  scopes: Iterable<string>,
  root = '.',
): Generator<string, void, void> {
  for (const scopeId of scopes) {
    if (!scopeExists(scopeId, root)) continue;
    const sdir = scopeDir(scopeId, root);
    if (!existsSync(sdir) || !statSync(sdir).isDirectory()) continue;
    yield* walkMd(sdir);
  }
}

function* walkMd(dir: string): Generator<string, void, void> {
...
export function resolveReadScopes(activeScope: string, includeGeneric = true): string[] {
...
export function formatScopeTable(scopes: AvailableScope[]): string {
  ...
  for (const s of scopes) {
    ...
    if (s.hint) lines.push(`  ${' '.repeat(widthId)}  ${' '.repeat(widthSrc + 2)}  ↳ ${s.hint}`);
  }
  return lines.join('\n');
}
