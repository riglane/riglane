...
export function ensureScopeInFrontmatter(content: string, scope: string): [string, boolean] {
  const m = FRONTMATTER_RE.exec(content);
  if (!m) return [content, false];

  const fmText = m[1] ?? '';
  if (/^scope:/m.test(fmText)) return [content, false];
...
function moveFsEntry(src: string, dst: string): void {
...
export function findSpecDomains(specsRoot: string, knownScopeIds: Set<string>): string[] {
  if (!existsSync(specsRoot) || !statSync(specsRoot).isDirectory()) return [];
  ...
  for (const entry of entries) {
    ...
    if (knownScopeIds.has(entry.name)) continue;
    if (entry.name.startsWith('_') || entry.name === 'local') continue;
...
function saveJson(path: string, data: unknown, dryRun: boolean, stdout: (s: string) => void): void {
  if (dryRun) {
...
export function rewriteIndexPaths(
  indexData: Record<string, unknown>,
  targetScope: string,
): Record<string, unknown> {
  const specs = (indexData.specs as Array<Record<string, unknown>> | undefined) ?? [];
  for (const entry of specs) {
    const fp = (entry.file_path as string | undefined) ?? '';
    let norm = fp;
    if (norm.startsWith(`${PRODUCT_DIR}/specs/`)) norm = norm.slice(`${PRODUCT_DIR}/specs/`.length);
    else if (norm.startsWith(`${PRODUCT_DIR}\\specs\\`)) norm = norm.slice(`${PRODUCT_DIR}\\specs\\`.length);
...
export function updateScopeConfig(
  ...
  const existingScopes = (config.scopes as Array<Record<string, unknown>> | undefined) ?? [];
  const existingIds = new Set(existingScopes.map((s) => s.id as string | undefined));
  if (existingIds.has(targetScope)) {
    stdout(`  SKIP        _scope-config.json already has '${targetScope}'\n`);
    return;
  }

  const entry: Record<string, unknown> = { id: targetScope };
  if (label) entry.label = label;
  if (!Array.isArray(config.scopes)) config.scopes = [];
  (config.scopes as Array<Record<string, unknown>>).push(entry);
...
export async function migrate(opts: MigrateOptions): Promise<number> {
  ...
  if (existsSync(targetScopeDir) && statSync(targetScopeDir).isDirectory()) {
    ...
    if (nonMgmt.length > 0 && !force) {
      w.stderr(
        `ERROR: ${targetScopeDir} already contains spec content:\n  ${JSON.stringify(nonMgmt)}\nUse --force to proceed anyway.\n`,
      );
      return 1;
    }
  }
  ...
  w.stdout(
    `Migrating specs -> scope '${targetScope}'\n` +
  ...
  if (existsSync(oldIndex) && statSync(oldIndex).isFile()) {
    ...
    saveJson(
      newIndex,
      { version: SPEC_INDEX_VERSION, scope: targetScope, domains: [], specs: [] },
      dryRun,
      w.stdout,
    );
  }
  ...
  if (existsSync(oldReg) && statSync(oldReg).isFile()) {
    ...
    saveJson(
      newReg,
      { version: SPEC_REGISTRY_VERSION, scope: targetScope, mappings: {} },
      dryRun,
      w.stdout,
    );
  }
  ...
  if (!existsSync(genericDir) || !statSync(genericDir).isDirectory() || !existsSync(genericIndex)) {
    saveJson(
      genericIndex,
      { version: SPEC_INDEX_VERSION, scope: 'generic', domains: [], specs: [] },
      dryRun,
      w.stdout,
    );
    saveJson(
      join(genericDir, '_registry.json'),
      { version: SPEC_REGISTRY_VERSION, scope: 'generic', mappings: {} },
      dryRun,
      w.stdout,
    );
  } else {
  ...
  updateScopeConfig(
    join(specsRoot, '_scope-config.json'),
    targetScope,
    label ?? null,
    dryRun,
    w.stdout,
...
export async function runScopeMigrateCli(
  ...
  try {
    parsed = parseArgs({
      ...
      options: {
        ...
        'project-root': { type: 'string' as const, default: '.' },
  ...
  if (!targetScope) {
    w.stderr(
      'usage: scope_migrate_flat_to_scoped <target_scope> [--label ...] [--project-root ...] [--dry-run] [--force]\n',
    );
    return 2;
  }
  if (parsed.positionals.length > 1) {
    w.stderr(
      `scope_migrate_flat_to_scoped: unrecognized arguments: ${parsed.positionals.slice(1).join(' ')}\n`,
    );
    return 2;
  }
  ...
  const labelVal = parsed.values.label as string | undefined;
  const migrateArgs: MigrateOptions = {
    targetScope,
    label: labelVal ?? null,
    projectRoot: resolve((parsed.values['project-root'] as string | undefined) ?? '.'),
